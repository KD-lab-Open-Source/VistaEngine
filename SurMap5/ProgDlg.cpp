...
CProgressDlg::CProgressDlg(UINT nCaptionID):stopPressed_(false)
{
...
void CProgressDlg::UpdateTimeElapsed()
{
	CWnd *pWndTimeElapsed = GetDlgItem(CG_IDC_PROGTIME_ELAPSED);
	time_t tmpTime;
    time(&tmpTime);
	tmpTime = tmpTime - startTime_; 
	CString strBuf;
	strBuf.Format(IDS_TIME_ELAPSED_LOAD_LANGUAGE, 
				gmtime(&tmpTime)->tm_hour,
				gmtime(&tmpTime)->tm_min,
				gmtime(&tmpTime)->tm_sec);
	CString strCur; // get current percentage
    pWndTimeElapsed->GetWindowText(strCur);

	if (strCur != strBuf) {
		pWndTimeElapsed->SetWindowText(strBuf);
		UpdateTimeLeft();
	}
}

void CProgressDlg::UpdateTimeLeft()
{
	CWnd *pWndTimeLeft = GetDlgItem(CG_IDC_PROGTIME_LEFT);
	time_t tmpTime;
    time(&tmpTime);
	tmpTime = tmpTime - startTime_; 
	time_t tmpTime_ = ((m_nUpper - m_nLower) * tmpTime) / m_Progress.GetPos();
	tmpTime = (0 > (tmpTime_ - tmpTime))? 0 : tmpTime_ - tmpTime;
	CString strBuf;
	strBuf.Format(IDS_TIME_LEFT_LOAD_LANGUAGE, 
				gmtime(&tmpTime)->tm_hour,
				gmtime(&tmpTime)->tm_min,
				gmtime(&tmpTime)->tm_sec);
	CString strCur; // get current percentage
    pWndTimeLeft->GetWindowText(strCur);

	if (strCur != strBuf)
		pWndTimeLeft->SetWindowText(strBuf);
}
...
bool CProgressDlg::BreakProcess()
{
	PumpMessages();
	bool res = stopPressed_;
	stopPressed_ = false;
	return res;
}
