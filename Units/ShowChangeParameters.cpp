	...
	for(int idx = 0; idx < showDelta.size(); ++idx)
		if(const ShowChangeSettings* param = owner->attr().getShowChangeSettings(showDelta[idx].index))
			if(param->showFlyParameters)
				switch(createType){
					case SET:
						controllers_.push_back(make_pair(showDelta[idx].index, environment->addShowChangeController(ShowChangeController(owner, param, 0, owner->attr().initialHeightUIParam))));
						break;
					case VALUES:
						controllers_.push_back(make_pair(showDelta[idx].index, environment->addShowChangeController(ShowChangeController(owner, param, showDelta[idx].value, owner->attr().initialHeightUIParam))));
						break;
					case SHOW:
						controllers_.push_back(make_pair(showDelta[idx].index, environment->addShowChangeController(ShowChangeController(owner, param, 0, owner->attr().initialHeightUIParam))));
