...
{
	Vect3f delta = To3Dzero(positionDelta_);
	...
	if(placementMode_ != PLACEMENT_NONE){
		pos.z = vMap.getVoxelW(pos.xi(), pos.yi());
...
bool EffectController::checkEnvironment(float radius)
{
	...
	if(!switchedOff && !onIce && !effectAttribute_->onWaterSurface() &&
		(!environment->waterIsLava() && effectAttribute_->switchOffUnderWater() ||
		environment->waterIsLava() && effectAttribute_->switchOffUnderLava()))
...
bool EffectController::effectStart(const EffectAttribute* attribute, float scale, sColor4c skin_color)
{
	release();
...
void EffectController::showDebugInfo() const
{
	...
	if(showDebugEffects.showName)
		show_text(owner()->position(), effectAttribute_->effectReference().c_str(), BLUE);
