...
void MicroAiScaner::operator()(UnitBase* unit)
{
	start_timer_auto();
	...
	if(owner_->isEnemy(unit)){
		...
		if(owner_->attr().attackTargetNotificationMode & AttributeBase::TARGET_NOTIFY_ALL){
			if(allies_.size() < SCAN_ALLIES_MAX){
				...
				if(ua->getUnitState() == UnitReal::AUTO_MODE && !ua->targetUnit()){
...
UnitInterface* MicroAiScaner::processTargets()
{
	start_timer_auto();
		...
		if(owner_->attr().attackTargetNotificationMode & AttributeBase::TARGET_NOTIFY_SQUAD){
			if(UnitSquad* sp = owner_->getSquadPoint()){
				...
				for(LegionariesLinks::const_iterator it = units.begin(); it != units.end(); ++it){
					...
					if(ua != first_owner && ua->getUnitState() == UnitReal::AUTO_MODE && !ua->targetUnit()){
						targets_.clear();
						owner_ = ua;
				...
				if(!leader_notify){
					UnitActing* ua = safe_cast<UnitActing*>(sp->getUnitReal());
...
bool MicroAiScaner::isTargetFree(const UnitInterface* target) const
{
...
void MicroAiScaner::addTarget(UnitInterface* target)
{
	TargetData data(target);
	targets_.push_back(data);
}

void MicroAiScaner::updateTarget(TargetData& target)
{
...
float MicroAiScaner::targetPriority(const TargetData& target,  const WeaponBase* weapon) const 
{
...
bool MicroAiScaner::isVisible(UnitBase* unit) const
{
	start_timer_auto();
	...
	if(owner_->player()->fogOfWarMap()){
		if(owner_->player()->fogOfWarMap()->checkFogStateInCircle(Vect2i(unit->position().xi(), unit->position().yi()), unit->radius()))
			return SourceShield::traceShieldsCoherence(owner_->position(), unit->position(), owner_->player()); //checkShield(unit);
		else
			return false;
	}
	else
...
bool MicroAiScaner::canAttack(UnitInterface* unit) const
{
	start_timer_auto();

	if(!weaponID_){
		if(!unit->attr().excludeFromAutoAttack && isVisible(unit)){
			...
			if(owner_->canAutoAttackTarget(WeaponTarget(unit))){
				if(unit->rigidBody() && unit->rigidBody()->isUnit() && safe_cast<RigidBodyUnit*>(unit->rigidBody())->isFlyDownMode())
					return false;
...
UnitInterface* MicroAiScaner::assignTargets(bool add_targets_to_list)
{
	start_timer_auto();
	...
	for(UnitActing::Weapons::const_iterator iw = owner_->weapons().begin(); iw != owner_->weapons().end(); ++iw){
		...
		if(weapon->isAccessible() && weapon->canAutoFire() && owner_->fireWeaponModeCheck(weapon)){
			...
			float priority = 0.f;
			if(target){
				if((!weapon->weaponPrm()->exclusiveTarget() || (!isTargetExcluded(target) && isTargetFree(target))) && isVisible(target) && weapon->checkTargetMode(target) && weapon->canAttack(WeaponTarget(target))){
					TargetData data(target);
					updateTarget(data);

					priority = targetPriority(data, weapon);
					if(weapon->fireDistanceCheck(WeaponTarget(target)))
			...
			for(Targets::iterator it = targets_.begin(); it != targets_.end(); ++it){
				if((!weapon->weaponPrm()->exclusiveTarget() || (!isTargetExcluded(it->target_) && isTargetFree(it->target_))) && weapon->checkTargetMode(it->target_) && weapon->canAttack(WeaponTarget(it->target_))){
					if(mode_ != ATTACK_MODE_DEFENCE || weapon->fireDistanceCheck(WeaponTarget(it->target_))){
...
UnitInterface* MicroAiScaner::selectedWeaponTarget()
{
	for(Targets::iterator it = targets_.begin(); it != targets_.end(); ++it)
		updateTarget(*it);
