...
public:
	...
	const char* tipsName() const { return tipsName_.c_str(); }
...
public:
	...
	std::string     editorGroupName() const{ return group_.c_str(); }
	static void     editorAddGroup(const char* name) { ParameterGroupTable::instance().add(name); }
	static const char* editorGroupsComboList() { return ParameterGroupTable::instance().comboList(); }
...
public:
	...
	void subClamped(const ParameterSet& rhs); // ќбрезает все, что ниже 0
	...
	void setRecovery(const ParameterSet& parameters); // ¬ыставл€ет [HEALTH_RECOVERY] и [ARMOR_RECOVERY] в [HEALTH] и [ARMOR] соответственно
	...
	void toString(XBuffer& out) const;
	void printSufficient(XBuffer& out, const ParameterSet& resource, const char* enableColor, const char* disableColor) const;
...
public:
	...
	void setInvertOnApply() { invertOnApply_ = true; unitType = TAKEN; }
	void setInverted() { inverted_ = true; restoreIterator_ = restoreVector_.begin(); }
...
private:
	bool invertOnApply_;
	bool inverted_;
	typedef vector<float> RestoreVector;
	mutable RestoreVector restoreVector_;
	mutable RestoreVector::const_iterator restoreIterator_;
};
...
struct ParameterShowSetting
{
	...
	/// ѕолоска
	sColor4f minValColor;
	sColor4f maxValColor;
	sColor4f borderColor;
