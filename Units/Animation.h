...
public:
	...
	void setChain(const AnimationChain& chain, UnitBase* owner, cObject3dx* model_, cObject3dx* modelLogic_, bool additionalChain = false);
	void stop(UnitBase* owner);
...
public:
	...
	void setChain(const AnimationChain* chain, bool additionalChain = false);
	bool setChainExcludeGroups(const AnimationChain* chain, vector<int>& animationGroups);
	void stop();
