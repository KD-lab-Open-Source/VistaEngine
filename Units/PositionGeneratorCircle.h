...
bool PositionGeneratorCircle<TUnitList>::checkConditions(const Vect2i pose, bool checkZones, bool checkUnits)
{
	...
	if(checkZones && currentUnit->rigidBody())
		zones = currentUnit->rigidBody()->checkImpassability(Vect3f(pose.x,pose.y,0.0f));
...
Vect2i PositionGeneratorCircle<TUnitList>::get(const UnitBase * unit, bool _checkZones = true, bool _checkUnits = true)
{
	...
	int a =0;
	do {
		point = centerPoint + addCircle(unit->radius() + radius).position;
...
{
	if(!unitInIgnore(p) && gridPoint.distance2(p->position2D()) < sqr(currentUnit->radius()+p->radius()) && p->checkInPathTracking(currentUnit)) 
