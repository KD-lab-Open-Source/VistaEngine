...
void ParameterSet::Value::serialize(Archive& ar)
{
	xassert(ar.inPlace());
}
...
bool ParameterSet::subClamped(const ParameterSet& rhs)
{
...
void ParameterSet::showDebug(const Vect3f& position, const Color4c& color) const
{
...
float ParameterSet::maxByType(ParameterType::Type type) const
{
	float result = 0;
	Values::const_iterator i;
	FOR_EACH(values_, i)
...
void ParameterSet::setRecovery(const ParameterSet& parameters)
{
	...
	FOR_EACH(parameters.values_, i){
		...
		else if(ParameterTypeTable::instance()[i->index].type() == ParameterType::OTHER_RECOVERY){
			for(int j = 0; j < ParameterTypeTable::instance().size(); j++)
				if(ParameterTypeTable::instance()[j].type() == ParameterType::OTHER && ParameterTypeTable::instance()[j].counter() == ParameterTypeTable::instance()[i->index].counter()){
					values_.push_back(Value(i->value, j));
					break;	  
				}
		}
...
void ParameterSet::addByIndex(float value, int index) 
{
	Values::iterator i;
	FOR_EACH(values_, i)
		if(i->index == index)
...
void ParameterSet::printSufficient(WBuffer& buffer, const ParameterSet& resource, const wchar_t* enableColor, const wchar_t* disableColor, const char* paramLabel) const
{
	...
	if(paramLabel && *paramLabel){
		...
		Values::const_iterator i = resource.values_.end();
		for(; j != i; ++j)
			if(!strcmp(ParameterTypeTable::instance()[j->index].label(), paramLabel)){
				for(i = values_.begin(); i != values_.end(); ++i)
					if(i->index > j->index)
						break;
					else if(i->index == j->index){
						buffer < (i->value < j->value ? disableColor : enableColor) <= round(j->value);
						break;
					}
				break;
			}
		...
		while(i != values_.end() && j != resource.values_.end()){
			if(i->index < j->index)
				...
				buffer < (i->value < j->value ? disableColor : enableColor) < ParameterTypeTable::instance()[i->index].tipsName() < L" " <= round(j->value) < L" ";
				++i;
				++j;
			}
			else
...
void ParameterSet::toString(WBuffer& buffer) const
{
	if(values_.empty())
		return;
	...
	FOR_EACH(values_, it)
		buffer < ParameterTypeTable::instance()[it->index].tipsName() < L" " <= round(it->value) < L" ";
}
...
void ArithmeticsData::apply(float& lvalue) const
{
	if(!inverted_){
 		switch(operation){
		case SET:
...
void ArithmeticsData::getUIData(WBuffer& buf) const
{
	buf < parameterType->tipsName();
...
//////////////////////////////////////////////////////
ParameterArithmetics::ParameterArithmetics(const ParameterArithmetics& origin, bool inverted)
{
	*this = origin;
	if(inverted){
		Data::iterator i;
...
bool ParameterArithmetics::getUIData(WBuffer& buf, const char* name) const
{
	Data::const_iterator i;
	FOR_EACH(data, i)
		if(strcmp(i->parameterType->label(), name)){
			i->getUIData(buf);
			return true;
		}
	return false;
...
void ParameterArithmetics::getUIData(WBuffer& buf) const
{
	Data::const_iterator i;
	FOR_EACH(data, i){
		i->getUIData(buf);
...
ParameterShowSetting::ParameterShowSetting() :
	...
	delayShowTime(0)
{
	minColor = Color4c::RED;
	maxColor = Color4c::GREEN;
	...
	case LOGIC:
		if(ar.isEdit()){
			...
			if(possibleParameters_ && !possibleParameters_->vector_.empty()){
				...
				paramTypeRef = possibleParameters_->vector_[newIndex]->type();
			}
		}
		else
...
void ParameterValue::editorAddGroup(const char* name) 
{ 
	ParameterGroupTable::instance().add(name); 
}

const char* ParameterValue::editorGroupsComboList() 
{ 
	return ParameterGroupTable::instance().comboList(); 
}

bool LookupParameter::operator()(const char* name, float& value)
{
