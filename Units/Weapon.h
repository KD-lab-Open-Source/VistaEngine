...
public:
	...
	virtual void showInfo(const Vect3f& pos) const;
	...
	bool fireDistanceCheck(const WeaponTarget& target) const;
	...
	bool getParametersForUI(const char* &name, char type, ParameterSet& out) const;
	...
	void reloadStart();
	bool isLoaded() const { return (state_ == WEAPON_STATE_LOAD && reloaded_); }
	...
	float fireDisp() const { return fireDisp_; }
	...
	int animationChainCounter() const;
	int animationChainAimCounter() const;
...
class WeaponCreatorBase
{
public:
	static void setParameters(WeaponSlot* slot, const WeaponPrm* prm = 0);

protected: 
	static WeaponSlot* slot_;
	static const WeaponPrm* weaponPrm_;
};

template<class Derived>
class ObjectCreator<WeaponBase, Derived> : public WeaponCreatorBase
{
public:
	static WeaponBase* create() { 
		return new Derived(slot_, weaponPrm_);
	}
};
