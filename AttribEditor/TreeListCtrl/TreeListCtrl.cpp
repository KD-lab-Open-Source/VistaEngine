...
LRESULT CTreeListCtrl::Notify( DWORD dwMessage, CTreeListItem* pItem, int iCol, int iMouseButton)
{
	// notify a message
	NMTREELIST nm;
...
bool CTreeListCtrl::SetItemText( CTreeListItem* pItem, int nSubItem, LPCTSTR lpszText)
{
	// set text of item specified by pItem
	ASSERT( pItem != NULL );
	bool bRet = pItem->SetText( lpszText, nSubItem );
...
void CTreeListCtrl::SetFont (CFont* pFont, BOOL bRedraw)
{
	ASSERT (pFont);
	LOGFONT lf;
	pFont->GetLogFont(&lf);
...
void CTreeListCtrl::SetFont()
{
	...
	{
/*		m_Font.DeleteObject ();
		m_Font.CreateFont (13, 0, 0, 0, FW_NORMAL, FALSE, FALSE, FALSE, RUSSIAN_CHARSET, OUT_DEFAULT_PRECIS,
...
void CTreeListCtrl::DrawItems( CDC* pDC, CRect rcClip )
{
	...
	int nFirstRow	=	GetScrollPos( SB_VERT );
	int nShowRows	=	m_rcTreeList.Height() ? (m_rcTreeList.Height() / GetItemHeight() + 1) : 1;
...
BOOL CTreeListCtrl::OnMouseWheel(UINT nFlags, short zDelta, CPoint pt) 
{
	...
	if(nPos > 0){
		for(int i = 0; i < 3; ++i)
			PostMessage(WM_VSCROLL, (WPARAM)SB_LINEUP);
	}
	else{
		for(int i = 0; i < 3; ++i)
			PostMessage(WM_VSCROLL, (WPARAM)SB_LINEDOWN);
	}
	return TRUE;
}
...
{
    Notify(TLN_BEFORESELCHANGED, 0, 0);
	if ((GetStyle()&TLC_MULTIPLESELECT))
	{
		...
		SelectItem(pItem, m_iSelCol, SI_SELECT, TRUE);
    Notify(TLN_AFTERSELCHANGED, 0, 0);
}
...
void CTreeListCtrl::OnMButtonDown(UINT nFlags, CPoint point)
{
	SetFocus();
	...
	LRESULT lResult = Notify(TLN_MCLICK, pItem, iSubItem, MK_MBUTTON);
	if (0 < lResult){
		if (!GetFocusedItem())
...
CTreeListItem* CTreeListCtrl::HitTest( CPoint pt, int* pFlag, int* pSubItem, CRect* prcText )
{
	...
	*pSubItem	= 0;
	...
	int nFirstRow	=	GetScrollPos( SB_VERT );
	int	nShowRows	=	m_rcTreeList.Height() ? (m_rcTreeList.Height() / GetItemHeight() + 1) : 0;
...
bool CTreeListCtrl::FinishModify()
{
	...
	if( m_ptrCustomEditControl && !UpdateModifyColumns() )
	{
		ASSERT( !GetState( TLS_MODIFY ) );
		SetState( TLS_MODIFY );
...
void CTreeListCtrl::AllowRedraw( BOOL bAllow )
{
	m_bAllowRedraw = bAllow;
	if(bAllow) {
		Layout();
		Invalidate(FALSE);
	}
