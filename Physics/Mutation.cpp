...
float mutation_upper_stiffness = 0.25f;
float mutation_lower_stiffness = 0.25f;

float mutation_compact_step = 1;
float mutation_compact_iterations = 20;
float mutation_compact_penetration_factor = 0.87f;
float mutation_compact_softer = 0.15f;

float mutation_base_volumes[3] = { 500, 500, 800 };

float mutation_specular_color[] = { 0.6f, 0.6f, 0.6f, 1 };

float mutation_move_sound_start_phase = 0.15f;

static float volume_power = 3;
static float Volume(float radius) { return 4./3*M_PI*pow(radius, 3); }
static float Radius(float volume) { return pow(volume*0.75/M_PI, 1./3.); }

sColor4f diffuse_color(1,1,1,1);
...
{
	start_autostop_timer(MutationProcessShow, STATISTICS_GROUP_LOGIC);
		...
		case PreMelting:
			{
				MTAuto lock(getLock());
				//FOR_EACH(source, bi)
				//	(*bi)->geometry->SetColor(0, &(diffuse_color*sColor4f(1 - t, 1 - t, 1 - t, 1)), &(sColor4f(mutation_specular_color)*t));
			}
			break;
		...
		case PostFreesing:
			{
				MTAuto lock(getLock());
				//FOR_EACH(target, bi)
				//	(*bi)->geometry->SetColor(0, &(diffuse_color*sColor4f(t, t, t, 1)), &(sColor4f(mutation_specular_color)*(1 - t)));
			}
			break;
		}
			...
			case MutationPhaseMax:
				{
					...
					//	(*bi)->geometry->SetColor(0, &diffuse_color, &sColor4f(0,0,0,0));
					//	}
				}
				break;
			}
