...
void WindMap::showDebugInfo()
{
	for(int i = 0; i < sizeX(); i++)
		for(int j = 0; j < sizeY(); j++) {
			...
			show_vector(point, Vect3f((*this)(i,j).x * 50.f, (*this)(i,j).y * 50.f, 0), GREEN);
			//Vect2f p=getPerlin(Vect2f(i,j));
			//show_vector(point, Vect3f(p.x * 50.f, p.y * 50.f, 0), GREEN);
...
void WindMap::init(WindMapAttributes& attributes)
{
	windType_ = attributes.windType_;
	direction_.normalize(attributes.windPower_);	
...
{
	clearMap();
	perlin_time=fast_fmod(perlin_time+0.1f,1000);
}
