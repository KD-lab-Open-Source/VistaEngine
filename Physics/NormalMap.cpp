...
void NormalMap::showDebugInfo()
{
	for(int i = 0; i < sizeX(); i++)
		for(int j = 0; j < sizeY(); j++) {
			Vect3f point(i << NormalMapTile::tileShift, j << NormalMapTile::tileShift, height(i,j));
			show_line(point, point + normal(i,j)*10.f, RED);
		}
}
...
void NormalMap::callcTileNormal( int x, int y )
{
	...
	int mapStep = 1 << NormalMapTile::tileShift;

	Vect3f tmp1(mapStep,0,height(x+1,y)-height(x,y));
	Vect3f tmp2(0,mapStep,height(x,y+1)-height(x,y));
	Vect3f tmp3(-mapStep,0,height(x-1,y)-height(x,y));
	Vect3f tmp4(0,-mapStep,height(x,y-1)-height(x,y));
