...
int RigidBody::testMissileShot(const RigidBodyPrm& prm, const Vect3f box_min, const Vect3f box_max, const RigidBody& firing_object, const Vect3f& position, const Vect3f& direction, RigidBody* target, const UnitBase& ownerUnit)
{
	RigidBody missile(ownerUnit);
	missile.build(prm, box_min, box_max, 1);
...
void RigidBody::startMissile(const RigidBody& firing_object, const Vect3f& position, const Vect3f& target_, const Vect3f& direction)
{
	...
	target = target_;
	target.z += prm().targetCorrectionZ;
...
void RigidBody::missile_quant(float dt)
{
	...
	if(waterAnalysis() && (target - position()).dot(target - posePrev_.trans()) < 0){
		colliding_ = TARGET_COLLIDING;
		pose_.trans() = target;
	} 
	else if(!ground_colliding_timer){
		if((colliding_ = checkCollisionPointed(position())) != 0){
			for(int i = 0; i < 8; i++){
...
void RigidBody::startDebris(const Vect3f& position, const Vect3f& velocity)
{
	...
	setAngularVelocity(Vect3f(logicRNDfrnd(1.f), logicRNDfrnd(1.f), logicRNDfrnd(1.f))*prm().debris_angular_velocity);
