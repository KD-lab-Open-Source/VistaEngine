...
LRESULT PASCAL wav_IO_proc(LPSTR lpmmioinfo,UINT wMsg,LPARAM lParam1,LPARAM lParam2)
{
	...
	static XZipStream fh(0);

	switch(wMsg){
	case MMIOM_OPEN:
		if(!fh.open((const char*)lParam1, XS_IN))
...
HRESULT CWaveFile::Open( const char* strFileName, WAVEFORMATEX* pwfx, DWORD dwFlags )
{
    ...
    if( m_dwFlags == WAVEFILE_READ )
    {
        ...
        m_hmmio = mmioOpen( (char*)strFileName, &inf, MMIO_ALLOCBUF | MMIO_READ );
    ...
    {
        m_hmmio = mmioOpen( (char*)strFileName, NULL, MMIO_ALLOCBUF  | 
                                                  MMIO_READWRITE | 
                                                  MMIO_CREATE );
        if( NULL == m_hmmio )
            return E_FAIL;
...
HRESULT CWaveFile::Read( BYTE* pBuffer, DWORD dwSizeToRead, DWORD* pdwSizeRead )
{
    ...
    else 
    {
		...
		if(mmioRead(m_hmmio, (HPSTR) pBuffer, cbDataIn) != (LRESULT) cbDataIn){
			return E_FAIL;
		}
		/*
