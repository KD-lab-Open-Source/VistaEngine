	...
	case PEER_ADD:
		{
			...
			//pGSI->gameHostList.push_back(sGameHostInfo(guid, "", "", name, sGameStatusInfo(4,1, gameRun, "", ZERO_GUID, 0,10)/*,server*/ ));
		}
		break;
	case PEER_REMOVE:
		{
	...
	case PEER_UPDATE:
		{
			...
			for(p=pGSI->gameHostList.begin(); p!=pGSI->gameHostList.end(); p++){
				...
				//	sGameStatusInfo gsi(4, 1, gameRun, 10, "", 0);
...
Peer2/peer/peerMain.h
Peer2/peer/peerMangle.h
...
void GameSpyInterface::StartGame()
{
