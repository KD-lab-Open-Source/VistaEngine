...
UniverseX::UniverseX(MissionDescription& mission, XPrmIArchive* ia)
: Universe(mission, ia), reelCheckFile_(0), debugCommandBuffer_(1024, 1)
{
	...
	//missionTemp.isMultiPlayer() ? NetClient : 0
	//pNetCenter = net_client;
	if(mission.isMultiPlayer() && PNetCenter::isNCCreated())
		pNetCenter = PNetCenter::instance();
	else
		pNetCenter = 0;
...
bool UniverseX::loadPlayReel(const char* fname)
{
	...
	xassert(fph.worldSignature == vMap.getCurrentGameMapCRC() && "Incompatible replay (worldSignature)"); 
...
bool UniverseX::savePlayReel(const char* _fname)
{
	...
	fph.worldSignature = vMap.getCurrentGameMapCRC();
...
long UniverseX::getInternalLagQuant()
{
...
bool UniverseX::MultiQuant()
{
	...
	if(allowedRealizingQuant > lastRealizedQuant){
		...
		for(; p!=fullListGameCommands.end(); p++) {
			if((*p)->curCommandQuant_ != currentQuant ) break; //�������� �� ����� ������ ������
			generalCommandCounter++; //only information
			receiveCommand(**p);
		}

		lagQuant=getInternalLagQuant(); //��� ������������
	...
	if(getNextQuantInterval())
		gameShell->setSpeed(clamp((float)logicTimePeriod/(float)getNextQuantInterval() + speedDelta_, 0.2f, 3.0f));
...
void UniverseX::logQuant()
{
	...
	if(pNetCenter && currentQuant!=0){
		...
		if((currentQuant & maskPeriodSendLogQuant)==0){ //������ 8 ����� ���������� ���������
...
{ 
	if(gameShell->isPaused(GameShell::PAUSE_BY_ANY)) // ��������, ����� ������� ������ �� �����
		return;
	...
	if(debugCommandBuffer_.filled_size > 1024){
		...
		InOutNetComBuffer in_buffer(debugCommandBuffer_.filled_size, false);
		in_buffer.putBufferPacket((unsigned char*)debugCommandBuffer_.buffer(), debugCommandBuffer_.filled_size);
		while(in_buffer.currentNetCommandID()!=NETCOM_None) {
			NCEventID event = (NCEventID)in_buffer.currentNetCommandID();
			switch(event){
			case NETCOM4G_UnitCommand: 
...
void UniverseX::drawDebug2D() const
{
	...
	if(DebugPrm::instance().showNetStat){
		...
		if(isMultiPlayer()){
			...
			//gb_RenderDevice->OutText(20, 40, msg, Color4f(1, 1, 1, 1));
			...
			//gb_RenderDevice->OutText(20, 40, msg, Color4f(1, 1, 1, 1));
			gb_RenderDevice->OutText(begXInfo, 20+40, msg, Color4f(1, 1, 1, 1));//360

			msg2 < pNetCenter->getStrWorkMode() < " " < pNetCenter->getStrState();
			msg2 < " byteR=" <= rb < " byteS=" <=sb;
			msg2 < " GCC=" <= generalCommandCounter;
			//gb_RenderDevice->OutText(20, 60, msg2, Color4f(1, 1, 1, 1));
			gb_RenderDevice->OutText(begXInfo, 35+40, msg2, Color4f(1, 1, 1, 1));//360
		}
#endif
	}
	...
	*p = char(0);
	...
	if(debugShowEnabled){
		...
		if(environment){
			...
			XBuffer buffer;
			activePlayer()->drawDebug2D(buffer);
			strcat(p, buffer);
		}
	}
	...
	if(*s){
		...
		gb_RenderDevice->OutText(10, 20, s, Color4f(1, 1, 1, 1));
	}
}

//�� ���� ����� ���������� �.�. reJoin �� �������� ���� ���� �� �����������(stopGame_HostMigrate)
...
{
	///if(pnc->curCommandQuant_ >= lastQuant_inFullListGameCommands) //� ������ ����� ����� - ���������� ����������� ��������
	if(pnc->curCommandQuant_ > allowedRealizingQuant) {//� ������ ����� ����� - ���������� ����������� ��������
		fullListGameCommands.push_back(pnc);
...
//		receiveCommand(*pnc);
		...
		//���������� ������������ �������
		if(pnc->flag_lastCommandInQuant_){ //�������� ���� ��������� � ������, ������� ��������� ����������� �� ������ ������������
	...
	case NETCOM4G_NextQuant:
		{
			...
			/*if(nc.numberQuant_ <= lastQuant_inFullListGameCommands) {
				XBuffer* pCurLogQuant=getLogInLogList(nc.numberQuant_); //� ������ ����� ����� - ���������� ����������� �������� �� �������� backGameInformation
				if(pCurLogQuant){
			...
			if(nc.quantConfirmation_!=netCommandNextQuant::NOT_QUANT_CONFIRMATION){ //������ quantConfirmation_ ���������� �� ���� ���������
...
#endif
			///xassert(nc.numberQuant_==(lastQuant_inFullListGameCommands+1));//�������� �� ������� ������(� ������ ���������!? ������) PS. ������ ��� ��������!
	...
	case NETCOM4C_SaveLog:
		{
			...
			pNetCenter->setGameDesynchronized();
...
void UniverseX::changeSelection(UnitInterface* oldUnit, UnitInterface* newUnit)
{
