...
//                          (LPVOID*) &m_pDPServer);
	...
	caps_ex.dwConnectTimeout = iniFile.network.dpConnectTimeout;
	caps_ex.dwConnectRetries = iniFile.network.dpConnectRetries;

	caps_ex.dwConnectTimeout = workMode == PNCWM_LAN ? iniFile.network.lanConnectTimeout : iniFile.network.inetConnectTimeout;
	caps_ex.dwConnectRetries = workMode == PNCWM_LAN ? iniFile.network.lanConnectRetries : iniFile.network.inetConnectRetries;
...
/*    if(port) {
	...
	if(flag_EnableHostMigrate) dpnAppDesc.dwFlags |= DPNSESSION_MIGRATE_HOST;
	...
	if( FAILED(hr) )
		DXTRACE_ERR_MSGBOX( TEXT("PNetCenter::ServerStart- start Host"), hr );
...
int PNetCenter::GetConnectionTimeout()
{
	//DPN_CAPS caps;
	//caps.dwSize = sizeof(DPN_CAPS);
	//m_pDPPeer->GetCaps(&caps, 0);
...
bool PNetCenter::isConnectedDP()
{
	return flag_connectedDP;//(m_mode != DP_NOBODY);
}
...
void PNetCenter::StopFindHostDP()
{
    HRESULT hr;
