...
public:
	...
	const char* missionDescription() const { return missionDescription_.c_str(); }
	...
	const char* playerName(int slotID, int cooperativeIdx) const;
	void setPlayerName(int slotID, int cooperativeIdx, const char* str);
	...
	static const char* getMapSizeName(float size);

	const char* getMapSizeName() const;
	const string& screenToPreload() const { return screenToPreload_; }
