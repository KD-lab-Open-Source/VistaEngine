...
netCommand4G_UnitListCommand::netCommand4G_UnitListCommand(const UnitInterfaceList& unitList, const UnitCommand& unitCommand, bool usePositionGenerator) 
:  unitCommand_(unitCommand), usePositionGenerator_(usePositionGenerator)
{
...
void netCommand4G_UnitListCommand::execute() const
{
	...
	if(usePositionGenerator_){
		PositionGeneratorCircle<UnitInterfaceLinks> positionGenerator;
		positionGenerator.init(0, unitCommand().position(), &unitList);
		UnitInterfaceLinks::iterator ui;
		FOR_EACH(unitList, ui){
			if((*ui)->getUnitReal()->canPlaiceInOnePoint())
				(*ui)->receiveCommand(unitCommand());
			else{
				UnitCommand command(unitCommand());
				command.setPosition(To3D(positionGenerator.get(*ui, false, false)));
				(*ui)->receiveCommand(command);
			}
		}
	}
	else if(unitCommand().commandID() != COMMAND_ID_ADD_SQUAD){
		UnitInterfaceLinks::iterator ui;
		FOR_EACH(unitList, ui)
			(*ui)->receiveCommand(unitCommand());
	}
