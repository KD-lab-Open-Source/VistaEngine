...
{
	return (float)powf(a, logf(b) / logf(0.5f));
}
...
static void gaussFilter(int * alt_buff, double filter_scaling, int xy_size)
{
	...
	int xx,yy;
	for(yy = 0;yy < xy_size;yy++){
		for(xx = 0;xx < xy_size;xx++){
			f = 0;
			for(y = -H;y < H;y++)
				for(x = -H;x < H;x++){
...
extern float turbulence(float point[3], float lofreq, float hifreq);
	...
	vMap.regRender(x - rad*2,y - rad,x + rad,y + rad, vrtMap::TypeCh_Height);
}
...
*/			
	...
	vMap.regRender(x_, y_, x_+sx_, y_+sy_, vrtMap::TypeCh_Height);
...
struct CWormForm {
	...
	CWormForm(){
		...
		if(tgahead.loadHeader(tb)){
			...
			xassert(tgahead.PixelDepth==8);
			tgahead.load2buf((unsigned char*)VBitMap.data, VBitMap.sx*VBitMap.sy*1);
			put2WF(0);
		}
		else{
...
/*		if(bForm0 == backupFormWorms0){
	...
	vMap.regRender(vMap.XCYCL(ChangeAreaXL), vMap.YCYCL(ChangeAreaYT), vMap.XCYCL(ChangeAreaXR), vMap.YCYCL(ChangeAreaYD), vrtMap::TypeCh_Height );
...
//		}
//		else {
//			wormFormLib.put2WF(alpha+M_PI_2, FormWormsNew);
//		}
		//ter.restore();
			...
			if( mask[yy*x_size + xx]!=0 ){
				for(y = -H;y < H;y++){
					for(x = -H;x < H;x++){
						...
						f += filter_array[H + y][H + x]*(double)vMap.getAlt(off);
					}
				}
				unsigned short v = round(f*norma_inv);
				vMap.putAltC(xx+begx, yy+begy, v);
			}
		}
	}
}
...
//	geoQuant();
	...
	vMap.regRender( vMap.XCYCL(begx-bsx05), vMap.YCYCL(begy-bsy05), vMap.XCYCL(begx+bsx05+nx), vMap.YCYCL(begy+bsy05+ny), vrtMap::TypeCh_Height|vrtMap::TypeCh_Texture );
...
//	else kScl=((MAX_VX_HEIGHT-h_begin)<<16)/(MAX_VX_HEIGHT);
	...
	const int kFirstScale = round((0.55f+logicRNDfrnd(0.15f))*(float)(1<<16));
	max_template_volcano_height = 0;
	XBuffer tfb;
	tfb < Path2TTerraResource < "volcano.dat";
	XZipStream fo(tfb, XS_IN);
...
void analyzeTerrain(Vect3f& position, float radius, Vect3f& outOrientation)
{
	...
	for(int y = -D; y <= D; y++)
		for(int x = -D; x <= D; x++){
			...
			int z = vMap.getAlt(vMap.XCYCL(x + x0), vMap.YCYCL(y + y0));
			int dz = z - z0;
...
//Выпячивание
	...
	////!int curKFluct=round((1.5+logicRNDfrnd(K_FLUCTUATION))*(1<<16));
	...
	vMap.regRender(vMap.XCYCL(curX-bubleWSXSY05-1), vMap.YCYCL(curY-bubleWSXSY05-1), vMap.XCYCL(curX+bubleWSXSY05+1), vMap.YCYCL(curY+bubleWSXSY05+1), vrtMap::TypeCh_Height|vrtMap::TypeCh_Texture );
...
//
	...
	for(i=0; i<num_bubble; i++){
		...
		float MAX_D=50.f;
		float r=abs((sqrt(-log(0.020f+logicRNDfabsRnd(1.f-0.021f)))-1.f)*MAX_D);
...
sUPoligon::sUPoligon(point4UP* _pArr) 
{
	...
	if(DY1>0){
		...
		for(yWrk=a.y; yWrk<b.y; yWrk++){
			xL=xLp>>16; xR=xRp>>16;
			for(xWrk=leftBorder; xWrk< xL; xWrk++) {arrayA[cnt]=0; arrayVx[cnt++]=vMap.getAlt(vMap.XCYCL(xWrk), vMap.YCYCL(yWrk));}
			for(/*xWrk=xL*/; xWrk<=xR; xWrk++) {
				arrayA[cnt]=1;
				short vx=vMap.getAlt(vMap.XCYCL(xWrk), vMap.YCYCL(yWrk));
				if(vx<vMin)vMin=vx;
				if(vx>vMax)vMax=vx;
				arrayVx[cnt++]=vx;
			}
			for(/*xWrk=leftBorder*/; xWrk<=rightBorder; xWrk++){arrayA[cnt]=0; arrayVx[cnt++]=vMap.getAlt(vMap.XCYCL(xWrk), vMap.YCYCL(yWrk));}
			xLp+=dL; xRp+=dR;
		}
	}
		...
		for(yWrk=round(a.y); yWrk<round(b.y); yWrk++){
			...
			if(xL!=xR){
				for(xWrk=xL; xWrk<=xR; xWrk++) {
					...
					for(dy = -1;dy <= 1; dy++){
						for(dx = -1; dx <= 1; dx++){
							v += vMap.getAlt(vMap.XCYCL(xWrk+dx), vMap.YCYCL(yWrk +dy));
						}
					}
					v/=9;
					vMap.putAlt(xWrk, yWrk, v);
				}
			}
			xLp+=dxL; xRp+=dxR;
		}
...
loc_begDraw2Part:
	...
	vMap.regRender(vMap.XCYCL(leftBorder), vMap.YCYCL(upBorder), vMap.XCYCL(leftBorder+sx), vMap.YCYCL(upBorder+sy), vrtMap::TypeCh_Height );
}
		...
		for(yWrk=round(a.y); yWrk<round(b.y); yWrk++){
			...
			if(xL!=xR){
				...
				for(xWrk=xL; xWrk<=xR; xWrk++) {
					...
					vMap.putAlt(vMap.XCYCL(xWrk), vMap.YCYCL(yWrk), round( (-(cD+cA*xWrk+cB*yWrk)/cC)*(1<<VX_FRACTION)+addVx) );
					//arrayVx[cnt++]=vx;
					zCur+=dzCur;
					uCur+=duCur;
					vCur+=dvCur;
...
bool poiligonUP(int _x, int _y)
{
	...
	if(flag_init==0) {
		...
		for(i=0; i<=MAX_CROSS_POINT; i++){
			float angleNorm=MIN_ANGLE_NORMAL+logicRNDfabsRnd(MAX_ANGLE_NORMAL-MIN_ANGLE_NORMAL);
			...
			prevPoint=curPoint;
			flag_right^=1;
			float SPEED=AVERAGE_SPEED+logicRNDfrnd(DELTA_SPEED);
	...
	if(pointArr.size()>=3){
		...
		if(step<pointArr.size()-1) { //от второго до предпоследнего
			...
			v1.normalize(1.f);
			v1*=distance1*(1.f/2.f)+logicRNDfabsRnd(distance1*(1.f/2.f)); // 1/2   2/3
			v1+=pointArr[step];

			float distance2=pointArr[step+1].distance(pointArr[step]);
			Vect2f v2=pointArr[step+1] - pointArr[step];
			v2.normalize(1.f);
			v2*=distance1/2.f+logicRNDfabsRnd(distance2*(1.f/2.f)); //  2/3  2/3
			v2+=pointArr[step];
...
{
	//const float begAngle=logicRNDfabsRnd(2*M_PI);//-M_PI/10.;
	...
	if(pointArr.size()>=3){
		...
		if( step!=round(fstep) ){
			...
			if(step<pointArr.size()-1) { //от второго до предпоследнего
				...
				Vect2f v1=pointArr[step-1] - pointArr[step];
				v1.normalize(1.f);
				v1*=distance1*(4.f/6.f)+logicRNDfabsRnd(distance1*(2.f/6.f)); // 1/2   2/3
				v1+=pointArr[step];
				...
				v11.normalize(1.f);
				v11*=distance1*(0.f/3.f)+logicRNDfabsRnd(distance1*(3.f/5.f)); // 1/2   2/3
				v11+=pointArr[step];
				...
				Vect2f v2=pointArr[step+1] - pointArr[step];
				v2.normalize(1.f);
				v2*=distance2*4.f/6.f+logicRNDfabsRnd(distance2*(2.f/6.f)); //  2/3  2/3
				v2+=pointArr[step];

				if(distance2>MAX_DISTANCE)distance2=MAX_DISTANCE;
				Vect2f v22=pointArr[step+1] - pointArr[step];
				v22.normalize(1.f);
				v22*=distance2*(0.f/3.f)+logicRNDfabsRnd(distance2*(3.f/5.f)); //  2/3  2/3
				v22+=pointArr[step];
...
//	step++;
	fstep+=0.33f + logicRNDfrnd(0.17f);
...
sGeoSwelling::sGeoSwelling(int xCentre, int yCentre)
{
	...
	for(i=0; i<numPoligons; i++){
		angleArr[i]=MIN_ANGLE_GEO_POLIGON+logicRNDfabsRnd(DELTA_ANGLE_GEO_POLIGON);
		sumNorm+=angleArr[i];
	}
				...
				for(i=0; i<nElAET; i+=2){
					...
					for(; xWrk<=(AET[i+1].x>>16); xWrk++) {
						...
						/*else*/ {
							...
							//vMap.putAlt(vMap.XCYCL(xWrk), vMap.YCYCL(yWrk), VVV );
							vMap.putAlt(offY+vMap.XCYCL(xWrk), VVV);
							//arrayVx[cnt++]=vx;
						}
						//zCur+=dzCur;
						uCur+=duCur;
...
void sUPoligonN::addAETRecord(char idxFrom, char idxTo, char dir) 
{
...
void sUPoligonN::delAETRecord(char idxAET) 
{
...
bool meshM2VM::put2KF(int quality, short * KFArr, int sxKF, int syKF, bool flag_inverse, short addH)
{
	...
	for (i = 0; i < numVrtx; i++) {
		...
		//my+=position.y;
		//mz+=position.z;
		//mx*=SCALING4PUT2EARCH;
		//my*=SCALING4PUT2EARCH;
	...
	for (i = 0; i < numVrtx; i++){
		...
		vrtx[i].isx=round(vrtx[i].sx*(1<<16));//+(1<<15);
		vrtx[i].isy=round(vrtx[i].sy*(1<<16));//+(1<<15);
		vrtx[i].iz1=round(vrtx[i].z1*(1<<16));//+(1<<15);
	}
...
//					zBuffer[(a->isy-minY)*sizeX + a->isx-minX] = a->iz1;
//				}
//			}
			continue;
		}
		...
		x_end = b->isx;
		z1_end = b->iz1;
		if(roundFInt(x_start - x_end)==0) continue;
		else dz1 = (z1_start - z1_end) / roundFInt(x_start - x_end); 

		x_start = a->isx;
		z1_start = a->iz1;
		dx_start = (c->isx - a->isx) / roundFInt(c->isy - a->isy); 
		dz1_start = (c->iz1 - a->iz1) / roundFInt(c->isy - a->isy); 
//#ifdef SUBPIXEL
		//tmp = ceil(a->sy) - a->sy;
...
//#endif
		...
		if (roundFInt(b->isy) > roundFInt(a->isy)) {
			//tmp = ceil(a->sy) - a->sy;
			tmp = ((a->isy+(1<<15))&0xFFff0000) - (a->isy);
			x_end = a->isx;
			z1_end = a->iz1;
			dx_end = (b->isx - a->isx) / roundFInt(b->isy - a->isy);
			dz1_end = (b->iz1 - a->iz1) / roundFInt(b->isy - a->isy);
		} else {
			//tmp = ceil(b->sy) - b->sy;
//?			tmp = ((b->isy+(1<<15))&0xFFff0000) - b->isy;
//?			x_end = b->isx;
//?			z1_end = b->iz1;
//?			dx_end = (c->isx - b->isx) / roundFInt(c->isy - b->isy);
...
loc_scip01:;
		...
		for (current_sy = roundFInt(a->isy); current_sy <= roundFInt(c->isy); current_sy++) {
			...
			if (current_sy == roundFInt(b->isy)) {
				if(roundFInt(c->isy - b->isy)==0) goto loc_scip02;
				x_end = b->isx;
				z1_end = b->iz1;
				dx_end = (c->isx - b->isx) / roundFInt(c->isy - b->isy);
				dz1_end = (c->iz1 - b->iz1) / roundFInt(c->isy - b->isy);
...
//#endif
			...
			if (x_start > x_end) {
			  ...
			  length = roundFInt(x_start) - roundFInt(x_end)+1;
			} else {
			  x = x_start;
			  z1 = z1_start;
...
s_Mesh2VMapDispather::s_Mesh2VMapDispather()
{
	...
	if(flag_move){
		...
		for(i=0; i<meshDate->sizeY; i++){
			...
			for(j=0; j<meshDate->sizeX; j++){
				...
				short vv=vMap.getAlt(off);
				short restoreV=inVx[cnt]+vv-tmpVx[cnt];
				vMap.putAlt(off, restoreV);
...
/*				//int inx=(Vold-V)>>3;
	...
	vMap.regRender(xL, yL, xL+sx, yL+sy, vrtMap::TypeCh_Height);

	if(flag_move){
		vMap.recalcArea2Grid(vMap.XCYCL(nxL-1), vMap.YCYCL(nyL-1), vMap.XCYCL(nxL + sx+1), vMap.YCYCL(nyL + sy+1) );
		vMap.regRender(nxL, nyL, nxL+sx, nyL+sy, vrtMap::TypeCh_Height);
		nxL=xL; nyL=nyL;
	}
	flag_move=0;
...
c3DSGeoAction* c3DSGeoActionCreator::BuildD(short xc, short yc, float orientation,s3DSGeoParameter* command)
{
	...
	s_EarthUnit* pEarthUnit=0;
	float tmp=fmodFast(orientation, 2*M_PI);
	if(tmp<0)tmp+=2*M_PI;
...
s_HeadGeoAction::s_HeadGeoAction()
{
	static bool flag_first=0;
	if(flag_first==0){
		flag_first=1;
...
CWormFormLib::CWormFormLib() 
{
	XBuffer tb;
	//tb < dirBitMap   < "\\" < fname;
	tb < Path2TTerraResource < "wormform.tga";
	TGAHEAD tgahead;
	if(tgahead.loadHeader(tb)){
		if((tgahead.PixelDepth!=8) || (tgahead.ImageType!=3)) {
		...
		xassert(tgahead.PixelDepth==8);
		tgahead.load2buf((unsigned char*)VBitMap1.data, VBitMap1.sx*VBitMap1.sy*1);
	}
	else{
...
void CWormFormLib::put2WF(float alpha, tFormWorm* FormWormsNew) 
{
...
void CWormFormLib::put2WF2(tFormWorm* FormWormsNew) 
{
	int i,j;
	for(i=0; i<WSXSY; i++){
		float y=(float)(i-WSXSY05)/(float)WSXSY05;
...
srBmp::srBmp(float _alpha, int _sx, int _sy) 
{
	alpha=_alpha;
