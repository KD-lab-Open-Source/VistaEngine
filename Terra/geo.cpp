...
struct CWormForm {
	...
	CWormForm(){
		...
		if(tgahead.loadHeader(tb)){
			...
			tgahead.load2buf(VBitMap.data, VBitMap.sx*VBitMap.sy*1);
			put2WF(0);
		}
		else{
			...
			if(dh>0){
				...
				if(v>curV) {
					*(deltaVx+cnt)=v-curV ;
					...
					surVx[cnt]=sur[off];
					if(Vm_IsDam(vMap.VxABuf[off])) surVx[cnt]|=256;//2^8
				}
				else *(deltaVx+cnt)=0;
			}
			else *(deltaVx+cnt)=0;
...
CWormFormLib::CWormFormLib() 
{
	...
	if(tgahead.loadHeader(tb)){
		...
		tgahead.load2buf(VBitMap1.data, VBitMap1.sx*VBitMap1.sy*1);
	}
	else{
