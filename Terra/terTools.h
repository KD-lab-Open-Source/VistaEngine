...
public:
	...
	struct Bitmap8C {
		...
		bool loadAndPutBitmap2Quantizer(const char* name, const float terTextureKScale, ColorQuantizer& cq);//��� ������������ ������ ������� �������� � �����
	...
	struct Bitmap8V {
		...
		bool load(const char* fname, const float terTextureKScale, const float terTextureVScale);
	};
	...
	~TerToolsDispatcher();
	void prepare4World();
	template <class T> void prepare4WorldT();
	template <> void prepare4WorldT<unsigned char>();
	template <> void prepare4WorldT<unsigned short>();
	...
	Bitmap8C* getBitmap8C(const char* fname, float kScale){
		...
		XBuffer buf;
		buf < "TerTool bitmapC not found!-" < fname < "scale-" <= kScale;
		xxassert(0 , buf);
		return 0;
	}
	...
	Bitmap8V* getBitmap8V(const char* fname, float kScale){
		...
		//xassert(0&&"TerTool bitmap not found!");
		XBuffer buf;
		buf < "TerTool bitmapV not found!-" < fname < "scale-" <= kScale;
		xxassert(0 , buf);
		return 0;
	}
...
protected:
	...
	bool prepareTerTools4World(const TerToolBase* pTerTool, int indexIn);
	...
	static TerToolsID unengagedTerToolID; //���������������� � cpp
...
public:
	TerToolBase(){ 
		...
		//flag_enableDrawDetailTexture=false;
		layerDetailTexture=0;
		layerZonePlacement=0;
		detaledTextueScale_=1.f;
		flag_stopOnInds=false;
		flag_changeTerrainType=false;
		newTerrainType = TERRAIN_TYPE0;
		memset(surKind4Work, true, sizeof(surKind4Work));
		kScaleTerrainType=1.f;
		radiusTerrainType=-1;
	}
	void stopOnInds(bool flag){
		flag_stopOnInds=flag;
	}
	void setSurKind4Working(int tt){
		for(int i=0; i<16; i++){
			surKind4Work[i] = ((tt&(1<<i)) != 0 )? true : false;
		}
	}
	void setScaleTerrainType(float newScale) { kScaleTerrainType=newScale; }
	void setRadiusTerrainType(int newradius) { radiusTerrainType=newradius; }
	...
	virtual const Se3f& getPosition() { return position; }
	...
	struct ResourceDescription {
		...
		float kScale;
		float vScale;
		ResourceDescription(eResourceDescriptionType _type, const string& _fname, float _kScale, float _vScale=1.f){
...
protected:
	...
	bool flag_changeTerrainType;
	TerrainType newTerrainType;
	bool surKind4Work[TERRAIN_TYPES_NUMBER];
    float kScaleTerrainType;
	int radiusTerrainType;

	//eDetaledTextueType detaledTextueType_;
	//bool flag_enableDrawDetailTexture;
	ExtraDrawType extraDraw;
	int layerDetailTexture;
	int layerZonePlacement;
	float detaledTextueScale_;
	bool flag_stopOnInds;
	bool drawRegionAndImpassability(int _x, int _y, int _r);
};

template <class cclass, class baseclass=TerToolBase>
class CTCreator : public baseclass{
public:
...
class TerToolTextureBase : public TerToolBase{
public:
	TerToolTextureBase();
	struct Vertex2i{
		int x,y;
	...
	virtual void getResourceDescription(list<ResourceDescription>& resdscrlist) const{
		switch(drawMetod){
		case DM_Cyrcle:		case DM_Bitmap:
			resdscrlist.push_back(ResourceDescription(ResourceDescription::RDT_Bitmap8Color4Surface, terTexture, terTextureKScale));
			break;
		case DM_ReliefBitmapPressIn:	case DM_ReliefBitmapSwellOut:
			resdscrlist.push_back(ResourceDescription(ResourceDescription::RDT_Bitmap8Voxel, terTexture, terTextureKScale, vScale)); //1.f
			break;
		}
	}
	virtual void serialize(Archive& ar);
...
class TerToolTexture  : public CTCreator<TerToolTexture, TerToolTextureBase> { //, public TerToolTextureBase  , public TerToolBase
public:
	TerToolTexture();
	template <eSetingAtrMetod setingIndsMetod> bool quant();
	virtual void serialize(Archive& ar);
...
class TerToolTextureTrack  : public CTCreator<TerToolTextureTrack, TerToolTextureBase> /*, public TerToolBase*/ {
public:
	TerToolTextureTrack();
	template <eSetingAtrMetod setingIndsMetod> bool quant();
	//virtual TerToolBase* clone()const{ return cloneI(*this); }
...
public:
	ShareHandle<TerToolBase> terTool;
