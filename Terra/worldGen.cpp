...
void m_net_init();
void rough_init();
void generate_roughness_map();
void map_init();
void generate_alt_map();
...
{
	unsigned char * lastlineF= new unsigned char[vMap.H_SIZE]; //Тип поверхности
	int i;
...
int vrtMap::buildMountainWorld()
{
	...
	ff.write(&VmpHeader,sizeof(VmpHeader));

	worldPreWrite(ff);
	int off = ff.tell();
	for(Stage = 0;Stage < PART_MAX;Stage++){
//		if(Verbose) cout <<endl<< "------------- Part: " << Stage << " -------------" <<endl;
		//Установка параметров генерируемого Stag-а(пока константа)
		...
		typedef vrtMap::sVmpHeader VmpHeader;
		register int i,j;
		unsigned short* pa = alt_map;
		...
		for(i = 0;i < (int)part_map_size_y;i++,pa += vMap.H_SIZE){
			...
			ff.seek(sizeof(vrtMap::sVmpHeader)+(Stage*(int)part_map_size_y*vMap.H_SIZE)+i*vMap.H_SIZE,XS_BEG);
			//Запись в VxGBuf
			ff.write(pw, vMap.H_SIZE*sizeof(unsigned char));
			for(j=0; j<vMap.H_SIZE; j++) {
				pw[j]=pa[j]&VX_FRACTION_MASK;
			}
			ff.seek(sizeof(VmpHeader)+(Stage*(int)part_map_size_y*vMap.H_SIZE)+i*vMap.H_SIZE+ vMap.H_SIZE*vMap.V_SIZE*2,XS_BEG); //Буфер атрибутов, где содержится дробная часть идет 3-м
