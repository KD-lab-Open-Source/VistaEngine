...
void sToolzerPMO::serialize(Archive& ar) 
{
	...
	ar.serialize(maxFH, "filterHeigh", 0);
...
void vrtMap::playPMOperation()
{
	procMapOp=0;
	flag_recordingPMO=false;
...
void sBitGenPMO::actOn()
{
	...
	case BGMID_EXP:
		vMap.drawBitMetod(x, y, rad, sBitGenMetodExp(expPower, maxHeight, rndVal), minFH, maxFH);
		break;
	case BGMID_PN:
		vMap.drawBitMetod(x, y, rad, sBitGenMetodPN(expPower, maxHeight, rndVal), minFH, maxFH);
		break;
	case BGMID_MPD:
		vMap.drawBitMetod(x, y, rad, sBitGenMetodMPD(expPower, maxHeight, rndVal, kRoughness), minFH, maxFH);
		break;
	}
}
...
{
	RoadTool roadTool;
	RoadTool::TextureSetting txset;
	txset.set(bitmapFileName, bitmapVolFileName, edgeBitmapFName, edgeVolBitmapFName, flag_onlyTextured, 0, 0);
	roadTool.init(nodePrimaryDataArr, texturingMetod, putMetod, txset);
	roadTool.buildRoad();
}
