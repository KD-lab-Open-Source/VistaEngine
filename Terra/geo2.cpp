...
bool sGeoWave::quant()
{
	...
	if(!flag_beginInitialization){
		...
		//if(maxRadius >= SGEOWAVE_MAX_RADIUS) maxRadius=SGEOWAVE_MAX_RADIUS-1; //�� ����������� (���� �������� ����)
...
void sGeoWave::serialize(Archive& ar)
{
	...
	ar.serialize(maxRadius, "maxRadius", "������");
	ar.serialize(begRadius, "begRadius", "��������� ������");
	ar.serialize(fadeRadius, "fadeRadius", "��������� ������ ���������");
	ar.serialize(maxWaveAmplitude, "maxWaveAmplitude", "������������ ���������");
	ar.serialize(waveLenght, "waveLenght", "������ �����");
	ar.serialize(waveSpeed, "waveSpeed", "�������� �����");
	ar.serialize(begPhase, "begPhase", "����(-90/0/+90)");
	ar.serialize(flag_Track, "flag_Track", "��������� ����");
}
...
//				if(inVx[cnt]>MAX_VX_HEIGHT)inVx[cnt]=MAX_VX_HEIGHT;
			...
			if(flag_occurrenceGeo && (V>>VX_FRACTION < maxVx[cnt]-DELTA_DEEP_GEO) ){
				if(Vm_IsDam(vMap.vxaBuf[offset])){
					vMap.vxaBuf[offset]=VmAt_Nrml_Geo; //Hint ������ ������������� ����
