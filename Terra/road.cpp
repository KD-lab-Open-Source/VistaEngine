...
void RoadTool::TextureSetting::serialize(Archive& ar)
{
	...
	ar.serialize(alphaRoad, "alphaRoad", 0);
	ar.serialize(alphaRoadSide, "alphaRoadSide", 0);
}
...
void RoadTool::serialize(Archive& ar)
{
	...
	ar.serialize(textureSetting, "textureSetting", 0);
	ar.serialize(roadDetailTex, "roadDetailTex", 0);
	ar.serialize(edgeDetailTex, "edgeDetailTex", 0);
}
...
void RoadTool::init(const vector<sNodePrimaryData>& primaryNodeArr, sRoadPMO::eTexturingMetod _texturingMetod, sRoadPMO::ePutMetod _putMetod, const TextureSetting& txSet)
{
...
void RoadTool::buildRoad()
{
	...
	if(vMap.isRecordingPMO()){
		sRoadPMO* pRoadPMO=new sRoadPMO(nodeArr, textureSetting.bitmapFileName, textureSetting.bitmapVolFileName, 
			textureSetting.edgeBitmapFName, textureSetting.edgeVolBitmapFName, textureSetting.flag_onlyTextured, texturingMetod, putMetod);
	...
	if(putMetod==sRoadPMO::PM_ByRoadHeight)
		for(p=nodeArr.begin(); p!=nodeArr.end(); p++){
			...
			putSpline(p, 1); //1-pass
		}
	for(p=nodeArr.begin(); p!=nodeArr.end(); p++){
		if(putMetod!=sRoadPMO::PM_ByRoadHeight)
			getRoadLenghtAndRecalcNodeLengt();
		putSpline(p,0); //1-pass
	}
	for(p=nodeArr.begin(); p!=nodeArr.end(); p++)
		putSpline(p,2); //2-pass
}
...
sRect RoadTool::recalcSpline(TypeNodeIterator ni)
{
	...
	if(lenght){
		...
		for(i=0; i<=lenght; i++){
			...
			if(normal.norm2() > FLT_EPS){
				normal.normalize(1.f);
				ni->spline2NextLeft.push_back(Vect3f(Vect2f(pnt) - normal*curWidth, pnt.z));
				retval.addBound(Vect2i(ni->spline2NextLeft.back()));
				ni->spline2NextRight.push_back(Vect3f(Vect2f(pnt) + normal*curWidth, pnt.z));
...
void alphaBlending(unsigned short alpha, unsigned long& src, unsigned long& dst)
{
	//short alpha=src>>24;
...
void RoadTool::putStrip(vector<sPolygon>& poligonArr, vector<VertexI> iPntArr, const BitmapDispatcher::Bitmap* texture, const BitmapDispatcher::Bitmap* vtexture, int numPass, int _alpha, int detTex)
{
	xassert(vMap.supBuf);
	...
	int iminX=0;
	int iminY=0;
	int imaxX=vMap.H_SIZE-1;
	int imaxY=vMap.V_SIZE-1;
	...
	for(p=poligonArr.begin(); p!=poligonArr.end(); p++){
		...
		if(a->y > c->y) swap(a, c);
		if(b->y > c->y) swap(b, c);
		//mp[0].set(ceilFIntF0(a->x), ceilFIntF0(a->y));
		//mp[1].set(ceilFIntF0(b->x), ceilFIntF0(b->y));
		//mp[2].set(ceilFIntF0(c->x), ceilFIntF0(c->y));
...
//loc_scip01:;
		...
		mrLine.resize(2);
		for (current_sy = drawBegY; current_sy <= drawMaxY; current_sy++) {
			...
			if(detTex!=0){
				MultiRegion::IntegerType w=current_sx;
				MultiRegion::Line::iterator  i = mrLine.begin();
				i->x0 = w; i->type = 1 + clamp(detTex-1, 0, cTileMap::miniDetailTexturesNumber-1);
				i++;
				i->x0 = w + length + 1; i->type = 0;
				MultiRegion::Line& destLine = vMap.region().lines()[current_sy];
				vMap.region().addLine(destLine, mrLine, 0);
			}
					...
					if( (current_sx<=imaxX) && (current_sx >= 0)) {
						...
						if(textureSetting.flag_onlyTextured){
							...
							if(pBitmap){
								...
								vMap.SetAtr(bufoff, VmAt_Nrml);
								int alpha=_alpha;
								if(flag_AlphaChanelPresent)
									alpha = (alpha * (pBitmap[offBitmap]>>24) + 255) >>8;
								alphaBlending(alpha, pBitmap[offBitmap], vMap.supBuf[bufoff]);
							}
							if(pBitmapV){
...
//loc_scip02:;
		//vMap.region().operate(shr);
	}
	vMap.region().unlock();
}

void RoadTool::putSpline(TypeNodeIterator ni, int numPass)
{
	//Bitmap !!!
	const BitmapDispatcher::Bitmap* texture=bitmapDispatcher.getBitmap(textureSetting.bitmapFileName.c_str());
	const BitmapDispatcher::Bitmap* vtexture=bitmapDispatcher.getBitmap(textureSetting.bitmapVolFileName.c_str());
	if(texture && vtexture && texture->size!=vtexture->size){
		xassert(0&&"Bitmap and bitmapV is not equal !");
	}
	const BitmapDispatcher::Bitmap* textureEdge=bitmapDispatcher.getBitmap(textureSetting.edgeBitmapFName.c_str());
	const BitmapDispatcher::Bitmap* vtextureEdge=bitmapDispatcher.getBitmap(textureSetting.edgeVolBitmapFName.c_str());
	...
	if(numPass==0)
		putStrip(poligonArr, iPntArr, texture, vtexture, numPass, textureSetting.alphaRoad, roadDetailTex );
	else 
		putStrip(poligonArr, iPntArr, 0, 0, numPass, textureSetting.alphaRoad, 0);
	...
	if(numPass==0)
		putStrip(poligonArr, iPntArr, textureEdge, vtextureEdge, 0, textureSetting.alphaRoadSide, edgeDetailTex );
	else 
		putStrip(poligonArr, iPntArr, 0, 0, numPass, textureSetting.alphaRoadSide, 0);


	if(numPass==0)
		putStrip(poligonArr, iPntArr2, textureEdge, vtextureEdge, 0, textureSetting.alphaRoadSide, edgeDetailTex );
	else 
		putStrip(poligonArr, iPntArr2, 0, 0, numPass, textureSetting.alphaRoadSide, 0);
