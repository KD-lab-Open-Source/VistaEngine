...
void geoGeneration(sGeoPMO& var)///(int _x, int _y, int sx, int sy, int MAX_GG_ALT, int kPowCellSize, int kPowShiftCS4RG, int GeonetMESH, int NOISELEVEL_GG, int BorderForm, bool _inverse)
{
	...
	vMap.addProcMapOpStatistic(sx, sy);

	Cmax_alt=MAX_GG_ALT;///!!!!

	//vMap.FilterMinHeight=0; vMap.FilterMaxHeight=MAX_VX_HEIGHT;///!!!
	vMap.setFilterHeight(0, MAX_VX_HEIGHT);
...
///////////////////////////////////////////////////////////////////////////////////
	...
	if(vMap.isRecordingPMO()) 
		vMap.UndoDispatcher_PutPreChangedArea(sRect(xBeg, yBeg, sx, sy),1,0, new sGeoPMO(var));
	...
	vMap.regRender(xBeg, yBeg, xBeg+sizeX, yBeg+sizeY, vrtMap::TypeCh_Height);
