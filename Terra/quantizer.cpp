...
bool ColorQuantizer::prepare4PutColor(int _nMaxColors)
{
	...
	pHead= new sOctreeNode(1, 0, nLeafs);	// head of list with reducible non-leaf nodes (dummy)
	pHead->nLevel = 0;
	pRoot= new sOctreeNode(7, 0, nLeafs);	// root of octree
...
int ColorQuantizer::quantizeOctree(unsigned long* pSource, unsigned char* pDest, Vect2s bitmapSize, int _nMaxColors)
{
	...
	//sOctreeNode head(1, 0, nLeafs);	// head of list with reducible non-leaf nodes (dummy)
	//head.nLevel = 0;
	//sOctreeNode root(7, 0, nLeafs);	// root of octree
