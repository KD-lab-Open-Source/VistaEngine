...
struct sToolzerPMO : public sBasePMOperation {
	...
	sToolzerPMO(int _x,int _y,int _rad,int _smth,int _dh,int _smode,int _eql, int _filterHeigh, int _rndVal=0){
...
struct sBitGenPMO : public sBasePMOperation {
	...
	sBitGenPMO() { pmoID=PMO_ID_BITGEN; }
	template<class GenerationMetod>
	sBitGenPMO(int _x, int _y, int _rad, GenerationMetod& genMetod, int _filterHeigh){
		pmoID=PMO_ID_BITGEN;
		x=_x; y=_y;
		rad=_rad;
	...
	sBitGenPMO(int _x, int _y, int _rad, sBitGenMetodMPD& genMetod, int _filterHeigh){
