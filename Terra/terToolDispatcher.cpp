...
TerToolsDispatcher::TerToolsID TerToolsDispatcher::unengagedTerToolID=1;
#ifdef TERTOOL_LOG
XStream terToolLogStream(".\\tertoollog.txt", XS_OUT);
#endif
///////////////////////////////////////

TerToolsDispatcher::TerToolsID TerToolsDispatcher::startTerTool(TerToolBase* pTTB)
{
	xassert(pTTB);
...
void TerToolsDispatcher::stopTerTool(TerToolsDispatcher::TerToolsID ttid)
{
...
bool TerToolsDispatcher::isTerToolFinished(TerToolsDispatcher::TerToolsID ttid)
{
	StartedTerTools::iterator p=find(ttid);
...
void createDirIfNotExist(const char* dirName)
{
	...
	if (hFind == INVALID_HANDLE_VALUE) {
		bool createDirResult=CreateDirectory(dirName, NULL);
		xxassert(createDirResult, dirName);
	}
	else {
		xxassert((FindFileData.dwFileAttributes&FILE_ATTRIBUTE_DIRECTORY)!=0, dirName);
		FindClose(hFind);
	}
}
...
string getCachDirAndCreateIfNotExist()
{
	...
	createDirIfNotExist(generalCacheFilePath);
	createDirIfNotExist(cacheFilePath);

	string str=cacheFilePath; str+='\\'; str+=vMap.worldName; 
	createDirIfNotExist(str.c_str());
...
void TerToolsDispatcher::prepare4World()
{
	...
	int i,j;
	for(i=0; i<256; i++){
		sColor4c rowColor=vMap.DamPal[i];
		for(j=0; j<256; j++){
			sColor4c colColor=vMap.DamPal[j];
			sColor4c colA20,colA40,colA60,colA80;
	...
	for(i=0; i<256; i++){
		sColor4c rowColor=vMap.GeoPal[i];
		for(j=0; j<256; j++){
			sColor4c colColor=vMap.GeoPal[j];
			sColor4c colA20,colA40,colA60,colA80;
			colA20.interpolate(rowColor, colColor, 0.20f);
			colA40.interpolate(rowColor, colColor, 0.40f);
			colA60.interpolate(rowColor, colColor, 0.60f);
...
bool TerToolsDispatcher::prepareTerTools4World(const TerToolBase* pTerTool, int indexIn, ColorQuantizer& cqGeo, ColorQuantizer& cqDam, const char* cachDir)
{
	...
	for(p=rdList.begin(); p!=rdList.end(); p++){
		...
		if(!fname.empty()){
			//Bitmap8CTable::iterator p=bitmap8CTable.find(fname);
			//if(p==bitmap8CTable.end()){
			//	bitmap8CTable[fname]=Bitmap();
			//	return bitmap8CTable[fname].prepare4World(fname.c_str(), indexIn, cqGeo, cqDam);
			//}
			...
			if(p->resourceDescriptionType==TerToolBase::ResourceDescription::RDT_Bitmap8Color4Surface){
				...
				for( p=r.first; p!=r.second; ++p){
					if(floatEqual(p->second.kScale,terTextureKScale))
						break;
				}
				if(p==r.second){
					...
					m->second.loadAndprepare4World(fname.c_str(), terTextureKScale, indexIn, cqGeo, cqDam, pCDB, cachDir);
					#ifdef TERTOOL_LOG
				...
				if(p==r.second){
					...
					m->second.load(fname.c_str(), terTextureKScale);
					#ifdef TERTOOL_LOG
...
//	const string& fname=pTerTool->getTexture();
//	const float terTextureKScale=pTerTool->getTextureKScale();
//	if(!fname.empty()){
//		//Bitmap8CTable::iterator p=bitmap8CTable.find(fname);
//		//if(p==bitmap8CTable.end()){
//		//	bitmap8CTable[fname]=Bitmap();
...
//		}
//		Bitmap8CTable::iterator m=bitmap8CTable.insert(make_pair(fname, Bitmap8C()));
//		m->second.prepare4World(fname.c_str(), terTextureKScale, indexIn, cqGeo, cqDam);
//#ifdef TERTOOL_LOG
//		terToolLogStream < "texturePrepared:" < fname.c_str() < " fScale=" <= terTextureKScale < "\n";
//#endif
//	}
	return true;
}
...
//		return false;
	...
	if(pCDB->flag_alfaPresent)
		sectionsize=allsize/3;
	else 
		sectionsize=allsize/2;
	if(sectionsize!=pCDB->sectionSize)
		return false;

	bool flag_error=false;
	unsigned int ccrc;
	pBitmap4Dam=new unsigned char[sectionsize];
	pBitmap4Geo=new unsigned char[sectionsize];
	fb.read(pBitmap4Dam, sectionsize);
	ccrc=crc32(pBitmap4Dam, sectionsize, startCRC32);
...
void TerToolsDispatcher::Bitmap8C::saveDataCash(const char* fname, CashDateBitmap8C* pCDB, const char* cachDir)
{
	...
	unsigned int crcdam=crc32(pBitmap4Dam, sectionsize, startCRC32);
...
bool TerToolsDispatcher::Bitmap8C::loadAndprepare4World(const char* fname, const float terTextureKScale, int indexIn, ColorQuantizer& cqGeo, ColorQuantizer& cqDam, CashDateBitmap8C* pCDB, const char* cachDir)
{
	index = indexIn;
	TGAHEAD tgahead;
	...
	if( (tgahead.PixelDepth!=24 && tgahead.PixelDepth!=32) || tgahead.ImageType!=2 ) {
		//AfxMessageBox("Не поддерживаемый тип TGA (необходим 24bit не компрессированный, с размерами, кратными степени 2)");
...
//			log_var_crc(pAlfaLayer, size.x*size.y*sizeof(pAlfaLayer[0]));
	...
	pBitmap4Dam=new unsigned char[size.x*size.y];
	...
	cqDam.ditherFloydSteinberg(ptb, pBitmap4Dam, size);
...
bool TerToolsDispatcher::Bitmap8V::load(const char* fname, const float terTextureKScale)
{
	TGAHEAD tgahead;
	if(!tgahead.loadHeader(fname)){
		string str="Error loading terTool texture-";
	...
	tgahead.load2buf(pBitmapV, originalSize.x*originalSize.y*sizeof(unsigned char));
