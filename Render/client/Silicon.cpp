...
float silicon_time_step = 0.1f;

float elastic_sphere_stiffness = .8f;
float elastic_sphere_damping = -0.04f;
float elastic_sphere_damping2 = 0.00025f;
float elastic_sphere_spawn_factor = 0.2f;
float elastic_sphere_spawn_factor_dz = 0.2f; 
float elastic_sphere_velocity_factor = 0.015f;

float elastic_sphere_CM_stiffness = 0.05f;
float elastic_sphere_CM_damping = -0.025f;
float elastic_sphere_CM_damping2 = .0001f;

float elastic_sphere_get_point_radius_factor = 0.85f;

float elastic_link_CM_stiffness = 0.03f;
float elastic_link_CM_damping = -0.05f;
float elastic_link_CM_damping2 = 0.0001f;

float elastic_link_stiffness = 0.1f;
float elastic_link_damping = -0.05f;
float elastic_link_damping2 = 0.001f;

float elastic_link_thickness_pow = 4;

float oscillating_link_density = 0.1f;

float oscillating_link_stiffness = 2;//.8;
float oscillating_link_damping = -0.04f;
float oscillating_link_damping2 = 0.000025f;
float oscillating_link_spawn_factor = 0.2f;
float oscillating_link_spawn_factor_dz = 0.2f; 
float oscillating_link_velocity_factor = 0.015f;

float oscillating_link_tail_length = 40;
...
void ElasticSphere::Draw(cCamera *DrawNode)
{
	...
	sVertexXYZDT2 *VertexFix=NULL;//Больше нет такого буфера.(sVertexXYZDT2*)rd->GetStripBuffer();
	int is=theta_size,js=psi_size;
	int VertexBufferSize=((is+1)*(js+1)+(is+2)*(js-1)-1)*sizeof(VertexFix[0]);
//	VISASSERT(VertexBufferSize<rd->GetStripBufferLength());

	Vect3f uv[2];
	Mat3f &mC=DrawNode->GetMatrix().rot();
	uv[0].set(	0.5f*mC[0][0],0.5f*mC[0][1],0.5f*mC[0][2]);
	uv[1].set(	0.5f*mC[1][0],0.5f*mC[1][1],0.5f*mC[1][2]);
	VISASSERT(GetTexture(0));
	rd->SetNoMaterial(blendMode,MatXf::ID,GetFrame()->GetPhase(),GetTexture(0),GetTexture(1),COLOR_MOD);//FIXME
	rd->SetVertexDeclaration(VertexFix->declaration);

	int cull=rd->GetRenderState(D3DRS_CULLMODE);
	rd->SetRenderState(D3DRS_CULLMODE,D3DCULL_CW);

	sColor4c Diffuse(GetDiffuse());
	float dv=0.5f-GetFrame()->GetPhase();
	int i, j;
	for(i=0;i<=is;i++)
	{
		sVertexXYZDT2 &v=VertexFix[2*i];
		const Vect3f &n=normal(0,i);
		v.pos=point(0,i);
		v.GetTexel().set(0/(float)psi_size,i/(float)theta_size);
	...
	for(j=0;j<js;j++)
		if(j&1)
			for(i=0;i<=is;i++)
			{
				...
				const Vect3f &n=normal(j+1,is-i);
				v.pos=point(j+1,is-i);
				v.GetTexel().set((j+1)/(float)psi_size,(is-i)/(float)theta_size);
				v.GetTexel2().set(n.y*0.5f+0.5f,n.z*0.5f+dv);
				v.diffuse=Diffuse;
			}
		else
			...
			{
				sVertexXYZDT2 &v=VertexFix[(2*is+3)*j+2*i+1];
				const Vect3f &n=normal(j+1,i);
				v.pos=point(j+1,i);
				v.GetTexel().set((j+1)/(float)psi_size,i/(float)theta_size);
				v.GetTexel2().set(n.y*0.5f+0.5f,n.z*0.5f+dv);
				v.diffuse=Diffuse;
			}
	for(j=0;j<js-1;j++)
	...
	rd->DrawPrimitiveUP(D3DPT_TRIANGLESTRIP,(is+1)*(js+1)+(is+2)*(js-1)-1-2,VertexFix,sizeof(VertexFix[0]));
	*rd->PtrNumberPolygon+=(is+1)*(js+1)+(is+2)*(js-1)-1-2;
	rd->NumDrawObject++;
...
void ElasticLink::evolve()
{
	...
	GlobalMatrix.trans() = 0.5f*(point1 + point2);
	z_axis = point2 - point1;
...
void ElasticLink::Draw(cCamera *DrawNode)
{
	...
	gb_RenderDevice->SetNoMaterial(blendMode,MatXf::ID,GetFrame()->GetPhase(),GetTexture(),GetTexture2(),COLOR_MOD);//FIXME
