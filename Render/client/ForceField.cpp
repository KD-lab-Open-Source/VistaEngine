...
float fade_duration = 1000;
int evolve_field_iterations = 3;
float force_field_height = 300;
int height_field_iterations = 10;
															   								   
float force_field_time_step = 0.2f;
float force_field_stiffness = 3;
float force_field_damping = 1.0005f;
float force_field_damping2 = 0.0001f;
float force_field_damping_min = 0.1f;
float force_field_spawn_factor = 0.25f;

float force_field_penalty_stiffness = 0.;

float force_field_check_place_height = 50;

int force_field_tangeta_length = 20;
...
void FieldDispatcher::Cell::specify(const Vect2i& delta, const Vect2i& tangenta, const FieldCluster* cluster_)
{
	...
	if(specify_error > d){
	   specify_error = d;
	   specify_delta = delta;
	   specify_normal.set(float(tangenta.y), float(-tangenta.x), 0);
...
void FieldDispatcher::Draw(cCamera *DrawNode)
{
	...
	float aaaa = gb_VisGeneric->GetLogicTimePeriodInv();
	interpolation_factor+=/*IParent->GetDTime()*/ float(dt*gb_VisGeneric->GetLogicTimePeriodInv());
...
Vect2f FieldDispatcher::findPlace(const Vect2f& pos0, const Vect2f& delta)
{
	...
	for(i = 1; ;i++)
	{
		if(checkPlace(pos + dx*float(i), delta))
		{
			dd = dx;
			break;
		}
		...
		if(checkPlace(pos + dy*float(i), delta))
		{
			dd = dy;
			break;
		}
...
{
	Vect3f points[3] = { Vect3f(float(m2w(x)), float(m2w(y + 1)), height_initial(x, y + 1)), 
		Vect3f(float(m2w(x + 1)), float(m2w(y)), height_initial(x + 1, y)), 
		Vect3f(float(m2w(x)), float(m2w(y)), height_initial(x, y)) };
	float t = min(ray_triangle_intersection(points, origin, direction), t_zero);
	points[2] = Vect3f(float(m2w(x + 1)), float(m2w(y + 1)), height_initial(x + 1, y + 1));
...
int FieldDispatcher::calcPenalty(const Vect3f& center, float radius, float feedback_factor, Vect3f& r, Vect3f&f)
{
	...
	if(fabs(d_best) < radius){
		...
		r.set(float(m2w(x_best)), float(m2w(y_best)), height(x_best, y_best));
		return 1;
		}
	return 0;
}
...
void FieldCluster::Animate(float dt)
{
	...
	float phase = clamp(fade_timer/fade_duration, 0.0f, 1.0f);
...
void FieldCluster::logicQuant()
{
	...
	{
		float fade_logic_duration=fade_duration*1e-2f;
