	...
	case SD_SHADER:
		{
			...
			SHADER shader;
			bool passed=true;
			const char* version_to_debug=NULL;
			while(CLoadData* ld=dir.next())
			switch(ld->id)
			{
			...
			case SD_SHADER_VERSION:
				{
					...
					LPCSTR max_version=NULL;
					xassert(version[0]);
					if(version[0]=='p')
					{
						max_version=D3DXGetPixelShaderProfile(gb_RenderDevice3D->lpD3DDevice);
					}else
					{
						xassert(version[0]=='v');
						max_version=D3DXGetVertexShaderProfile(gb_RenderDevice3D->lpD3DDevice);
					}
			...
			case SD_SHADER_COMPILED:
				if(passed)
				{
					xassert(shader.uncompiled_shader_data==NULL);
...
void cShaderStorageInternal::SHADER::CompilePS()
{
	...
	RDCALL(gb_RenderDevice3D->lpD3DDevice->CreatePixelShader(uncompiled_shader_data, &pixel_shader));
...
{
	xassert(!is_pixel_shader);
	RDCALL(gb_RenderDevice3D->lpD3DDevice->CreateVertexShader(uncompiled_shader_data, &vertex_shader));
	uncompiled_shader_data=NULL;
}
