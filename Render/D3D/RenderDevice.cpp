...
cInterfaceRenderDevice::~cInterfaceRenderDevice()
{ 
	xassert(RenderMode==0);
	xassert(xScr==0&&yScr==0&&xScrMin==0&&yScrMin==0&&xScrMax==0&&yScrMax==0);
	xassert(xScr==0&&yScr==0);
	xassert(CurrentFont==0 || CurrentFont==DefaultFont);
	gb_RenderDevice=0;
}
...
void BuildMipMap(int x,int y,int bpp,int bplSrc,void *pSrc,int bplDst,void *pDst,int Attr)
{
		...
		for(int j=0;j<y;j++,Dst+=ofsDst,Src+=ofsSrc+bplSrc)
			for(int i=0;i<x;i++,Dst+=bpp,Src+=2*bpp)
			{
				...
				{ // bilinear/trilinear mipmap
					Color4c p00=*((Color4c*)&Src[0]),p01=*((Color4c*)&Src[bpp]),
						p10=*((Color4c*)&Src[bplSrc]),p11=*((Color4c*)&Src[bplSrc+bpp]);
