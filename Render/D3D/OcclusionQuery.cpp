...
bool cOcclusionQuery::Init()
{
	...
	if(Option_EnableOcclusionQuery)
		hr=gb_RenderDevice3D->D3DDevice_->CreateQuery(D3DQUERYTYPE_OCCLUSION,&pQuery);
	else
		hr=E_FAIL;
	return SUCCEEDED(hr);
}
...
void cOcclusionQuery::Test(const Vect3f* point, int numPoints)
{
	...
	if(true)
	{
		gb_RenderDevice3D->SetNoMaterial(ALPHA_BLEND,MatXf::ID,0,0);
	...
	Camera* camera=gb_RenderDevice3D->camera();
	int num_visible=0;

	sVertexXYZ* v=(sVertexXYZ*)buf.Lock(numPoints);
	for(int i = 0; i < numPoints; ++i)
	{
		if(camera->TestVisible(point[i],0))
		{
			num_visible++;
			(v++)->pos = point[i];
		}
