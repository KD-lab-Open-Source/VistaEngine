#include "StdAfxRD.h"
#include "VertexDeclarationMacro.h"
#include "D3DRender.h"
...
BEGIN_VERTEX_DECLARATION(sVertexXYZWDT4)
{ 0, 0,  D3DDECLTYPE_FLOAT4, D3DDECLMETHOD_DEFAULT, D3DDECLUSAGE_POSITIONT, 0 },
{ 0, 16, D3DDECLTYPE_D3DCOLOR, D3DDECLMETHOD_DEFAULT, D3DDECLUSAGE_COLOR, 0 },
{ 0, 20, D3DDECLTYPE_FLOAT2, D3DDECLMETHOD_DEFAULT,      D3DDECLUSAGE_TEXCOORD, 0 },
{ 0, 28, D3DDECLTYPE_FLOAT2, D3DDECLMETHOD_DEFAULT,      D3DDECLUSAGE_TEXCOORD, 1 },
...
END_VERTEX_DECLARATION(sVertexXYZWDT4)
...
//Фигня это всё, статические переменные, и томленье духа. 
//То что они непонятно когда инициализируются и удаляются - сильно портит картину.
/*
...
{
	return declaration[num_weight?1:0][bump?1:0][uv2?1:0][fur?1:0];
}

cSkinVertex::cSkinVertex(int num_weight_,bool bump_,bool uv2_,bool fur_)
:p(0),num_weight(num_weight_),vb_size(0),bump(bump_),uv2(uv2_),fur(fur_)
{
	...
	offset_fur=9+(num_weight?1:0)+(bump?6:0)+(uv2?2:0);
}

IDirect3DVertexDeclaration9* cSkinVertex::declaration[2][2][2][2];

void cSkinVertex::Register()
{
	vector<D3DVERTEXELEMENT9> elements;

	for(int num_weight=0;num_weight<=1;num_weight++)
	for(int bump=0;bump<=1;bump++)
	for(int uv2=0;uv2<=1;uv2++)
	for(int fur=0;fur<=1;fur++)
	{
		elements.clear();

		int cur_offset=0;
#define ADD(size_in_float,type,usage,usageindex) {D3DVERTEXELEMENT9 e={ 0, cur_offset,  type, D3DDECLMETHOD_DEFAULT, usage, usageindex }; elements.push_back(e);cur_offset+=size_in_float*4; }
		ADD(3,D3DDECLTYPE_FLOAT3,D3DDECLUSAGE_POSITION,0);
		ADD(1,D3DDECLTYPE_D3DCOLOR,D3DDECLUSAGE_BLENDINDICES,0);
		ADD(3,D3DDECLTYPE_FLOAT3,D3DDECLUSAGE_NORMAL,0);
		if(num_weight)
			ADD(1,D3DDECLTYPE_D3DCOLOR,D3DDECLUSAGE_COLOR,0);
		ADD(2,D3DDECLTYPE_FLOAT2,D3DDECLUSAGE_TEXCOORD,0);
		if(bump)
		{
			ADD(3,D3DDECLTYPE_FLOAT3,D3DDECLUSAGE_BINORMAL,0);
			ADD(3,D3DDECLTYPE_FLOAT3,D3DDECLUSAGE_TANGENT,0);
		}

		if(uv2)
			ADD(2,D3DDECLTYPE_FLOAT2,D3DDECLUSAGE_TEXCOORD,1);
		if(fur)
			ADD(1,D3DDECLTYPE_D3DCOLOR,D3DDECLUSAGE_COLOR,1);
