...
{
	xassert(page >= 0 && page < total_pages && free_pages < total_pages);
...
void VertexPool::Create(const PoolParameter* p)
{
	...
	RDCALL(gb_RenderDevice3D->D3DDevice_->CreateVertexBuffer(
...
{
	RDCALL(gb_RenderDevice3D->D3DDevice_->SetStreamSource(0, vb, 0, vertex_size));
}
...
{
	xassert(page >= 0 && page < total_pages);
	void *ptr;
	RDCALL(vb->Lock(
