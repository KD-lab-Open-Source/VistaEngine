...
{
protected:
	vector<BaseGraphObject*> slot;
public:
	typedef vector<BaseGraphObject*>::iterator iterator;
	
	sGrid2d() { }
	~sGrid2d() { }

	inline int size()	{ return slot.size(); }
	BaseGraphObject* operator[](int i){return slot[i];}
	iterator begin(){return slot.begin();}
	iterator end(){return slot.end();}

	void Attach(BaseGraphObject *UnkObj);
