...
void MultiRegion::addLine(Line& destLine, const Line& srcLine, IntegerType offset)
{
	...
	xassert(!tempLine.empty());
	xassert(tempLine.begin()->x0==0);
	destLine = tempLine;
}
...
void MultiRegion::serialize(Archive& ar)
{
	...
	if(ar.isOutput()){
		...
		save(buffer);
		ar.serialize(MemoryBlock(buffer), "data", 0);
	}
	else{
		MemoryBlock memoryBlock;
		if(ar.serialize(memoryBlock, "data", 0) && memoryBlock.size()){
			XBuffer buffer(memoryBlock.buffer(), memoryBlock.size());
			load(buffer);
		}
	}
}
