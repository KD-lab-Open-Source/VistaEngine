...
public:
    LensFlareSprite(float _position = 0.0f, float _radius = 0.1f, const char* _filename = "", const sColor4f& _color = sColor4f(1.0f, 1.0f, 1.0f, 1.0f))
    : position_(_position)
    , radius_(_radius)
	, filename_(_filename)
    , texture_(0)
...
class LensFlareRenderer : public cBaseGraphObject{
public:
    LensFlareRenderer();
...
private:
	...
	void drawSprite(cCamera* camera, const Vect3f& point, const LensFlareSprite& sprite, float alpha);
