...
void GrassUpdateMapFunction(const Vect2i& pos1, const Vect2i& pos2,void* data)
{
	((GrassMap*)data)->UpdateMap(pos1,pos2);
}
...
GrassMap::GrassMap():cBaseGraphObject(0)
{
	grassMap_ = NULL;
	terMapPoint = NULL;
	textureMap_ = NULL;
...
GrassMap::~GrassMap()
{
	terMapPoint->UnRegisterUpdateMap(GrassUpdateMapFunction,this);
	RELEASE(terMapPoint);
	delete [] grassMap_;
	RELEASE(texture_);
	RELEASE(textureMap_);
...
void GrassMap::Init(cScene* pScene)
{
	terMapPoint = pScene->GetTileMap();
	terMapPoint->AddRef();
	terMapPoint->RegisterUpdateMap(GrassUpdateMapFunction,this);
...
void GrassMap::SaveMap()
{
	if (!textureMap_)
		return;
	int bpp=textureMap_->GetBitsPerPixel();
	xassert(bpp==32);
...
void GrassMap::BuildGrass(Vect2i min, Vect2i size)
{
	...
	int pitch;
	sColor4c* data = (sColor4c*)textureMap_->LockTexture(pitch,min,size);
	pitch /= 4;
	vector<int> storedpos;
	for (int y=0; y<size.y; y++)
	{
		sColor4c* p = data+y*pitch;
		for (int x=0; x<size.x; x++,p++)
		{
			...
			for (int i=0; i<count; i++)
			{
				...
				bush.pos.x = clamp(bush.pos.x,0,terMapPoint->GetTerra()->SizeX()-1);
				bush.pos.y = yy/8*tileSize+yy%8*8+n/8;//+i*step+graphRnd.frnd()*4;
				bush.pos.y = clamp(bush.pos.y,0,terMapPoint->GetTerra()->SizeY()-1);
				bush.pos.z = terMapPoint->GetTerra()->GetZ(bush.pos.x,bush.pos.y);
				...
				Vect3f normal;
				GetNormal(terMapPoint->GetTerra(),bush.pos.x,bush.pos.y,1,normal);
...
void GrassMap::SetGrass(float x, float y, float radius, int texture, int count, bool clear, int intensity,int intensity2)
{
	...
	int pitch;
	sColor4c* data = (sColor4c*)textureMap_->LockTexture(pitch,min,lock_size);
	pitch/=4;
...
void GrassMap::SetAllGrass(int texture, int intensity,int intensity2)
{
	int pitch;
	int pix = (texture+1)<<textureCountShift;
	sColor4c* data = (sColor4c*)textureMap_->LockTexture(pitch,Vect2i(0,0),grassMapSize_);
	pitch/=4;
	unsigned char intens = round(255.f/100*intensity);
	unsigned char intens2 = round(255.f/100*intensity2);
...
void GrassMap::UpdateMap(const Vect2i& pos1, const Vect2i& pos2)
{
	...
	for(int x=xmin; x<=xmax; x++)
	{
		...
		FOR_EACH(tile.bushes_,it)
		{
			it->pos.z = terMapPoint->GetTerra()->GetZ(it->pos.x,it->pos.y);
		}
		tile.initVertex = false;
	}
}
...
void GrassMap::PreDraw(cCamera *pCamera)
{
	start_timer_auto();
	if (!enable_)
		return;
	...
	for(int x=0; x<tileNumber_.x; x++)
	{
		...
		SortedTile stile;
		stile.tile = &tile;
		stile.distance = pCamera->GetPos().distance2(Vect3f(tile.pos.x,tile.pos.y,0));
		sortedTile_.push_back(stile);
	}
	pCamera->SetGrass(this);
}
...
void GrassMap::CalcVertex(GrassTile& tile)
{
	...
	for(int i=0; i<drawTile->bladeCount; i++)
	{
		...
		color.RGBA() = vMap.getColor32(bush.pos.x,bush.pos.y)|0xFF000000;
...
bool GrassMap::TestVisible(GrassTile& tile,cCamera* pCamera)
{
	if (tile.bushes_.size()==0)
		return false;
	if (!pCamera->TestVisible(tile.pos.x,tile.pos.y))
		return false;
	float dist=pCamera->GetPos().distance2(Vect3f(tile.pos.x,tile.pos.y,0));
...
void GrassMap::DrawGrass(eBlendMode mode,cCamera *UCamera)
{
	if (sortedTile_.size()==0)
		return;
	sColor4c color(255,255,255,64);
	
	bool is_shadow=UCamera->IsShadow()&&enbaleShadow_;
	...
	bool toZbuffer = UCamera->GetAttr(ATTRCAMERA_ZBUFFER);
	bool toFloatZBuffer = UCamera->GetAttr(ATTRCAMERA_FLOAT_ZBUFFER);
	vsGrass->Select(time_,invHideDistance2_,IParent->GetTileMap()->GetDiffuse(),toZbuffer||toFloatZBuffer);
	if (toZbuffer||toFloatZBuffer)
	{
		...
		psSkinZBuffer->SetSecondOpacity(NULL);
		psSkinZBuffer->Select();
		//gb_RenderDevice3D->dtAdvance->pPSZBuffer->Select(toFloatZBuffer);
	}else
	if (is_shadow)
	{
		psGrassShadow->SetShadowIntensity(GetScene()->GetShadowIntensity());
	...
	{
		GrassTile* tile = it->tile;
		Vect2f vec2(tile->pos.x+tileSize/2 - UCamera->GetPos().x,tile->pos.y+tileSize/2 - UCamera->GetPos().y);
...
void GrassMap::Draw(cCamera *UCamera)
{
	start_timer_auto();
	if (!enable_)
		return;
	...
	if (UCamera->GetAttr(ATTRCAMERA_ZBUFFER)||UCamera->GetAttr(ATTRCAMERA_FLOAT_ZBUFFER))
	{
...
void GrassMap::serialize(Archive& ar)
{
	...
	ar.serialize(ResourceSelector(textureNames_[0], ResourceSelector::TEXTURE_OPTIONS),"textureName0","Текстура травы - 1");
	ar.serialize(ResourceSelector(textureNames_[1], ResourceSelector::TEXTURE_OPTIONS),"textureName1","Текстура травы - 2");
	ar.serialize(ResourceSelector(textureNames_[2], ResourceSelector::TEXTURE_OPTIONS),"textureName2","Текстура травы - 3");
	ar.serialize(ResourceSelector(textureNames_[3], ResourceSelector::TEXTURE_OPTIONS),"textureName3","Текстура травы - 4");
	ar.serialize(ResourceSelector(textureNames_[4], ResourceSelector::TEXTURE_OPTIONS),"textureName4","Текстура травы - 5");
	ar.serialize(ResourceSelector(textureNames_[5], ResourceSelector::TEXTURE_OPTIONS),"textureName5","Текстура травы - 6");
	ar.serialize(ResourceSelector(textureNames_[6], ResourceSelector::TEXTURE_OPTIONS),"textureName6","Текстура травы - 7");
	ar.closeBlock();
	hideDistance2_ = hideDistance_*hideDistance_;
	invHideDistance2_ = 1/hideDistance2_;
	if(ar.isInput())
...
int GrassMap::drawTileAlloc(int bladeCount)
{
	...
	if (i == drawTiles.size()) drawTiles.push_back(NULL);
