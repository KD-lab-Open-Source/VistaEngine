...
void cLine3d::Draw(cCamera *DrawNode)
{
	...
	if(DrawNode->GetCameraPass()==SCENENODE_OBJECTSORT_NOZ)
		gb_RenderDevice->SetNoMaterial(GetAttribute(ATTRUNKOBJ_ADDBLEND)?ALPHA_ADDBLEND:ALPHA_BLEND,MatXf::ID,0,GetTexture(1));//FIXME
	else
		gb_RenderDevice->SetNoMaterial(GetAttribute(ATTRUNKOBJ_ADDBLEND)?ALPHA_ADDBLEND:ALPHA_BLEND,MatXf::ID,0,GetTexture());//FIXME
