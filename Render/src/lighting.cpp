...
{
	time=0;
	pTexture=NULL;//GetTexLibrary()->GetElement(param.texture_name.c_str());
	stopGenerate = false;
...
void cLighting::Update(const Vect3f& pos_begin_, const vector<Vect3f>& pos_end_)
{
...
void cLighting::OneLight::Update(Vect3f pos_delta)
{
	for(int i=0;i<strip_list.size();i++)
	{
		OneStrip& p=strip_list[i];
