...
| 10b | **`FieldOfViewMap`** | `VistaRender/FieldOfView.cpp` — `FieldOfViewMap::Draw` | The other quad into the lightmap, and *not* the fog of war (#10, now ported) despite sharing its channel. It blends colour **and** alpha (`ALPHA_BLEND`, no mask), so its alpha would land in the channel the fog of war now owns — port it with that collision in mind, not by copying `FogOfWar::Draw`. Its `updateTexture()` is also still unwired (`Environment::graphQuant`). |
