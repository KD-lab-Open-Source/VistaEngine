...
void CAnimationGroupMatDlg::OnBnClickedAddGroup()
{
	...
	AddGroup((LPCSTR)m_NewGroupName);
...
BOOL CAnimationGroupMatDlg::OnInitDialog()
{
	...
	for(int iag=0;iag<animation_group.size();iag++)
	{
		...
		{
			IGameMaterial* node=ag.materials[inode];
			NODE_MAP::iterator it=node_map.find(node);
			if(it!=node_map.end())
			{
				IGameMaterial* cur_node=it->first;
				const char* name=cur_node->GetMaterialName();
...
void CAnimationGroupMatDlg::BuildTreeGroup()
{
	...
	for(int igroup=0;igroup<animation_group.size();igroup++)
	{
		...
		if(hgroup)
		{
			...
			{
				string object_name=ag.materials[iobject]->GetMaterialName();
...
void CAnimationGroupMatDlg::BuildTreeObject()
{
	...
	{
		IGameMaterial *mat=pRootExport->GetMaterial(loop);
		AddTreeNode(mat,NULL,TVI_ROOT);
	}
}
//===================================================================================================================
void CAnimationGroupMatDlg::AddTreeNode(IGameMaterial* current,IGameMaterial* parent,HTREEITEM hparent)
{
	char* name=current->GetMaterialName();
...
{
	IGameMaterial* pGameNode=(IGameMaterial*)m_ObjectTree.GetItemData(hObject);
	AddObjectInGroup(hGroup,pGameNode);
}
//===================================================================================================================
void CAnimationGroupMatDlg::AddObjectInGroup(HTREEITEM hGroup,IGameMaterial* pGameNode)
{
	{//Добраться до root
	...
	string object_name=pGameNode->GetMaterialName();
...
//===================================================================================================================
bool CAnimationGroupMatDlg::DeleteGroupObject(IGameMaterial* pGameNode,string group_name_)
{
	string object_name=pGameNode->GetMaterialName();
...
void CAnimationGroupMatDlg::UpdateObjectTree(HTREEITEM hparent)
{
	if(hparent!=TVI_ROOT)
	{
		IGameMaterial* pGameNode=(IGameMaterial*)m_ObjectTree.GetItemData(hparent);
		string group_name;
		NODE_MAP::iterator it=node_map.find(pGameNode);
		if(it!=node_map.end())
			group_name=it->second;

		string name=pGameNode->GetMaterialName();
