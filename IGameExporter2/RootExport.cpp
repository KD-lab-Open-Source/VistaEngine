...
void RootExport::CalcAllNodes()
{
	...
	for(int inode=0;inode<all_nodes.size();inode++)
	{
		...
		IGameNode* parent_parent=NULL;
		if(n.pParent)
		{
			parent_name=n.pParent->GetName();
			parent_parent=n.pParent->GetNodeParent();
		}
	}
}
...
void RootExport::CalcNodeMapOptimize()
{
	...
	*/
	...
	for(int inode=0;inode<all_nodes.size();inode++)
	{
		...
		{
			IGameNode* node=n.additional[imesh];
		...
		}else
		{
			if(strcmp(name,"logic bound")!=0 && !IsLogicNodeBound(n.node))
			{
				for(int iac=0;iac<animation_data.animation_chain.size();iac++)
				{
					...
					if(CalcNodeAnimate(n,ac.begin_frame,max(ac.end_frame-ac.begin_frame+1,1),ac.cycled))
					{
						n.is_nodelete=true;
						break;
					}
...
bool RootExport::CalcNodeAnimate(NextLevelNode& n,int interval_begin,int interval_size,bool cycled)
{
	IGameNode * node=n.node;
	IGameNode* pParent=n.pParent;
	...
	{
		IGameObject* obj=n.node->GetIGameObject();
		if(obj)
		{
...
void RootExport::SaveNodes(Saver& saver, ChainsBlock& chains_block)
{
	...
	{//test duplicate
		typedef map<string,IGameNode*> ObjectMap;
		ObjectMap omap;
		for(int inode=0;inode<all_nodes.size();inode++)
		{
			NextLevelNode& n=all_nodes[inode];
	...
	for(int inode=0;inode<all_nodes.size();inode++)
	{
		...
		for(int imesh=-1;imesh<(int)n.additional.size();imesh++)
		{
			...
			if (IsNodeMesh(node)) 
			{
				...
				mesh.Export((IGameMesh*)node->GetIGameObject(),n.current);
			}
		}
	}
	saver.pop();
...
void RootExport::SaveLights(Saver& saver, ChainsBlock& chains_block)
{
	...
	for(int inode=0;inode<all_nodes.size();inode++)
	{
		...
		if(IsNodeLight(n.node))
		{
			...
			light.Export((IGameLight*)n.node->GetIGameObject(),n.current, chains_block);
		}
	}
	saver.pop();
}
...
/**/
	bool save=false;
//	if(strcmp(node_name,"Turret_logic")==0)
//	{
//		save=true;
//	}
	...
	for(int icurrent=0;icurrent<interval_size;icurrent++)
	{
		...
		if(is_2)
		{
			...
			mparent.NoTrans();//Непонятно, почему нет NoScale
			m=m*mparent;
		}
...
//		Msg("  pos=%f %f %f\n",ap.t.x,ap.t.y, ap.t.z);
	...
	ipos.interval_size = interval_size;
	...
	*/
	...
	irot.interval_size = interval_size;
	...
	if(save)
	{
		...
		for(int iout=0;iout<irot.out_data.size();iout++)
		{
			...
			Msg("%i begin=%f, size=%f\n",iout, float(o.interval_begin) / float(interval_size),
					float(o.interval_size) / float(interval_size));
			{
				for(int i=0;i<4;i++)
				{
	...
	*/
	...
	iscale.interval_size = interval_size;
...
void RootExport::BuildMaterialList()
{
	...
	for(int i=0;i<size;i++)
	{
		...
		if (mat->IsMultiType())
		{
			...
			{
				IGameMaterial *sub_mat=  mat->GetSubMaterial(j);
	...
	{//duplicate material
		IGameMaterial* mat_i=materials[i];
		const char* mat_name_i=mat_i->GetMaterialName();
		for(int j=i+1;j<materials.size();j++)
		{
			IGameMaterial* mat_j=materials[j];
			const char* mat_name_j=mat_j->GetMaterialName();
	...
	{
		vector<IGameMaterial*> materials_correct(duplicate_material_names.size(),NULL);
		for(int i=0;i<materials.size();i++)
		{
			IGameMaterial* mat=materials[i];
	...
	{//duplicate material
		...
		int num_tex = mat_i->GetNumberOfTextureMaps();
		for(int t=0; t<num_tex; t++)
		{
			if(mat_i->GetIGameTextureMap(t)->GetStdMapSlot() == ID_DI)
				diffname_i = GetFileName(mat_i->GetIGameTextureMap(t)->GetBitmapFileName());
			if(mat_i->GetIGameTextureMap(t)->GetStdMapSlot() == ID_FI)
				fillcolorname_i = GetFileName(mat_i->GetIGameTextureMap(t)->GetBitmapFileName());
		}
		for(int j=i+1;j<materials.size();j++)
		{
			IGameMaterial* mat_j=materials[j];
			...
			int num_tex = mat_j->GetNumberOfTextureMaps();
			for(int t=0; t<num_tex; t++)
			{
				if(mat_j->GetIGameTextureMap(t)->GetStdMapSlot() == ID_DI)
					diffname_j = GetFileName(mat_j->GetIGameTextureMap(t)->GetBitmapFileName());
				if(mat_j->GetIGameTextureMap(t)->GetStdMapSlot() == ID_FI)
					fillcolorname_j = GetFileName(mat_j->GetIGameTextureMap(t)->GetBitmapFileName());
			}
...
void RootExport::CheckMaterialInAnimationGroup()
{
	...
	{
		IGameMaterial* mat=materials[imat];
		if(mat==NULL)
			continue;
		bool found=false;
...
void RootExport::SaveBasement(Saver& saver)
{
	if(!all_nodes.empty() && node_base)
	{
		...
		//
		if(node_base->GetIGameObject() && node_base->GetIGameObject()->GetIGameType()==IGameObject::IGAME_MESH)
		{
			ExportBasement base(saver,node_base->GetName());
			Matrix3 m_root=root->GetWorldTM(GetBaseFrameMax());
			m_root.NoRot();
			m_root.NoScale();
			base.Export((IGameMesh*)node_base->GetIGameObject(),node_base,m_root);
		}else
		{
...
void RootExport::FindLogicBounds(IGameMesh*& pLogicBound,vector<sLogicNodeBound>& node_bounds)
{
	pLogicBound=NULL;
	node_bounds.clear();
	for(int inode=0;inode<all_nodes.size();inode++)
	{
		...
		for(int imesh=-1;imesh<(int)n.additional.size();imesh++)
		{
			...
			if(strcmp(name,"logic bound")==0)
			{
				if(!(node->GetIGameObject() && node->GetIGameObject()->GetIGameType()==IGameObject::IGAME_MESH))
				{
					Msg("Объект с именем logic bound должен состоять из треугольников.\n");
					break;
				}
				pLogicBound=(IGameMesh*)node->GetIGameObject();
			}
			...
			if(IsLogicNodeBound(node))
			{
				...
				{
					sLogicNodeBound s;
					s.pobject=(IGameMesh*)node->GetIGameObject();
					s.inode=inode;
...
void RootExport::SaveLogicBounds(Saver& saver,IGameMesh* pLogicBound,vector<sLogicNodeBound>& node_bounds)
{
	if(pLogicBound)
...
void RootExport::SaveLogicBoundRoot(Saver& saver,IGameMesh* pobject)
{
	NextLevelNode* root=&all_nodes[0];
...
void RootExport::SaveLogicBoundLocal(Saver& saver,IGameMesh* pobject,int inode)
{
...
IGameMaterial* RootExport::FindMaterial(const char* name)
{
	for(int i=0;i<materials.size();i++)
	{
		IGameMaterial *mat=materials[i];
...
void RootExport::LoadMapNode(CLoadIterator it,MAP_NODE& nodes,const char* error_message)
{
	...
	FOR_EACH(non_delete,itn)
	{
		...
		IGameNode* node=Find(node_name.c_str());
		if(node)
			nodes[node]=1;
		else
...
void RootExport::CalcBoundBox()
{
	...
	for(int inode=0;inode<all_nodes.size();inode++)
	{
		...
		for(int imesh=-1;imesh<(int)n.additional.size();imesh++)
		{
			...
			Matrix3 mparent=node->GetWorldTM(GetBaseFrameMax()).ExtractMatrix3();
			...
			{
				IGameMesh* mesh=(IGameMesh*)node->GetIGameObject();
				int  num_verts=mesh->GetNumberOfVerts();
...
bool RootExport::IsLogicNodeBound(IGameNode* node)
{
	const char* name=node->GetName();
