...
void CVisibleGroupDlg::AddNode(IGameNode* current,vector<IGameNode*> &groups)
{
...
void CVisibleGroupDlg::AddTreeNode(IGameNode* current,int aset,int ag)
{
	if(current->IsTarget())
		return;
...
void CVisibleGroupDlg::OnBnClickedAddSet()
{
	...
	for(int i=0;i<animation_sets.size();i++)
	{
		if(animation_sets[i].name == (LPCSTR)dlg.m_Name)
		{
	...
	av.name = (LPCSTR)dlg.m_Name;
	animation_sets.push_back(av);

	HTREEITEM hitem = m_SetsTree.InsertItem(dlg.m_Name,0,0);
}
...
void CVisibleGroupDlg::OnNMClickListVisible(NMHDR *pNMHDR, LRESULT *pResult)
{
	...
	IGameNode* current = (IGameNode*)m_VisibleList.GetItemData(nItem);
	if (!current)
		return;
