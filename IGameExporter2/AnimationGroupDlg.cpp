...
{
	typedef map<IGameNode*,int> NODE_MAP;
	NODE_MAP node_map;
	bool found=false;
	for(int iag=0;iag<animation_group.size();iag++)
	{
		...
		{
			IGameNode* node=ag.groups[inode];
			NODE_MAP::iterator it=node_map.find(node);
			if(it!=node_map.end())
			{
				IGameNode* cur_node=it->first;
...
{
	typedef map<IGameMaterial*,int> NODE_MAP;
	NODE_MAP node_map;
	bool found=false;
	for(int iag=0;iag<animation_group.size();iag++)
	{
...
//===================================================================================================================
void CAnimationGroupDlg::AddTreeNode(IGameNode* current,IGameNode* parent,HTREEITEM hparent)
{
	if(current->IsTarget())
		return;
...
{
	IGameNode* pGameNode=(IGameNode*)m_ObjectTree.GetItemData(hObject);
	AddObjectInGroup(hGroup,pGameNode);
}
//===================================================================================================================
void CAnimationGroupDlg::AddObjectInGroup(HTREEITEM hGroup,IGameNode* pGameNode)
{
	{//Добраться до root
...
//===================================================================================================================
bool CAnimationGroupDlg::DeleteGroupObject(IGameNode* pGameNode,string group_name_)
{
	string object_name = pGameNode->GetName();
...
void CAnimationGroupDlg::UpdateObjectTree(HTREEITEM hparent)
{
	if(hparent!=TVI_ROOT)
	{
		IGameNode* pGameNode=(IGameNode*)m_ObjectTree.GetItemData(hparent);
		string group_name;
		NODE_MAP::iterator it=node_map.find(pGameNode);
		if(it!=node_map.end())
