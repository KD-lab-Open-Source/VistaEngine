...
void ExportLight::Export(IGameLight* pobject_,int inode_current_, ChainsBlock& chains_block)
{
	pobject=pobject_;
	...
	saver.push(C3DX_LIGHT);
		...
		int time=0;
		Point3 color=LightObj->GetRGBColor(time);

		Vect3f colorv(color.x,color.y,color.z);
		saver<<colorv;

		float fAttenStart=0,fAttenEnd=0;
		fAttenStart=LightObj->GetAtten(time,ATTEN_START);
		fAttenEnd=LightObj->GetAtten(time,ATTEN_END);
		saver<<fAttenStart;
		saver<<fAttenEnd;
		...
		Texmap *tex=LightObj->GetProjMap();
		if(tex)
		if (tex->ClassID() == Class_ID(BMTEX_CLASS_ID, 0))
		{
			const char* tex_name=((BitmapTex*)tex)->GetMapName();
			if(tex_name)
			{
...
void ExportLight::SaveColor(int interval_begin,int interval_size,bool cycled, ChainsBlock& chains_block)
{
	...
	for(int icurrent=0;icurrent<interval_size;icurrent++)
	{
		...
		Point3 c=LightObj->GetRGBColor(current_max_time);
		float alpha=LightObj->GetIntensity(current_max_time);
		VectColor p;
		p[0]=c.x;
		p[1]=c.y;
		p[2]=c.z;
