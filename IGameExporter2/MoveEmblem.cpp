...
BOOL cMoveEmblem::OnInitDialog()
{
	...
	for(int i=0;i<ItemsCount;i++)
	{
		...
		IGameMaterial* material=pRootExport->GetMaterial(i);
		int  num_maps=material->GetNumberOfTextureMaps();
		if(num_maps==0)
			continue;
		const char* fname = material->GetIGameTextureMap(0)->GetBitmapFileName(); 
		bool found = false;
...
void cMoveEmblem::OnLvnItemchangedMaterialsList(NMHDR *pNMHDR, LRESULT *pResult)
{
	...
	if (ItemIndex>-1 && pNMLV->uNewState == 3)
	{
		...
		IGameMaterial* material=pRootExport->GetMaterial(n);
			int  num_maps=material->GetNumberOfTextureMaps();
			if(num_maps==0)
				return;
			const char* fname = GetFileName(material->GetIGameTextureMap(0)->GetBitmapFileName()); 
