...
bool ChainLightningController::checkLevel(int level) const
{
	...
	case WORKING:
		return true;
	case FADE_ON:
		return phaseTime_() > attr_->spreadingTime_ * 1000 * level / attr_->chainFactor_;
		break;
	case FADE_OFF:
		return phaseTime_() <= attr_->spreadingTime_ * 1000 * level / attr_->chainFactor_;
		break;
...
void ChainLightningController::quant()
{
	...
	switch(phase_){
	case FADE_OFF:
		if(phaseTime_() >= 1000 * attr_->spreadingTime_){
			phase_ = NOT_STARTED;
			phaseTime_.stop();
			release();
	case NOT_STARTED:
...
void ChainLightningController::showDebugChainRecurse(const ChainNode& node, int deep) const
{
	...
	if(showDebugSource.showLightningUnitChainRadius)
		show_vector(node.unitNode->position(), attr_->unitChainRadius_, node.unitNode->selected() ? RED : GREEN);
	ChainNodes::const_iterator it;
	FOR_EACH(node.graf, it){
		if(it->unitNode)
			show_line(node.unitNode->position(), it->unitNode->position(), checkLevel(deep) ? GREEN : WHITE);
...
void ChainLightningController::showDebug() const
{
	if(phase_ == NOT_STARTED)
		return;
	show_vector(owner_->position(), attr_->chainRadius_, GREEN);
	ChainNodes::const_iterator it;
	FOR_EACH(chainGraf_, it){
		if(!it->unitNode)
			continue;
		show_line(owner_->position(), it->unitNode->position(), GREEN);
