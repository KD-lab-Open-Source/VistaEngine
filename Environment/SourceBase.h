...
public:
	...
	/// поверхность, где источник может работать
	enum SurfaceClass {
		/// везде
		SOURCE_SURFACE_ANY,
		/// только на земле
	...
	/// род поверхности, где источник может работать
...
protected:
	...
	/// тип поверхности, на которой может работать источник
	/// проверяется один раз - при создании источника в игре
	/// если поверхность неподходящая, то источник не создаётся
