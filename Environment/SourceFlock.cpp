...
void Bird::create(const SourceFlock* owner)
{
	...
	model_->SetScale(logicRndInterval(owner->modelSize()));
...
void SourceFlock::showEditor() const
{
	...
	FOR_EACH(knots_, kit){
		...
		gb_RenderDevice->DrawLine(pos - Vect3f::I * crossSize, pos + Vect3f::I * crossSize, polus ? RED : GREEN);
		gb_RenderDevice->DrawLine(pos - Vect3f::J * crossSize, pos + Vect3f::J * crossSize, polus ? RED : GREEN);
		gb_RenderDevice->DrawLine(pos - Vect3f::K * crossSize, pos + Vect3f::K * crossSize, polus ? RED : GREEN);
		polus = !polus;
	}
}
...
void SourceFlock::showDebug() const
{
	...
	FOR_EACH(knots_, kit){
		Vect3f pos = kit->position_;
		show_vector(pos - Vect3f::I * crossSize, 2.f * Vect3f::I * crossSize, polus ? RED : color);
		show_vector(pos - Vect3f::J * crossSize, 2.f * Vect3f::J * crossSize, polus ? RED : color);
		show_vector(pos - Vect3f::K * crossSize, 2.f * Vect3f::K * crossSize, polus ? RED : color);
		polus = !polus;
	}
}
