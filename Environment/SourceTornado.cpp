...
void SourceTornado::apply( UnitBase* unit )
{
	...
	if(unit->attr().isActing()  &&	safe_cast<UnitReal*>(unit)->currentState() != CHAIN_TRIGGER && !unit->isDocked()){
		if(!safe_cast<RigidBodyUnit*>(body)->isBoxMode()){
			safe_cast<RigidBodyUnit*>(body)->disableWaterAnalysis();
			safe_cast<RigidBodyBox*>(body)->setAngularEvolve(false);
			safe_cast<RigidBodyUnit*>(body)->enableBoxMode();
		}
		Vect3f pulse = getForce(unit->position()); 
		pulse *= tornadoFactor_ / unit->attr().mass;
		pulse -= body->velocity();
		pulse += Vect3f::K * (body->prm().gravity * logicPeriodSeconds);
		pulse *= safe_cast<RigidBodyPhysics*>(body)->core()->mass();
		safe_cast<RigidBodyPhysics*>(body)->addImpulse(pulse, getTorque(unit->position()));
	}
...
Vect3f SourceTornado::getForce( const Vect3f& unitPos )
{
	...
	float horizontalForce = 1.0f + (1.0f + sin(pointDist/radius_ * 2.0f*M_PI - M_PI_2))/2.1f;
	dir.normalize(horizontalForce);
	Vect3f horizontDir(dir.x, dir.y, 0);

	float rotAngle = 90.0f - sinf((pointDist/radius_) * M_PI_2)*90.0f;
	QuatF rotQuat(rotAngle / (180.0f/M_PI), Vect3f::K);
	rotQuat.xform(horizontDir);

	float pointH = unitPos.z - position().z;
	horizontDir += Vect3f(-dir.x, -dir.y, 0.0f) * (pointH/height_) * 3.0f;
...
Vect3f SourceTornado::getTorque( const Vect3f& unitPos )
{
	...
	return Vect3f(0.0f, 0.0f, horizontalForce);
}
