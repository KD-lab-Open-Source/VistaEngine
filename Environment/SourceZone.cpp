...
void SourceZone::quant()
{
	...
	if(editType_ == ZONE_GENERATOR){
		if(!isUnderEditor()){
			if(!unitGenerationTimer_.busy() && !unitIntervalTimer_.busy()){
				generateUnits();
				if(unitCur_){
					...
					if(unitGenerationOnce_){
						...
						unitGenerationTimer_.start(int((unitGenerationPeriod_.minimum() + logicRNDfabsRnd(unitGenerationPeriod_.length())) * 1000.0f));
						if(unitGenerationMode_ == GENERATION_MODE_ZONE)
							unitCur_ = unitCount_;
					}
				}
		...
		if(editType_ == ZONE_WALK_EFFECT){
			...
			float da = 2.0f * M_PI / (10.0f * wander_time), rr = radius_ / 2.50f;
	...
	if(!effectAttribute_.isEmpty())
		if((effectAttribute_.switchOffUnderWater() || effectAttribute_.switchOffUnderLava() || effectAttribute_.switchOffByDay())
			&& ((effectAttribute_.switchOffByDay() && environment->isDay())
				|| (((water->isLava() && effectAttribute_.switchOffUnderLava()) || (!water->isLava() && effectAttribute_.switchOffUnderWater()))
...
void SourceZone::generateUnits()
{
	if(unitGenerationMode_ == GENERATION_MODE_ZONE){
		...
		do{
			...
			float groundZ = float(vMap.getApproxAlt(point.xi(), point.yi()));
			Vect3f unitPosition(point.x, point.y, groundZ + unitGenerationHeight_);
			if(unitGenerationHeight_ > FLT_EPS && unit->rigidBody() && unit->rigidBody()->isUnit()){
				...
				safe_cast<UnitReal*>(unit)->changeState(StateBirthInAir::instance());
			}
		...
		for(int i = 0; i < unitCount_; i++){
			...
			if(unit->rigidBody() && unit->rigidBody()->isBox()){
				...
				unit->rigidBody()->setForwardVelocity(unitVelocity_.minimum() + logicRNDfabsRnd(unitVelocity_.length()));
...
void SourceDetector::quant()
{
	...
	sourceManager->sourceGrid.Scan(position().xi(), position().yi(), round(radius()), scanOp);
}
...
void SourceDetector::apply(UnitBase* target)
{
	...
	if(target->attr().isActing())
		safe_cast<UnitActing*>(target)->setVisibility(editType_ == ZONE_DETECTOR);
}
...
void SourceFreeze::apply(UnitBase* target)
{
	...
	if(active() && target->attr().isObjective())
		safe_cast<UnitReal*>(target)->makeFrozen(freezeAttack_);
}

void SourceFreeze::serialize(Archive& ar)
{
	__super::serialize(ar);
