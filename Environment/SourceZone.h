...
{
public:
	/// тип зоны (исключительно для редактирования)
	enum ZoneEditType {
		ZONE_GENERATOR,
		ZONE_AFFECT,
		ZONE_WALK_EFFECT
	...
	{
		/// объекты появляются на поверхности мира
		GENERATION_MODE_TERRAIN,
		/// пачка юнитов распределенная равномерно по зоне
...
private:
	...
	/// диапазон отклонения от вертикали
	...
	/// периодичность появления объектов, в секундах
	Rangef unitGenerationPeriod_;
	/// количество одновременно появляющихся юнитов
	int unitCount_;
	/// количество оставшихся в очереди юнитов
	int unitCur_;
	/// интервал генерации юнитов (если 0 появляются все сразу)
	int unitGenerationInterval_;
	/// однократное создание
	bool unitGenerationOnce_;
	/// высота, на которой появляются объекты (для GENERATION_MODE_SKY)
	float unitGenerationHeight_;
	/// генерация по радиусу
	bool generateByRadius_;
	
	LogicTimer unitGenerationTimer_;
	LogicTimer unitIntervalTimer_;
	...
	/// время за которое блуждающий эффект пройдет полный цикл
	int wander_time;
	...
	/// создаёт юнит, устанавливает ему позицию и скорость
	void generateUnits();
