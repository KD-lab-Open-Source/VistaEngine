...
void SourceImpulse::apply(UnitBase* unit)
{
	...
	if(abnormalState_.isEnabled() && (unit->attr().isLegionary() || unit->attr().isBuilding())){
		if(unit->harmAttr().abnormalStateEffect(abnormalState_.type()))
	...
	if((body->isUnit() && 
		...
		!safe_cast<RigidBodyUnit*>(body)->isFrozen() && !unit->isDocked() && 
		...
		if(norm > radius())
			return;
		float pulse(sqrtf(safe_cast<RigidBodyPhysics*>(body)->core()->mass() * 10.0f / (norm + 10.0f)));
		...
		if(fabsf(torqueImpulse) < 0.1f){
			...
			if(unit->attr().isEnvironment()){
				...
				safe_cast<RigidBodyPhysics*>(body)->addImpulse(dir, angVel);
			}
			else
				safe_cast<RigidBodyBox*>(body)->addImpulseLinear(dir);
		}
		else{
			if(body->prm().moveVertical)
				return;
