...
bool needTranslation(const std::pair<std::string, std::string*>& p)
{
	...
	for(i = 0; i < p.first.size(); ++i){
		if(reinterpret_cast<const unsigned char&>(p.first[i]) >= (unsigned char)(0xC0)) // ������� '�'
			return true;
	}
	return false;
}
...
void Dictionary::serialize(Archive& ar) 
{
    ...
    ar.serialize(codePage_, "codePage", "������� ��������");
	ar.serialize(useFallback_, "useFallback", 0);
	untranslated.serialize(ar, "untranslated", "��������������");
	translated.serialize(ar, "translated", "�������");
	if(ar.isInput()){
		strings_.clear();
