...
void Console::graphQuant()
{
	...
	if(!logic_.messages_.empty()){
		int count = 0;
		{
			MTAuto lock(logicLock_);
			if(count = logic_.messages_.size()){
				std::copy(logic_.messages_.begin(), logic_.messages_.end(),
						  back_inserter(graph_.messages_));
				logic_.messages_.clear();
			}
		}
		if(count){
			Messages::iterator it = graph_.messages_.begin();
			std::advance(it, graph_.messages_.size() - count);

			for(; it != graph_.messages_.end(); ++it){
				flushMessage(*it);
			}
		}
	}
...
void Console::write(Level level, const char* author, const char* text, Location location)
{
	if(isAvailable()){
		...
		ThreadBlock& b = MT_IS_GRAPH() ? graph_ : logic_;
		
		if(MT_IS_GRAPH()){
			b.messages_.push_back(message);
...
bool Console::openLogFile()
{
	...
	for(int i = 0; i < 10; i++){
		...
		if(file_.open(buf, XS_OUT)){
			...
			graph_.messages_.insert(graph_.messages_.begin(), message);
			++graph_.unflushedCount_;
			logFileOpened_ = true;
			break; 
		}
	}
