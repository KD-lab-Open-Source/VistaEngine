...
{
	string sname;
	normalize_path(name.c_str(),sname);
	string uniqName;
	GetTexLibrary()->CreateUniqName(uniqName,sname.c_str());
...
void cObjStatistic::SetTexturesByObject(char* name)
{
	...
	for(i=0; i<textures.size(); i++)
	{
		...
		if (texture)
		{
			int newitem;
			itm.mask = LVIF_TEXT;
			itm.pszText = (LPSTR)texture->GetName();
			itm.iItem   = 0;
			itm.iSubItem= 0;
...
void cObjStatistic::RecalculateAndShow()
{
	...
	for(int index = 0; index<texLab->GetNumberTexture(); index++)
	{
		...
		newitem=InsertItem(hTextureListView,index,tex->GetName(),tex->GetLoaded()?-1:1);
	...
	if (environment)
	{
		...
		if(environment->PEManager())
		{
			...
			SetItem(hPostEffects,newitem,1,GetFormatedSize(environment->PEManager()->GetBaseTexturesSize()));
		}
		if (environment->PEManager() && environment->PEManager()->IsActive(PE_BLOOM))
		{
			newitem=InsertItem(hPostEffects,0,"Bloom");
			SetItem(hPostEffects,newitem,1,GetFormatedSize(environment->PEManager()->GetTexturesSize(PE_BLOOM)));
		}
		if (environment->PEManager() && environment->PEManager()->IsActive(PE_MONOCHROME))
		{
			newitem=InsertItem(hPostEffects,0,"Monochrome");
			SetItem(hPostEffects,newitem,1,GetFormatedSize(environment->PEManager()->GetTexturesSize(PE_MONOCHROME)));
		}
		if (environment->PEManager() && environment->PEManager()->IsActive(PE_UNDER_WATER))
		{
			newitem=InsertItem(hPostEffects,0,"UnderWater");
			SetItem(hPostEffects,newitem,1,GetFormatedSize(environment->PEManager()->GetTexturesSize(PE_UNDER_WATER)));
		}
		if (environment->PEManager() && environment->PEManager()->IsActive(PE_DOF))
		{
			newitem=InsertItem(hPostEffects,0,"Depth of Field");
			SetItem(hPostEffects,newitem,1,GetFormatedSize(environment->PEManager()->GetTexturesSize(PE_DOF)));
		}
		if (environment->PEManager() && environment->PEManager()->IsActive(PE_MIRAGE))
		{
			newitem=InsertItem(hPostEffects,0,"Mirage");
			SetItem(hPostEffects,newitem,1,GetFormatedSize(environment->PEManager()->GetTexturesSize(PE_MIRAGE)));
		}
	}	
	//
...
void cObjStatistic::SetVisibleObjects()
{
	...
	typedef map<string,PolygonStatistic> smap;
	smap m;
...
void cObjStatistic::PrepareUniqueObjects(vector<cObject3dx*>& objects)
{
	...
	for(int i = 0; i < libraryObjects.size(); ++i){
		...
		if(staticObject->GetRef() == 1){
			...
			addObj.name = staticObject->file_name.c_str();
			...
			addObj.animationSize = staticObject->chains_block.GetBlockSize();
	...
	for(int i=0; i<objects.size(); i++)
	{
		...
		if(!ident)
		{
			...
			addObj.animationSize = obj->GetStatic()->chains_block.GetBlockSize();
...
{
	int total_,free_;
	gb_RenderDevice3D->GetIndexPool()->GetUsedMemory(total_,free_);
	totalIndexMapSize = total_;
	gb_RenderDevice3D->GetVertexPool()->GetUsedMemory(total_,free_);
...
{
	int total_,free_;
	gb_RenderDevice3D->GetTilemapTextureMemory(total_,free_);
