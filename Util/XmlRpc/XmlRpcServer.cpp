...
XmlRpcServer::bindAndListen(int port, int backlog /*= 5*/)
{
	...
	XmlRpcUtil::log(1, "XmlRpcServer::bindAndListen: server listening on port %d fd %d", port, fd);
...
{
	XmlRpcUtil::log(4, "XmlRpcServer::work: running");
	_disp.work(msTime);
}
...
{
	unsigned long addr;
	int s = XmlRpcSocket::accept(this->getfd(), &addr);
	XmlRpcUtil::log(3, "XmlRpcServer::acceptConnection from %d.%d.%d.%d: socket %d", addr & 0xFF, (addr >> 4) & 0xFF, (addr >> 8) & 0xFF, (addr >> 12) & 0xFF, s);
	if (s < 0)
	{
		//this->close();
