...
int const COMBO_ID = ID_SCALE_COMBO;

CScaleBar::CScaleBar():
m_pScalable(NULL)
{}

BEGIN_MESSAGE_MAP(CScaleBar, CExtToolControlBar)
...
void CScaleBar::AddCombo()
{
	...
	*/
	m_wndCombo.Create(WS_VISIBLE|WS_CHILD|WS_VSCROLL|CBS_DROPDOWN, CRect(0, 0, 80, 300), this, ID_SCALE_COMBO);
	SetButtonCtrl(CommandToIndex(ID_SCALE_COMBO), &m_wndCombo, false);
...
bool CScaleBar::Create(CWnd* pparent, DWORD id)
{
	...
	if (!CExtToolControlBar::Create("Масштаб", pparent, id))
		return false;

	if (!LoadToolBar(IDR_TOOLBAR_SCALE, RGB(192, 192, 192)))
		return false;
