...
void TEBaseWorkMode::OnLButtonDblClk(TriggerEditorView* source, UINT nFlags, CPoint p)
{
	...
	if((GetAsyncKeyState(VK_MENU) >> 15) != 0){
		if(triggerInterface().editAction(getStrategyTrigger(triggerIndex), source->GetParent()->m_hWnd)) {
			getTriggerEditorLogic()->setDataChanged();
			source->InvalidateRect(NULL);
		}
	}
	else{
		if(triggerInterface().editCondition(getStrategyTrigger(triggerIndex), source->GetParent()->m_hWnd)) {
			getTriggerEditorLogic()->setDataChanged();
			source->InvalidateRect(NULL);
		}
	}
...
bool TEBaseWorkMode::OnCommandUpdateUI(TriggerEditorView *view, CCmdUI* pCmdUI)
{
	...
	case ID_EDIT_COPY:
		...
		return true;
	case ID_EDIT_FIND:
		onUpdateEditFind(view, pCmdUI);
		return true;
	case ID_EDIT_FIND_NEXT:
		onUpdateEditFindNext(view, pCmdUI);
		return true;
	case ID_EDIT_PASTE:
...
static UINT_PTR CALLBACK findReplaceFunc(HWND hdlg, UINT uiMsg, WPARAM wParam, LPARAM lParam)
{
	return FALSE;
}

void TEBaseWorkMode::onEditFind(TriggerEditorView* source)
{
	static char buffer[180];

	if(!findReplaceDialog_){

		CWnd* parent = source->GetParentFrame();
		findReplaceDialog_ = new CFindReplaceDialog();
		CFindReplaceDialog& dlg = *findReplaceDialog_;
		ZeroMemory(&dlg.m_fr, sizeof(dlg.m_fr));
		dlg.m_fr.lStructSize = sizeof(dlg.m_fr);
		dlg.m_fr.lCustData = reinterpret_cast<LPARAM>(this);
		dlg.m_fr.hwndOwner = parent->GetSafeHwnd();
		dlg.m_fr.Flags = FR_HIDEWHOLEWORD | FR_HIDEMATCHCASE | FR_HIDEUPDOWN | FR_ENABLEHOOK;
		dlg.m_fr.lpfnHook = &findReplaceFunc;
		dlg.m_fr.lpstrFindWhat = buffer;
		dlg.m_fr.wFindWhatLen = sizeof(buffer) - 1;

		dlg.Create(TRUE, buffer, 0, 0, parent);
		CRect rt;
		dlg.GetWindowRect(&rt);
		dlg.MoveWindow((GetSystemMetrics(SM_CXSCREEN) - rt.Width()) / 2, (GetSystemMetrics(SM_CYSCREEN) - rt.Height()) / 4, rt.Width(), rt.Height());
		dlg.ShowWindow(SW_SHOW);
	}
	findReplaceDialog_->SetFocus();
}

void TEBaseWorkMode::onFindReplace(TriggerEditorView* source)
{
	if(findReplaceDialog_){
		const char* str = findReplaceDialog_->m_fr.lpstrFindWhat;
		if(findLastText_ != str)
			findOccurence_ = 0;
		findLastText_ = str;

		if(!(findReplaceDialog_->m_fr.Flags & FR_FINDNEXT))
			findOccurence_ = 0;

		if(findReplaceDialog_->m_fr.Flags & FR_DIALOGTERM)
			findReplaceDialog_ = 0;
		else
			onFind(source);
	}
...
void TEBaseWorkMode::onFind(TriggerEditorView* source)
{
	const char* str = findLastText_.c_str();

	Trigger* trigger = getTriggerChain()->findBySubString(str, findOccurence_);
	if(trigger){
		int index = getTriggerChain()->triggerIndex(*trigger);
		getSelectionManager().deselectAll();
		getSelectionManager().select(index);

		CRect rect = trigger->boundingRect();
		CPoint point = rect.CenterPoint();

		CRect viewRect;
		source->GetClientRect(&viewRect);
		source->scalePoint(&point);
		point.x -= viewRect.Width() / 2;
		point.y -= viewRect.Height() / 2;

		source->setViewportOrg(-point.x, -point.y);
		...
		if(findOccurence_ > 0){
			int result = AfxMessageBox(TRANSLATE("Ничего не найдено, искать сначала?"), MB_YESNO | MB_ICONQUESTION);
			if(result == IDYES){
				findOccurence_ = 0;
				onFind(source);
			}
			...
			AfxMessageBox(TRANSLATE("Ничего не найдено"), MB_OK | MB_ICONINFORMATION);
		}
	}
}
...
void TEBaseWorkMode::onUpdateEditFind(TriggerEditorView* source, CCmdUI* pCmdUI)
{
	pCmdUI->Enable(TRUE);
}

void TEBaseWorkMode::onUpdateEditFindNext(TriggerEditorView* source, CCmdUI* pCmdUI)
{
	pCmdUI->Enable(findLastText_ != "");
}
