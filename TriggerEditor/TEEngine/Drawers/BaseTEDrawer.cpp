									...
									Trigger const& trigger) const
{
	...
	for(; itr != end; ++itr)
	{
		...
		CPoint begPoint(center);
		begPoint.Offset(link.parentOffset());

		CPoint endPoint(te_h::get_center(link.child->boundingRect()));
		endPoint.Offset(link.childOffset());

		DrawLink(source, *pdc, begPoint, endPoint, link);
	}
