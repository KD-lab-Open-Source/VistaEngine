...
void TriggerEditorView::setMiniMapManager(TEMiniMapManager* miniMap) {
	miniMapManager_ = miniMap;
}
...
void TriggerEditorView::setScaleRanges()
{
	...
	float fH = float(rcClient.Height())/max(getWorkArea().Height(), 1);
	float fW = float(rcClient.Width())/max(getWorkArea().Width(), 1);
...
void TriggerEditorView::onFindReplace(LPFINDREPLACE findReplace)
{
	if(ITEWorkMode* workMode = reinterpret_cast<ITEWorkMode*>(findReplace->lCustData)){
		workMode->onFindReplace(this);
	}
}
