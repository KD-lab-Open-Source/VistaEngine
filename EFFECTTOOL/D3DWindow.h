...
class CD3DWindow : public CView
{
	...
	Vect3f Trace(const CPoint& pt);
	Mat3f GetMatRot(CPoint ptDelta);
	int HitTestPoint(const Vect3f &pos, const CPoint &point);
	int HitTestPoints(const vector<Vect3f> &points, const CPoint &point);
	void RotateAround(vector<Vect3f>& points, Vect3f center, const Mat3f& rot);
	void SetKeyPos(KeyPos* key_pos, const CPoint& pt, const MatXf* offs =NULL);
	void SetVect3fPos(Vect3f& key_pos, const CPoint& pt, const MatXf* offs =NULL);
