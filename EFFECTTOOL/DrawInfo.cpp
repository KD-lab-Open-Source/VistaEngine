...
{
	Vect3f prev;
	for (float a = begin_a; a<=end_a; a+=0.1f)
	{
		prev.x = r*sin(a)*cos(begin_t);
		prev.y = r*cos(a)*cos(begin_t);
		prev.z = r*sin(begin_t);
		prev = m*prev;
		for (float t = begin_t; t<=end_t; t+=0.1f)
		{
			Vect3f cur;
			cur.x = r*sin(a)*cos(t);
			cur.y = r*cos(a)*cos(t);
...
void DrawCylinder(cInterfaceRenderDevice* pRenderDevice, const MatXf& mt,float r,float r2,float h, const sColor4c& c)
{
	InitCirclePos();
	MatXf m=MatXf::ID;
	MatXf p1,p2;
	...
	m.trans().z=h;
	p2=mt*m;
	DrawCircle(pRenderDevice, p2,r2,c);
