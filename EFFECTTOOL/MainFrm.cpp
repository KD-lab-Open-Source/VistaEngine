...
BEGIN_MESSAGE_MAP(CMainFrame, CFrameWnd)
	...
	ON_COMMAND(ID_TOOLS_TARGET, OnToolsTarget)
	ON_COMMAND(ID_TOOLS_COLOR, OnToolsColor)
	ON_COMMAND(ID_TOOLS_ENABLE_OVERDRAW, OnToolsEnableOverDraw)
	ON_COMMAND(ID_TOOLS_ENABLE_PRESET, OnToolsEnablePreset)
	...
	ON_UPDATE_COMMAND_UI(ID_LOAD_WORLD, OnUpdateToolsOptEnb)
	ON_UPDATE_COMMAND_UI(ID_TOOLS_TARGET, OnUpdateToolsTarget)
	ON_UPDATE_COMMAND_UI(ID_TOOLS_ENABLE_OVERDRAW, OnUpdateToolsEnableOverDraw)
	ON_COMMAND(ID_FINDCENTER,OnFindCenter)
	ON_COMMAND(ID_LOAD_WORLD, OnLoadWorld)
	ON_WM_ACTIVATEAPP()
	//}}AFX_MSG_MAP
...
CSize MakeVoluntaryImageList(UINT inBitmapID, UINT toolBarBitDepth, int amountImg, CImageList&	outImageList)
{
	...
	VERIFY(outImageList.Add(&bm, RGB(255,0,255)) != -1);
...
int CMainFrame::OnCreate(LPCREATESTRUCT lpCreateStruct)
{
	...
	int buttonCount = 19;
	CSize buttonSize = MakeVoluntaryImageList(IDB_TOOL_BAR_IMAGES, ILC_COLOR24, buttonCount, ToolBarImages);
	m_wndToolBar.GetToolBarCtrl().SetImageList(&ToolBarImages);
	m_wndToolBar.SetSizes(CSize(buttonSize.cx+7,buttonSize.cy+6+3), buttonSize); 
...
*/
	...
	COLORREF clr = RGB(50,100,50);
	clr = theApp.GetProfileInt("Scene","BackgroundColor",clr);
	theApp.scene.backgroundColor.r = GetRValue(clr);
	theApp.scene.backgroundColor.g = GetGValue(clr);
	theApp.scene.backgroundColor.b = GetBValue(clr);
	SetMenu(0);
	OnToolsEnablePreset();
	return 0;
}
...
void CMainFrame::OnToolsFullScreen()
{
	...
	m_wndToolBar.GetToolBarCtrl().EnableButton(ID_TOOLS_ENABLE_OVERDRAW,t);
}
...
void CMainFrame::OnTools() 
{
	...
	else if(m_wndToolBar.GetToolBarCtrl().IsButtonChecked(ID_TOOLS_TARGET))
		theApp.scene.m_ToolMode = CD3DScene::TOOL_TARGET;
	else
...
void CMainFrame::OnUpdateToolsTarget(CCmdUI* pCmdUI)
{
	pCmdUI->SetCheck(theApp.scene.m_ToolMode == CD3DScene::TOOL_TARGET);
}
void CMainFrame::OnUpdateTools(CCmdUI* pCmdUI) 
{
	pCmdUI->Enable(GetDocument()->ActiveEmitter() != 0 /*&& theApp.scene.m_bPause*/);
}
void CMainFrame::OnUpdateToolsInt(CCmdUI* pCmdUI)
{
	pCmdUI->Enable(GetDocument()->ActiveEmitter() != 0 &&
		(GetDocument()->ActiveEmitter()->Class() == EMC_INTEGRAL || GetDocument()->ActiveEmitter()->Class() == EMC_INTEGRAL_Z));
}
...
{
	pCmdUI->Enable(GetDocument()&&GetDocument()->ActiveEmitter()&&
		GetDocument()->ActiveEmitter()->Class()==EMC_SPLINE);
}
...
void CMainFrame::OnUpdateToolsEnableOverDraw(CCmdUI* pCmdUI)
{
}
void CMainFrame::OnUpdateToolsDir(CCmdUI* pCmdUI)
{
	...
	if (edat)
	{
		bs = edat->IsBase() && edat->particle_position().type != EMP_3DMODEL;
		lc = edat->IsCLight();
	}
	pCmdUI->Enable(	bs || lc);
}
...
void CMainFrame::OnToolsColor()
{
	CColorDialog dlg(RGB(theApp.scene.backgroundColor.r,theApp.scene.backgroundColor.g,theApp.scene.backgroundColor.b),CC_FULLOPEN);
	if (dlg.DoModal()==IDOK)
	{
		...
		theApp.scene.backgroundColor.g = GetGValue(clr);
		theApp.scene.backgroundColor.b = GetBValue(clr);
		theApp.WriteProfileInt("scene","BackgroundColor",clr);
	}
}
void CMainFrame::OnToolsEnableOverDraw()
{
	theApp.scene.enableOverDraw = !theApp.scene.enableOverDraw;
	if (theApp.scene.GetEffect())
	{
		theApp.scene.GetEffect()->enableOverDraw = theApp.scene.enableOverDraw;
	}
	if (m_wndToolBar.GetToolBarCtrl().IsButtonEnabled(ID_TOOLS_ENABLE_OVERDRAW))
		m_wndToolBar.GetToolBarCtrl().CheckButton(ID_TOOLS_ENABLE_OVERDRAW, theApp.scene.enableOverDraw);

}
void CMainFrame::OnToolsEnablePreset()
{
	theApp.scene.SetPreset();
	m_wndToolBar.GetToolBarCtrl().CheckButton(ID_TOOLS_ENABLE_PRESET, theApp.scene.GetPreset());
}

void CMainFrame::OnLoadWorld()
{
	GetDocument()->LoadWorld(true);
}
