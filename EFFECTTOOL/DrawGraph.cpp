...
CGraphObject* need_new = (CGraphObject*)(void*)0x0001;
...
static CBrush brushBlue(RGB(0, 0, 255));
static CBrush brushBlack(RGB(0, 0, 0));
				
static CPen   penBlue(PS_SOLID, 5, RGB(0, 0, 255));
static CPen   penRed(PS_SOLID, 4, RGB(255, 0, 0));
static CPen   penRedPoint(PS_DOT, 1, RGB(255, 0, 0));
static CPen   penGrayPoint(PS_DOT, 1, RGB(128, 128, 128));
...
void CGraphPoint::Draw(CDC* pDC)
{
	...
	if (!((CGraphLine*)GetParent())->editable)
		if (m_bActive)
...
void CGraphPoint::DoubleClick(CPoint& point)
{
	int a=0;
}
...
#include "OptTree.h"
extern COptTree* tr;
void CBlueLinePoint::DoubleClick()
{
	if(m_nNumber==0)
		return;
	...
	y *= pParent->koef;
	float pos = _pDoc->ActiveEmitter()->GenerationPointTime(m_nNumber)*_pDoc->ActiveEmitter()->emitter_life_time();
	CSetPositionDlg dlg;
	dlg.Position() = pos;
	if(dlg.DoModal()!=IDOK)
		return;
	m_pt.x = transX(dlg.Position());
}
void CBlueLinePoint::OnChange()
{
	...
	{
		float y = untransY(m_pt.y)*((CGraphLine*)m_pParent)->koef;
		CGraphKeyLine* pParent = (CGraphKeyLine*)m_pParent;
		if (pParent&&IS_CTRL())
		{
	...
	if (m_nNumber == edat->emitter_position().size()-1 && edat->emitter_position().size()!=1)
		edat->ChangeLifetime(t);
	else if(type==0 || m_key.size()>1)
		edat->set_gen_point_time(m_nNumber, t);
}
int CBlueLinePoint::GetState()
{ 
	DWORD st=0;
	CKeyPos& keypos = _pDoc->ActiveEmitter()->emitter_position();
	if (((CGraphBlueLine*)m_pParent)->type==0)
	{
		if (m_nNumber > 0 && m_nNumber < keypos.size() - 1)
			st|= TRACK_HORZ|REMOVEABLE;
		else if (m_nNumber > 0 && m_nNumber < keypos.size())
			st|= TRACK_HORZ;
	}
...
void CGraphLine::Draw(CDC* pDC)
{
	if(!m_bVisible)
		return;
	CPen* pen = editable ? m_pPen : &penGray;
	if(!m_points.empty())
	{
...
{
	if(!_pDoc->ActiveEmitter()||!_pWndGraph){
		m_points.clear();
		return;
	}
...
//		int ty = _nGraphHeight + n_x_axis_bottom_space*0.8;
		...
		if (type == 1 && nPoints==1)
		{
			float x = _pDoc->ActiveEmitter()->emitter_life_time();
		...
		else for(int i=0; i<nPoints; i++)
		{
			...
			float x = keypos[i].time*_pDoc->ActiveEmitter()->emitter_life_time();//_pDoc->get_gen_point_time(i);
			Transform(x,y);
			if (type==0)
				y = work.bottom + 20;
...
CGraphPoint* CGraphBlueLine::CreateNewPoint(const CPoint& pt)
{
	...
	FOR_EACH(m_points, it)
	{
		if((*it)->GetPos().x > pt.x)
		{
			...
			float x = pt.x;
			float tg =(x-x1)/(x2-x1);
			_pDoc->ActiveEmitter()->InsertGenerationPoint(k,tg);
			_pDoc->ActiveEmitter()->SetGenerationPointTime(k,untransX(pt.x)/_pDoc->ActiveEmitter()->emitter_life_time());
...
void CGraphBlueLine::OnRemoveObject(CGraphObject* pObject)
{
	...
	FOR_EACH(m_points, i)
	{
		if(pObject == *i)
		{
			_pDoc->ActiveEmitter()->DeleteGenerationPoint(k);
...
void CKeyLinePoint::DoubleClick()
{
	if(m_nNumber==0)
		return;
	_pWndGraph->CalcTransform();
	...
	CGraphKeyLine* pParent = (CGraphKeyLine*)m_pParent;
	y *= pParent->koef;
	float pos = _pDoc->ActiveEmitter()->get_particle_key_time(_pDoc->m_nCurrentGenerationPoint, m_nNumber);
	CSetPositionDlg dlg;
	dlg.Position() = pos;
	if(dlg.DoModal()!=IDOK)
		return;
...
void CKeyLinePoint::OnChange()
{
	...
	if(bLastPoint) //последняя точка - изменяем время жизни частиц
	{
		...
		_pDoc->ActiveEmitter()->change_particle_life_time(_pDoc->m_nCurrentGenerationPoint, x);
	}
	_pDoc->ActiveEmitter()->set_particle_key_time(_pDoc->m_nCurrentGenerationPoint, m_nNumber, x);
}
...
/////////////////////////////////////////////////////////////////////////////
//
CGraphKeyLine::CGraphKeyLine(KeyWrapBase& key, bool bPrimary, bool bNoVertTrack, float k) :
...
CGraphPoint* CGraphKeyLine::CreateNewPoint(const CPoint& pt)
{
	...
	FOR_EACH(m_points, it)
	{
		if((*it)->GetPos().x > pt.x)
		{
			...
			_pDoc->ActiveEmitter()->InsertParticleKey(k,tg);
			if(m_bNoVertTrack)
			{
...
//		int y = _nGraphHeight - m_key(k)*fScaleY;
		float x = _pDoc->ActiveEmitter()->get_particle_key_time(_pDoc->m_nCurrentGenerationPoint, k);
		float y = m_key(k)/koef;
		Transform(x,y);
	...
	FOR_EACH(m_points, it)
	{
		if(*it == pObject)
		{
			if(k==0||k==m_points.size()-1)
				return;
			_pDoc->ActiveEmitter()->DeleteParticleKey(k);
...
void CTimelineContainer::Update(bool bFullUpdate)
{
	if(bFullUpdate)
	{
		...
		if(_pDoc->ActiveEmitter() == 0)
		{
			if(_pDoc->ActiveEffect())
			{
				CEffectData *eff = _pDoc->ActiveEffect();
				for(int i=eff->EmittersSize()-1; i>=0; i--)
				{
					CEmitterData* em = eff->Emitter(i);
					m_items.push_back(new CEmitterTimeline(this, i, em->name().c_str()));
				}
...
{
	m_nEmitter = n;
	kindObject = CGraphObject::KIND_EMITTER_TIME_LINE;
...
void CEmitterTimeline::Draw(CDC* pDC)
{
	...
	CEmitterData* pKey = _pDoc->ActiveEffect()->Emitter(m_nEmitter);
	//------ Время частиц (пунктирная)
	if(pKey->IsActive())
		pDC->SelectObject(&penRedPoint);
	else
		pDC->SelectObject(&penGrayPoint);
	...
	CRect rc(m_endPoint, m_endPoint);
	rc.InflateRect(4, 4);
	pDC->Ellipse(rc);
	...
	{
		CRect rc(m_points[i], m_points[i]);
		rc.InflateRect(4, 4);
		pDC->Ellipse(rc);
	}
	int c = pKey->GetGenerationPointNum();
	if(c == 1)
		pDC->SelectObject(&brushBlack);

	rc = CRect(m_points.back(), m_points.back());
	rc.InflateRect(4, 4);
	pDC->Ellipse(rc);
	...
	int x;
	if(m_endPoint.x > m_points.back().x)
		x = m_endPoint.x + 10;
	else
		x = m_points.back().x + 10;
...
CGraphObject* CEmitterTimeline::HitTest(const CPoint& pt)
{
	...
	CRect rcp(m_endPoint, m_endPoint);
	rcp.InflateRect(4, 4);
	if (rcp.PtInRect(pt))
	{
		m_bEndPoint = true;
...
void CEmitterTimeline::DoubleClick()
{
	if(m_CurPoint<0&&!m_bEndPoint)
		return;
	_pWndGraph->CalcTransform();
	if(m_CurPoint>=0)
	{
		float pos = untransX(m_points[m_CurPoint].x);
		CSetPositionDlg dlg;
		dlg.Position() = pos;
		if(dlg.DoModal()!=IDOK)
			return;
		m_points[m_CurPoint].x = transX(dlg.Position());
	}else
	if(m_bEndPoint)
	{
		float pos = untransX(m_endPoint.x);
		CSetPositionDlg dlg;
		dlg.Position() = pos;
		if(dlg.DoModal()!=IDOK)
			return;
		m_endPoint.x = transX(dlg.Position());
	}
	//if(m_CurPoint>0)
	//	if (m_points[m_CurPoint].x<m_points[m_CurPoint-1].x) 
	//		m_points[m_CurPoint].x = m_points[m_CurPoint-1].x;
...
void CEmitterTimeline::Update(bool bFullUpdate)
{
	...
	_pWndGraph->CalcTransform();
	float y = transY(_pDoc->ActiveEffect()->EmittersSize()-m_nEmitter);
	...
	//		if (t<0) t=0;
	//		float dt = pKey->emitter_create_time() - t;
	//		pKey->emitter_create_time() = t;
	//		pKey->emitter_life_time() += dt;
	//	}
	//	else if (m_CurPoint==m_points.size()-1)
	//	{
	//		float t = untransX(m_points[m_points.size()-1].x);
	//		t-=pKey->emitter_create_time();
	//		if (t<.05f) t = .05f;
	//		pKey->ChangeLifetime(t);
	//	}
	//	else
	//	{
	//		float time_from = pKey->emitter_create_time();
	//		float t = untransX(m_points[m_CurPoint].x) - time_from;
	//		pKey->SetGenerationPointTime(m_CurPoint, t/pKey->emitter_life_time());
	//	}
	//}
	//if(m_bEndPoint)
	//{
	...
	//	{
	//		float t = untransX(m_endPoint.x) - pKey->emitter_create_time();
	//		if(pKey->generate_prolonged() || pKey->GetGenerationPointNum() > 1)
	//		{
	//			t -= pKey->emitter_life_time();
	//		}
	//		pKey->SetParticleLifeTime(t);
	//	}
	//}
	m_points.clear();
	...
	float time_from = 0; 
	int c = 0;
	time_from = pKey->emitter_create_time();
	c = pKey->GetGenerationPointNum();
	for(int i=0; i<c; i++)
	{
		float x = transX(pKey->GetGenerationPointTime(i));
	...
	float px = round(transX(time_from+pKey->ParticleLifeTime()));
	m_endPoint = CPoint(px,y);
}
...
//	float fScale = _nGraphWidth/_pWndGraph->GetTimescaleMax();
	...
	EmitterKeyInterface* peKey = _pDoc->ActiveEffect()->key[m_nEmitter];
	if (m_CurPoint<0 && !m_bEndPoint)
		peKey->emitter_create_time = untransX(m_points.front().x);
	CEmitterData* pKey = _pDoc->ActiveEffect()->Emitter(m_nEmitter);
	if (m_CurPoint>=0)
	{
		if (m_CurPoint==0 || m_CurPoint==m_points.size()-1)
		{
			float t = untransX(m_points[0].x);
			if (t<0) t=0;
			float dt = pKey->emitter_create_time() - t;
			...
			t = untransX(m_points[m_points.size()-1].x);
			t-=pKey->emitter_create_time();
			if (t<.05f) t = .05f;
			pKey->ChangeLifetime(t);
		}
...
//	pKey->emitter_create_time = float(m_points.front().x - _spacing_left)/fScale;
	if(peKey->emitter_create_time < 0)
		peKey->emitter_create_time = 0;
...
void CDrawGraph::Update(bool bFullUpdate)
{
	if(_pDoc)
	{
		...
		if(_pDoc->ActiveEmitter() ==  0 && _pDoc->ActiveEffect() && bFullUpdate)
			m_fEffectTimelineMax = 1.1*_pDoc->ActiveEffect()->GetTotalLifeTime();
...
void CDrawGraph::ShowKeys(list<DataLine>& keys, bool b)//list<CKey*>& keys, BSKey* pKeyBS, bool b, bool last,list<CKeyWithParam>& emit_param )
{
	...
	list<DataLine>::iterator it;
	FOR_EACH(keys, it)
	{
		switch(it->type)
		{
		case KeyLine:
			m_objects.push_back(new CGraphKeyLine(*(new KeyFltWrap(*it->pKey)), it->red, b, it->dat));
			break;
		case BlueLine:
			m_objects.push_back(new CGraphBlueLine(1,*(new KeyFltWrap(*it->pKey)),it->dat));
			break;
		case BSLine:
			m_objects.push_back(new CGraphKeyLine(*(new KeyBsWrap(*it->pKeyBS)), false, false));
			break;
		case HermitLine:
			m_objects.push_back(new CGraphKeyLine(*(new KeyFltWrap(*it->splkey)), true, true));
			break;
		}
		((CGraphLine*)m_objects.back())->editable = it->editable;
	}
/*	
	int k = 0;
...
//		else
		{
			m_fLastTimeMax = _pDoc->ActiveEmitter()->ParticleLifeTime();
			if (all)
			{
				float t  = _pDoc->ActiveEmitter()->emitter_life_time();	
				if (m_fLastTimeMax<t)
					m_fLastTimeMax = t;
			}
			if(m_fLastTimeMax < 0.1f)
				m_fLastTimeMax = 0.1f;
			tmax=m_fLastTimeMax;
		}
	}
	else if(_pDoc->ActiveEffect())
	{
		float f = _pDoc->ActiveEffect()->GetTotalLifeTime();
...
void CDrawGraph::OnLButtonDown(UINT nFlags, CPoint point) 
{
	...
	FOR_EACH(m_objects, i)
		if(m_pTrack = (*i)->HitTest(point))
		{
			...
			lockScale = true;
			const type_info& _type = typeid(**i);

			if(_type == typeid(CGraphBlueLine))
			{
				_pDoc->History().PushEmitter();
				if (m_pTrack==need_new)
					m_pTrack = ((CGraphLine*)*i)->CreateNewPoint(point);
				if (!m_pTrack)
					continue;
				_pDoc->m_nCurrentGenerationPoint = ((CBlueLinePoint*)m_pTrack)->m_nNumber;
			}
			else if((_type == typeid(CGraphKeyLine)))
			{
				_pDoc->History().PushEmitter();
				if (m_pTrack==need_new)
					m_pTrack = ((CGraphLine*)*i)->CreateNewPoint(point);
			...
			}else if (_type == typeid(CEmitterTimeline))
			{
				int ix = ((CEmitterTimeline*)(*i))->IxEmitter();
				_pDoc->History().PushEmitter(*_pDoc->ActiveEffect()->Emitter(ix), _pDoc->ActiveEffect(), _pDoc->ActiveGroup(), false);
...
void CDrawGraph::OnMouseMove(UINT nFlags, CPoint point) 
{
	if((nFlags & MK_LBUTTON)&&m_bDragging)
	{
		...
		if(m_pTrack && (m_pTrack->GetState() & CGraphObject::TRACK_ALL))
		{
			...
			if (m_bVertical && _pDoc->ActiveEmitter())
				m_pTrack->OffsetPos(0, ptDelta.y);
			else
...
void CDrawGraph::OnRButtonDown(UINT nFlags, CPoint point) 
{
	...
	ObjectVectorType::iterator i;
	FOR_EACH(m_objects, i)
		if((pObject = (*i)->HitTest(point)) && pObject!=need_new)
		{
			const type_info& _type = typeid(**i);
...
void CDrawGraph::OnLButtonDblClk(UINT nFlags, CPoint point)
{
	ObjectVectorType::iterator i;
	FOR_EACH(m_objects, i)
	{
		if((m_pTrack = (*i)->HitTest(point)) && m_pTrack!=need_new)
		{
			const type_info& _type = typeid(**i);
			if(_type == typeid(CGraphBlueLine))
			{
				_pDoc->m_nCurrentGenerationPoint = ((CBlueLinePoint*)m_pTrack)->m_nNumber;
				m_pTrack->DoubleClick();
				m_pTrack->OnChange();
				tr->CheckRangeEmitter();
				Update(false);
			}
			else if((_type == typeid(CGraphKeyLine)))
			{
				...
				tr->CheckRangeEmitter();
				Update(false);
			}else if (m_pTrack->kindObject == CGraphObject::KIND_EMITTER_TIME_LINE)
			{
				m_pTrack->DoubleClick();
				m_pTrack->OnChange();
				tr->CheckRangeEmitter();
