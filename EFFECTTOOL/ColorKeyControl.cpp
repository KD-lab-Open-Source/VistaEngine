...
void CColorKeyControl::OnMButtonDown(UINT nFlags, CPoint point)
{
	if(!m_pKey||!m_Valid)
		return;
	if (!m_bAlpha)
	...
	if (MessageBox("Вы уверены что хотите удалить этот и ключ и все следующие за ним?","Предупреждение!!!",MB_ICONQUESTION|MB_YESNO)!=IDYES)
		return;

	float d = 1/pKey->time;
	float t = pKey->time;
	for (int i=0; i<m_pKey->size()-1; i++)
	{
		KeyColor* k = &((*m_pKey)[i]);
		if (k->time >= t)
		{
			m_pKey->erase(m_pKey->begin()+i);
			i--;
		}else
		{
			k->time *= d;
	...
	Invalidate();
	pEmitter->ChangeParticleLifeTime(pEmitter->GetParticleLifeTime()*t);
	for (int i=0; i<pEmitter->p_velocity().size();i++)
	{
		pEmitter->p_velocity()[i].f *= t;
		//pEmitter->begin_speed()[i].mul *= t;
	}
