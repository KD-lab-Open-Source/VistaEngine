...
public:
	...
	void operator =(const CEmitterDeskr &copy);
};
...
public:
	...
	void operator =(const CEffectDeskr &copy);
};

class CGroupData : public CObject
{
	static UINT cur_id;
	UINT id;
public:
...
//	CArray<CEffectDeskr,CEffectDeskr> m_effect;
	...
	CString d2,d3;
	int m_bExpand,i2,i3;
	CGroupData(){id = cur_id++;i2 = i3 = 0; m_bShow3DBack = true; m_bExpand = true;};
	CGroupData(LPCTSTR name){ id = cur_id++; m_name = name; i2 = i3 = 0; m_bShow3DBack = true; m_bExpand = true;};
	void Init(){
//		m_effect.RemoveAll();
	};
	...
	void AddEmitter(LPCTSTR name);
	virtual void Serialize(CArchive& ar);
	void Reset(CGroupData* group);
	void Clear();
	bool operator ==(CGroupData& gr){return id==gr.id;}
};
*/
////
...
{
	CControlView *v_control;
	CEffectTreeView* m_pEffTree;

	CEffectData*	pActiveEffect;
	CEmitterData*	pActiveEmitter;
	CGroupData*		pActiveGroup;

	GroupListType m_group;
	bool sform;
...
//	void SaveAll();
	...
	void LoadWorld(bool select = false);
	string CEffectToolDoc::CopyTexture(string path, string& name);

public:
	int GroupsSize(){return m_group.size();}
	CGroupData* Group(int ix);
	int GroupIndex(CGroupData* group);
	void SetControlView(CControlView *view){v_control = view;}

	CEffectData*	ActiveEffect(){return  pActiveEffect; }
	CEmitterData*	ActiveEmitter(){return pActiveEmitter; }
	CGroupData*		ActiveGroup(){return   pActiveGroup;}

	CEffectData* SetActiveEffect(CEffectData* p)
	{
		return pActiveEffect = p;
	}
	CEmitterData* SetActiveEmitter(CEmitterData* p)
	{
		return pActiveEmitter = p;
	}
	CGroupData* SetActiveGroup(CGroupData* p)
	{
		return pActiveGroup = p;
	}
...
//	void LoadEmitter(CEffectData* pEffect, LPCTSTR szPath, LPCTSTR szFolderSprite, int mode = 1);
	...
	CEffectTreeView* GetEffTree(){return m_pEffTree;}
	void SetEffectTreeView(CEffectTreeView* tree){m_pEffTree = tree;}
...
public:
	...
	CGroupData* AddGroup();
	void DeleteGroup(CGroupData* group);
//	void DeleteCurEffect();
	virtual BOOL OnSaveDocument(LPCTSTR lpszPathName);
	CString m_StorePath;
	CString m_StorePathOld;
	void Init(void);
	...
	afx_msg void OnVisibleGroupNum(UINT nID);
	afx_msg void OnNodeNum(UINT nID);
	afx_msg void OnChainNum(UINT nID);
	...
	afx_msg void OnTbVertical();
	afx_msg void OnUpdateTbVertical(CCmdUI *pCmdUI);
	void OnFileOpen();
...
public:
	void Update();
	CUndoRedo &History(){return undo;}
	afx_msg void TestPut();
	afx_msg void TestUndo();
	afx_msg void SaveAs();
	afx_msg void UpdSaveAs(CCmdUI *pCmdUI);
