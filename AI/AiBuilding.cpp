...
RadiusScanOp::RadiusScanOp(const UnitReal* unit,const AttributeBase* attr, Player& aiPlayer, float extraRadius)
...
{	
	...
	if(pos.x  < radius || pos.x >= vMap.H_SIZE - radius || pos.y < radius || pos.y >= vMap.V_SIZE - radius 
		|| !unit_->rigidBody()->checkImpassabilityStatic(Vect3f(pos.x, pos.y, 0.f))){
		invalidPosition_ = true;
		return;
	}
...
bool UnitTargetScanOp::operator()(UnitBase* unit)
{
	...
	if(player_ != unitPlayer && !unitPlayer->isWorld()){
		...
		if(attr.isLegionary() || attr.isBuilding())
...
{
	if((vect1.norm()* vect2.norm()) > FLT_EPS)
		return Acos((vect1.x * vect2.x + vect1.y * vect2.y) / (vect1.norm()* vect2.norm()));
	else{
		xassert(0 && "(vect1.norm()* vect2.norm()) < FLT_EPS");
		return 0;
	}
}
...
PlaceScanOp::PlaceScanOp(UnitReal* unitToOrient, const AttributeBase* attribute, Player& aiPlayer, 
		...
		aiPlayer_(aiPlayer), closeToEnemy_(closeToEnemy), radius_(radius), maxLimit_(maxBuildingOfThisType),
		extraDistance_(extraDistance), foundPosition_(false)
{
	...
	FOR_EACH(universe()->Players, pi)
		if(aiPlayer_.isEnemy(*pi)){
			...
			if(!enemyCenter.eq(Vect2f::ZERO, FLT_EPS)){
				float angleEnemy = 0;
				if((angleEnemy = fabs(angleBeetwenVectors(posInstall - posCenter, enemyCenter - posCenter)) < M_PI * 0.33f) && angleEnemy < bestAngle){ 
...
{	
	...
	if(!radius_){
		...
		/*count_ = 0; // пока отключено за ненадобностью
		if(maxLimit_){
        ...
        for(int i = pass; i < pass + 4; i++){
			...
			if(curPass == 0){
				...
				{
					Vect2f installPos = installBound;
					installPos *= 0.5;
					...
					if(checkBuildingPosition(attrBuilding, installPos, &aiPlayer_, true, snapPoint_)){
						count_ = 0;
						if(extraDistance_ > FLT_EPS)
							universe()->unitGrid.Scan(installPos.x, installPos.y, extraDistance_ , *this);
						if(!count_){
							if(!closeToEnemy_){
		...
		float orientRadius = max(orientBound.x * 0.5f, orientBound.y * 0.5f);
		float installRadius = max(installBound.x * 0.5f, installBound.y * 0.5f);
...
void PlaceScanOp::show(Vect2f pos, const sColor4c& color)
{
