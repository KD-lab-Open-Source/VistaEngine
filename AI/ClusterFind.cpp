...
//		xassert(0);
	...
	AStarTile max_walk=max(cprev.walk,ceq.walk);
#ifdef CF_UP_BIT
	//Не считать непроходимым поле, 
	//если юнит направился сквозь него
	AStarTile xor_mask=((cprev.walk^ceq.walk)&UP_MASK)?0:UP_MASK;
#endif
	...
	for(t=0;t<=maxt;t++)
	{
		...
		if(q==eq && prev!=eq)
		{
			...
			HeuristicLine(xfrom,yfrom,xeq,yeq,
				dx,dy,walk_map,ClusterHeuristicComplex(),
					debug_xor);
		...
		for(int i=0;i<size;i++)
		{
			...
			while(1)
			{
				...
				if(i>0)
				{
					...
					AStarTile wfrom=walk_map[p0.y*dx+p0.x],
						 wto  =walk_map[p1.y*dx+p1.x];
					AStarTile max_walk=max(wfrom,wto);
				...
				if(i<size-1 && is_ok)
				{
					...
					AStarTile wfrom=walk_map[p1.y*dx+p1.x],
