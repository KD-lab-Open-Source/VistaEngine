...
cWaterPlume::cWaterPlume(UnitReal* unit, const WaterPlumeAttribute& attribute) : cBaseGraphObject(0), frequency(attribute.waterPlumeFrequency)
{
	rate = 0.0f;
...
void cWaterPlume::PreDraw(cCamera *pCamera)
{
	if(pCamera->TestVisible(pose.trans(), radius))
...
void cWaterPlume::Draw(cCamera *pCamera)
{
