...
void cWaves::PreDraw(Camera* camera)
{
	if (active)
...
cont:
			...
			{
				cEffect* eff = pScene->CreateEffectDetached(*pena, 0, true);		
...
/*
	...
	FOR_EACH(front[0], wl)
	{
		...
		rd->DrawLine(p1, p2, Color4c(255,0,0));
		p1.set(wl->rect[3].x, wl->rect[3].y,ez);
		rd->DrawLine(p2, p1, Color4c(255,0,0));
...
void cFixedWaves::ShowInfo(const Color4c& color)
{
	Color4c pntColor = Color4c(255,255,255);
	for (int i=0; i<points_.size(); i++)
	{
		Vect3f pos = points_[i];
		if (i == selectedPoint_)
			pntColor = Color4c(255,0,0);
		else
			pntColor = Color4c(255,255,255);
	...
	{
		gb_RenderDevice->DrawLine(segments_[i].begin-segments_[i].T1*distance_, segments_[i].end-segments_[i].T2*distance_,Color4c(255,255,0));
...
void cFixedWaves::serialize(Archive& ar)
{
	...
	ar.serialize(speed_,"speedWave", 0);
...
void cFixedWaves::PreDraw(Camera* camera)
{
}
void cFixedWaves::Draw(Camera* camera)
{
	if (points_.size() <= 1)
		return;
...
//////////////////////////////////////////////////////////////////////////
cFixedWavesContainer::cFixedWavesContainer(cWater* water, cTemperature* temperature) : BaseGraphObject(0)
{
	selected_ = -1;
	water_ = water;
...
void cFixedWavesContainer::ShowInfo()
{
	...
	{
		if (i == selected_)
			listWaves_[i]->ShowInfo(Color4c(0,0,255));
		else
...
void cFixedWavesContainer::PreDraw(Camera* camera)
{
	camera->Attach(SCENENODE_OBJECTSORT,this);
}
