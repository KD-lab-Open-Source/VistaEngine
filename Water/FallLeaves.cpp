...
void cFallLeaves::PreDraw(cCamera *pCamera)
{
	start_timer_auto();
...
void cFallLeaves::Draw(cCamera *pCamera)
{
	start_timer_auto();

	if(!pTexture || pTexture->GetFramesCount() == 0)
		return;
	...
	sColor4c tileMapColor(GetScene()->GetTileMap()->GetDiffuse());
	sColor4c sunColor(environment->environmentTime()->GetCurSunColor());
	sColor4c color(tileMapColor);

	Vect3f lightDirection = Vect3f::ZERO - environment->environmentTime()->sunPosition();
	lightDirection.normalize();

	Mat3f mat=pCamera->GetMatrix().rot();
...
void cFallLeaves::AnimateObjects(cCamera *pCamera)
{
	MTAuto lock(objects_lock);
	...
	for(int index=0;index<size;index++)
	{
		...
		const Vect3f& posf=p.model->GetPosition().trans();
...
void cFallLeaves::serialize(Archive& ar)
{
    ...
    if(!ar.serialize(textureAttributes_, "textureModelList", "Текстуры"))
		ar.serialize(textureAttributes_, "textureTextureAttribute", "Текстуры");
