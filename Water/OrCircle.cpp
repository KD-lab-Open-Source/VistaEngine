...
void DrawCircle(const Vect2f& pos,float r,sColor4c color=sColor4c(255,255,255))
{
...
void DrawSegment(const Vect2f& pos,float r,const Vect2f& A,const Vect2f& B,sColor4c color,sColor4c* pcolor2=NULL)
{
	float dalpha=3/r;
	...
	sColor4c color2=pcolor2?*pcolor2:color;
...
void TestIntersect(const Vect2f& pos0,float r0,const Vect2f& pos1,float r1)
{
	...
	if(!IntersectCircle(pos0,r0,pos1,r1,
					 A0,B0,A1,B1))
	{
		//DrawSegment(pos0,r0,A0,B0,sColor4c(0,0,255));
		DrawSegment(pos1,r1,A1,B1,sColor4c(0,0,255));
//		gb_RenderDevice->DrawRectangle(A0.x+pos0.x-1,A0.y+pos0.y-1,3,3,sColor4c(255,0,0));
//		gb_RenderDevice->DrawRectangle(B0.x+pos0.x-1,B0.y+pos0.y-1,3,3,sColor4c(0,255,0));
		gb_RenderDevice->DrawRectangle(A1.x+pos1.x-1,A1.y+pos1.y-1,3,3,sColor4c(255,0,0));
		gb_RenderDevice->DrawRectangle(B1.x+pos1.x-1,B1.y+pos1.y-1,3,3,sColor4c(0,255,0));
	}
...
void TestSegment(const Vect2f& pos0,float r0,const Vect2f& pos1,float r1,const Vect2f& pos2,float r2)
{
					 ...
					 A0,B0,A1,B1))
	{
						...
						A20,B20,A21,B21))
		{
			...
			if(segments>=1)
				DrawSegment(pos0,r0,s0.A,s0.B,sColor4c(0,0,255));
			if(segments>=2)
				DrawSegment(pos0,r0,s1.A,s1.B,sColor4c(255,0,0));
		}
	}
...
void OrCircle::testDraw()
{
	...
	FOR_EACH(circles,itc)
	{
		...
		FOR_EACH(segments,it)
		{
			...
			if(s.type==SS_SEGMENT)
			{
				DrawSegment(circle.pos,circle.r,s.A,s.B,colors[color],&sColor4c(255,255,255));
				//DrawSegment(circle.pos,circle.r,s.A,s.B,s.color,&s.pB->color);
/*
				if(s.pA->s0)
					DrawSegment(circle.pos,circle.r,s.A(),s.B(),s.pA->s0->color,&s.color);
				else
					DrawSegment(circle.pos,circle.r,s.A(),s.B(),sColor4c(0,0,0));
*/
				color=(color+1)%num_colors;
			}
			...
			if(s.type==SS_ALL_CIRCLE)
			{
				DrawCircle(circle.pos,circle.r,sColor4c(255,255,255));
			}
		}
	}
}
...
void OrCircle::get_spline(list<vector<OrCircleSpline> >& spline,float segment_len)
{
	vector<OrCircleSpline> vct;
	vector<Circle*>::iterator itc;
	FOR_EACH(circles,itc)
	{
		Circle& circle=**itc;
		...
		FOR_EACH(segments,it)
		{
			...
			if(!s.in_spline)
			{
				spline.push_back(vct);
				BuildSpline(spline.back(),&s,segment_len);
			}
		}
	}
...
void OrCircle::BuildSpline(vector<OrCircleSpline>& spline,Segment* begin,float segment_len)
{
	Segment* cur=begin;
	while(!cur->in_spline)
	{
...
void OrCircle::BuildSegment(vector<OrCircleSpline>& spline,Segment* segment,float segment_len,bool add_first,float length)
{
...
void OrCircle::get_circle_segment(list<vector<OrCircleSpline> >& spline,float segment_len,float length)
{
	vector<OrCircleSpline> vct;
	vector<Circle*>::iterator itc;
	FOR_EACH(circles,itc)
	{
		...
		FOR_EACH(segments,it)
		{
			Segment& s=**it;
			spline.push_back(vct);
			BuildSegment(spline.back(),&s,segment_len,true,length);
		}
	}
}
