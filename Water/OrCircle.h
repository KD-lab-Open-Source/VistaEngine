...
public:
	...
	void calc();
	void get_spline(list<vector<OrCircleSpline> >& spline,float segment_len);
	void get_circle_segment(list<vector<OrCircleSpline> >& spline,float segment_len,float length);
	...
	{	
		...
		Segment(){pA=pB=NULL;sA=sB=NULL;type=SS_ALL_CIRCLE;source=0;in_spline=false;}
	};
...
protected:
	...
	void BuildSpline(vector<OrCircleSpline>& spline,Segment* begin,float segment_len);
	void BuildSegment(vector<OrCircleSpline>& spline,Segment* begin,float segment_len,bool add_first,float length);
