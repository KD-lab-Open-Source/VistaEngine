...
void DrawRect(Vect3f p, Color4c c)
{
	static float d = 1000;
...
Vect2f cCircles::GetPtOnZ(Vect3f& p1,Vect3f& p2, Plane& back)
{
...
void cCircles::PreDraw(Camera* camera)
{
//	if(pWater)
//		camera->Attach(SCENENODE_OBJECTSORT,this);
}

void cCircles::Draw(Camera* camera, float dt)
{
	...
	pBuf->BeginDraw();
	{
		...
		for(it = drops.begin(); it != drops.end();){
			it->phasa+=(dt*it->time);
			if (it->phasa<=1){
				if(camera->scene()->GetFogOfWar())
					if(!camera->scene()->GetFogOfWar()->GetSelectedMap()->isVisible(Vect2f(it->pos.x,it->pos.y))){
						++it;
						continue;
					}
