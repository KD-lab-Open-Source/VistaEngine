...
void ColorMapManager::paintGeoColorMap(
	...
	for (
		...
		for (
			...
			point.x = vMapX;
			point.y = vMapY;
			point.z = vMap.GetAlt(vMapY * vMapWidth + vMapX) / 32.0 + zShift;
...
void ColorMapManager::lightUpGeoColorMap(
	...
	for (
		...
		for (
			...
			float h = vMap.GetAlt(offset);
			float h1 = vMap.GetAlt(offset + 1);
...
unsigned int ColorMapManager::getColor(int offset, bool showDamLayer) {
	...
	} else if (!showDamLayer || Vm_IsGeo(vMap.VxABuf[offset]) ) {
