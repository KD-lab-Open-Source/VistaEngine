...
void VMapOperator::planeToZeroplast(const Rect4i& rect) {
	for (int y = rect.yMin(); y < rect.yMax(); y++) {
		...
		for (int x = rect.xMin(); x < rect.xMax(); x++) {
			...
			//vMap.VxDBuf[offset + x] = 0;
			vMap.SetGeo(offset + x, vMap.hZeroPlast<<VX_FRACTION);
		}
	}
	updater->updateRect(rect);
}
...
void VMapOperator::planeDamLayer(int height, const Rect4i& rect) {
	...
	for (int y = rect.yMin(); y < rect.yMax(); y++) {
		...
		for (int x = rect.xMin(); x < rect.xMax(); x++) {
			...
			//	}
			//}
			vMap.SetGeo(offset, height<<VX_FRACTION);
		}
	}
	updater->updateRect(rect);
}
...
/*		int brushX = digArea.xMin() - vMapRect.xMin();
		...
		toolzerAligmentTerrainVariableH.influenceDZ(center.x, center.y, max(brWidthHalf, brHeightHalf), -10<<VX_FRACTION, 10 );
		toolzerAligmentTerrainVariableH.influenceDZ(center.x, center.y, max(brWidthHalf, 20+brHeightHalf), 0<<VX_FRACTION, 10 );
	/*
