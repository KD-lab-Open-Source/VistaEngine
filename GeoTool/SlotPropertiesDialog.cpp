...
bool CSlotPropertiesDialog::textureToControls(tx3d::Texture3D* tx, bool isMixer) {
	theApp.pSlotsDialog->setLocationsVisible(false);
	locationSelected = false;
...
bool CSlotPropertiesDialog::locationToControls(tx3d::Location3D* location) {
	theApp.pSlotsDialog->setLocationsVisible(true);
...
bool CSlotPropertiesDialog::slotUnchecking(Slot* slot, bool isMixer) {
	if (slot) {
		...
		if (treeManager->getFocusedItem() == NULL || treeManager->isTextureFocused()) {
			...
			HTREEITEM selected = treeManager->getSelectedTexture();
			if (newTx) {
				theApp.pSlotsDialog->textureSelected(newTx, selected);
			} else {
				theApp.pSlotsDialog->textureSelected(currentTx, selected);
			}
		} else {
			tx3d::Location3D* newLoc = controlsToLocation(currentLc);
			HTREEITEM selected = treeManager->getSelectedLocation();
			if (newLoc) {
				theApp.pSlotsDialog->locationSelected(newLoc, selected);
			} else {
				theApp.pSlotsDialog->locationSelected(currentLc, selected);
			}
		}
	}
	return true;
	...
	if (item != NULL) {
		...
		if (node) {
			...
			if (txNode) {
				...
				treeManager->setLocationSelected(theApp.pSlotsDialog->getSelectedSlotLocationItem());
				...
				textureToControls(currentTx, theApp.pSlotsDialog->isSelectedSlotMixer());
				return;
			}
			...
			if (locNode) {
				...
				treeManager->setTextureSelected(theApp.pSlotsDialog->getSelectedSlotTextureItem());
				HTREEITEM selected = treeManager->getSelectedTexture();
				if (selected != NULL) {
...
/*
	if (!controlsIsUpdating) {
		...
		UpdateData();
		shouldValidate = true;
		Slot* slot = theApp.pSlotsDialog->getSelectedSlot();
...
void CSlotPropertiesDialog::onActionSelected(ActionNode::ActionType type) {
	if ( theApp.pSlotsDialog->getSelectedActionType() != type ) {

		theApp.pSlotsDialog->actionSelected(type);
		slotChecked
			(
				theApp.pSlotsDialog->getSelectedSlot(),
				theApp.pSlotsDialog->isSelectedSlotMixer(),
				theApp.pSlotsDialog->getSelectedSlotTextureItem(),
				theApp.pSlotsDialog->getSelectedSlotLocationItem()
			);
	}
}
