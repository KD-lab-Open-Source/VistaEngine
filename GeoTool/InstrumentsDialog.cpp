...
void CInstrumentsDialog::updateTexture(Operator* op) {
	...
	if ( op->getViewportPos(&p) ) {
		if (!textureUpdating) {
			...
			if (theApp.pSlotsDialog->updateSelectedSlot()) {
				CWaitCursor wait;
				op->setTexture( theApp.pSlotsDialog->assembleTexture() );
	...
	if (theApp.slotPropertiesDialog.isLocationSelected()) {
			...
			if (needUpdateTexture) {
				...
				if ( op->cursorTrace(&p, &dir, &dir2) ) {
					if ( GetAsyncKeyState(VK_CONTROL) < 0 ) {
						theApp.pSlotsDialog->update(p, dir, dir2, LocationUpdater::SCALE);
					} else if ( GetAsyncKeyState(VK_SHIFT) < 0 ) {
						theApp.pSlotsDialog->update(p, dir, dir2, LocationUpdater::SHIFT_ZY);
					} else {
						theApp.pSlotsDialog->update(p, dir, dir2, LocationUpdater::SHIFT_XY);
					}
				}
			}
		} else {
		...
		if( op->getViewportPos(&p) && (GetAsyncKeyState(VK_LBUTTON) < 0) ) {
			if (!textureUpdating) {
				...
				if (theApp.pSlotsDialog->updateSelectedSlot()) {
					CWaitCursor wait;
					tx3d::Texture3D* tx = theApp.pSlotsDialog->assembleTexture();
					if (!tx) {
