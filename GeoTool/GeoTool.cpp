...
BOOL CGeoToolApp::InitInstance()
{
	...
	pSlotsDialog->Create(CSlots::IDD, m_pMainWnd);
	OnLoadMap();
	return TRUE;
}
...
{
	pSlotsDialog->ShowWindow(pSlotsDialog->IsWindowVisible() ? SW_HIDE : SW_SHOW);
}

void CGeoToolApp::OnUpdateShowSlots(CCmdUI* pCmdUI) 
{
	pCmdUI->SetCheck(pSlotsDialog->IsWindowVisible() ? 1 : 0);
}
...
void CGeoToolApp::OnLoadTexture() 
{
	...
	if (fileDialog.DoModal() == IDOK) {
		...
		pTx->referenced();
		pSlotsDialog->parseTexture(pTx);

		string folder;
		folder = string(fileDialog.GetPathName());
		folder.resize(folder.length() - 3);
		//string tmp=fileDialog.GetPathName();
		//string::size_type i=tmp.find_last_of("\\");
		//if(i!=string::npos)
		//	folder=tmp.substr(0, i);
		//else
		//	folder=".";
		//folder+="\\";
...
//
			...
			//ifstream ifsLattice((folder + "geoLattice.bin").c_str(), ios::binary | ios::in /*| ios::nocreate*/);
			//if (ifsLattice) {
		...
		XStream ifsLattice((folder + "geoLattice.bin").c_str(), XS_IN);
		if (ifsLattice) {
...
{
	tx3d::Texture3D* tx = pSlotsDialog->assembleTexture();
...
//				folder.resize(folder.length() - file.length());
				...
				ofs.open( (folder + "pal.pal").c_str(), ios::binary );
				//ofs.setmode(filebuf::binary);
				ofs.write((char*)(&binPal[0]), 768);
				ofs.close();
				//-----


				ofs.open( (folder + "geoLattice.bin").c_str(), ios::binary);
				//ofs.setmode(filebuf::binary);
				ofs.write((char*)(&lattice[0]), 65536);
				ofs.close();
