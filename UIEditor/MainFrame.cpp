...
{
	UI_FontLibrary::instance().editLibrary();
}

void CUIMainFrame::OnEditParameters()
{
    EditArchive ea;
    UI_Dispatcher::instance().serializeUIEditorPrm(static_cast<EditOArchive&>(ea));
	if(ea.edit()){
		UI_Dispatcher::instance().serializeUIEditorPrm(static_cast<EditIArchive&>(ea));
...
void CUIMainFrame::OnEditSnapOptions()
{
	...
	TreeControlSetup treeControlSetup(0, 0, 400, 600, ".\\Scripts\\TreeControlSetups\\UIEditorOptions", false, false);
	if(dlg.edit(Serializeable(Options::instance(), "Options", ""), GetSafeHwnd(), treeControlSetup))
		Options::instance().saveLibrary();
}
	...
	FOR_EACH(selection, it){
		...
		if(control && !control->states().empty()){
			...
			if(!mode->sprite().isEmpty()){
				Vect2f size = UI_Render::instance().relativeSpriteSize(mode->sprite());
