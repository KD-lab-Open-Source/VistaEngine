...
void Options::serialize (Archive& ar)
{
	...
	ComboListString resolution(GameOptions::instance().getUnfilteredList(OPTION_SCREEN_SIZE), buf);
	...
	sColor3c backgroundColor(backgroundColor_);
	ar.serialize(backgroundColor, "backgroundColor", "Цвет фона интерфейса");
	backgroundColor_ = sColor4c(backgroundColor.r, backgroundColor.g, backgroundColor.b, 255);

	sColor3c workspaceColor(workspaceColor_);
	ar.serialize((sColor3c&)(workspaceColor), "workspaceColor", "Цвет рабочей области");
	workspaceColor_ = sColor4c(workspaceColor.r, workspaceColor.g, workspaceColor.b, 255);
