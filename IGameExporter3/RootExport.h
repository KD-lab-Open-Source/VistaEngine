#pragma once
typedef map<IVisNode*,int> MAP_NODE;
...
public:
	...
	int FindNodeIndex(IVisNode* node);
	IVisNode* GetNode(int index)
	{
...
public:
	...
	bool IsLogicNodeBound(IVisNode* node);
...
protected:
	...
	bool IsIgnore(IVisNode* pGameNode,bool root);
	...
	bool CalcNodeAnimate(NextLevelNode& n,int interval_begin,int interval_size,bool cycled,bool& scale_error);
	...
	void FindLogicBounds(IVisMesh*& pLogicBound,vector<sLogicNodeBound>& node_bounds);
	void SaveLogicBounds(Saver& saver,IVisMesh* pLogicBound,vector<sLogicNodeBound>& node_bounds);
	void SaveLogicBoundRoot(Saver& saver,IVisMesh* pobject);
	void SaveLogicBoundLocal(Saver& saver,IVisMesh* pobject,int inode);
