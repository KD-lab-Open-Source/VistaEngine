...
//a0+a1*t+a2*t^2+a3*t^3==a0`+a1`*t`+a2`*t`^2+a3`*t`^3, где 
//t`=t*tscale+toffset
//t`=(t+0.5)*tscale-0.5
//toffset=-0.5+0.5*tscale
//ts=1/tscale
//to=-toffset*ts

//tm=t`*ts
//a0+a1*(tm+to)+a2*(tm^2+2*tm*to+to^2)+a3*(tm^3+3*tm^2*to+3*tm*to^2+to^3)

//a0+a1*to+a2*to^2+a3*to^3
//ts*(a1+a2*2*to+a3*3*to^2)
//ts^2*(a2+a3*3*to)
//ts^3*a3

void ScaleSpline(real& a0,real& a1,real& a2,real& a3,real tscale,real toffset)
{
	real ts=1/tscale,ts2=ts*ts,ts3=ts2*ts;
	real to=-toffset*ts,to2=to*to,to3=to2*to;
