...
//				s<<inode;
			...
			{
				IVisMaterial* node=ag.materials[j];
				s<<node->GetName();
			}
			s.pop();
		s.pop();
	...
	case C3DX_ACG_ONE:
		{
			...
			for(int i=0;i<size;i++)
			{
				string node_name;
				li>>node_name;
				IVisNode* node=pRootExport->Find(node_name.c_str());
				if(node)
...
int AnimationData::FindAnimationGroupIndex(IVisNode* node)
{
	for(int i=0;i<animation_group.size();i++)
	{
		AnimationGroup& g=animation_group[i];
		vector<IVisNode*>::iterator it=find(g.groups.begin(),g.groups.end(),node);
		if(it!=g.groups.end())
			return i;
	}
	return -1;
