...
string getStringFromReg(const string& folderName, const string& keyName) {
	...
	if ( lRet == ERROR_SUCCESS ) {
		lRet = RegQueryValueEx( hKey, keyName.c_str(), 0, 0, (LPBYTE) name, &nameLen );
...
void putStringToReg(const string& folderName, const string& keyName, const string& value) {
	...
	lRet = RegCreateKeyEx( HKEY_CURRENT_USER, folderName.c_str(), 0, "", 0, KEY_ALL_ACCESS, 0, &hKey, &dwDisposition );
...
cMoveEmblem::cMoveEmblem(CWnd* pParent /*=0*/)
	: CDialog(cMoveEmblem::IDD, pParent)
	, m_EnableLogo(FALSE)
	, m_PosX(_T(""))
	, m_PosY(_T(""))
	, m_Angle(_T(""))
	, m_TexturePath(_T("Textures\\"))
{
}
...
BOOL cMoveEmblem::OnInitDialog()
{
	...
	int ItemsCount = exporter->GetMaterialNum();
	vector<string> unkNames;
	for(int i=0;i<ItemsCount;i++)
	{
		
		IVisMaterial* material=exporter->GetMaterial(i);
		...
		{
			if(unkNames[j] == extractFileName(fname))
			{
				found = true;
				break;
			}
		...
		if(found)
			continue;
		unkNames.push_back(extractFileName(fname));
		int n = m_MaterialsList.InsertItem(i, extractFileName(fname).c_str());
		m_MaterialsList.SetItemData(n,i);


		sLogo logo(extractFileName(fname).c_str());
		for(int j=0; j<exporter->logos.size(); j++)
		{
			if(logo.TextureName == exporter->logos[j].TextureName)
			{
				logo.enabled = true;
				logo.rect = exporter->logos[j].rect;
				logo.angle = exporter->logos[j].angle;
			}
		}
...
void cMoveEmblem::OnLvnItemchangedMaterialsList(NMHDR *pNMHDR, LRESULT *pResult)
{
	...
	if (ItemIndex>-1 && pNMLV->uNewState == 3)
	{
		...
		IVisMaterial* material=exporter->GetMaterial(n);
			int  num_maps=material->GetTexmapCount();
			if(num_maps==0)
				return;
			string fname = extractFileName(material->GetTexmap(0)->GetBitmapFileName()); 
