...
protected:
	...
	void BuildTreeGroup();
	void AddTreeNode(IVisNode* cur,IVisNode* parent,HTREEITEM hparent);
	...
	void AddObjectInGroup(HTREEITEM hgroup,IVisNode* pGameNode);
	bool DeleteGroupObject(IVisNode* pGameNode,string group_name_);
	void ExpandAll(CTreeCtrl* tree,HTREEITEM hItem = TVI_ROOT);
protected:
	vector<string> animation_group;
	typedef map<IVisNode*,string> NODE_MAP;
	NODE_MAP node_map;
