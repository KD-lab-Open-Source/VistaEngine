...
const DWORD sizeDSBuffer=512*1024/4;
...
MpegFuncGetNextFile pMpegFuncGetNextFile=NULL;

void MpegSetGetNextFile(MpegFuncGetNextFile p)
{
	pMpegFuncGetNextFile=p;
}
...
//			OutputDebugString("Mpeg PLAY\n");
	...
	{
		if(pMpegFuncGetNextFile)
		{
			const char* f=pMpegFuncGetNextFile();
			if(f && MpegOpen(f))
				mpeg_state=MPEG_PLAY;
			else
				pMpegFuncGetNextFile=NULL;
		}
...
bool MpegStreamStop()
{
	...
	STREAM_COMMAND& sc=command[cc];
	num_command++;
	sc.command=STREAM_COMMAND::STOP;
...
static void ProcessStream()
{
	...
	bool enable_volume_to=true;
	if(sc.command==STREAM_COMMAND::VOLUME_TO)
	{
		DWORD status;
		if(FAILED(pDSBuffer->GetStatus(&status)))
			enable_volume_to=false;
		else
			enable_volume_to=(status&DSBSTATUS_PLAYING)?true:false;
	}

	if(sc.command==STREAM_COMMAND::VOLUME_TO && enable_volume_to)
	{
		sc.first=false;
		if(sc.first)
			sc.old_volume=volume;
	...
	if(sc.command==STREAM_COMMAND::STOP)
	{
		InternalMpegStop();
	}
...
int MpegGetLen(const char* fname)
{
	...
	bool bopen=MpegOpen(fname);
	if(!bopen)return -1;
	unsigned int ret=MpegGetNumOfSampleFrames();

	MpegClose();
	return (ret-1)*(1152)+(1342/2);
}