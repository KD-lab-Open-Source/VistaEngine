...
//
//////////////////////////////////////////////////////////////////////////////////
#ifdef _INC_STRSTREAM

//  Vect2s  I/O //////////////////////////
ostream& operator<<(ostream &os, const Vect2s& v)
{
	os << v.x << " \t" << v.y;
	return os;
}

istream& operator>>(istream &is, Vect2s& v)
{
	is >> v.x >> v.y;
	return is;
}
...
istream& operator>>(istream &is, Vect3d& v)
{
	is >> v.x >> v.y >> v.z;
	return is;
}
...
//  Vect3f  I/O //////////////////////////
ostream& operator<<(ostream &os, const Vect3f& v)
{
	os << v.x << " \t" << v.y << " \t" << v.z;
	return os;
}

istream& operator>>(istream &is, Vect3f& v)
{
	is >> v.x >> v.y >> v.z;
	return is;
}
...
//  Vect4f  I/O //////////////////////////
ostream& operator<<(ostream &os, const Vect4f& v)
{
	os << v.x << " \t" << v.y << " \t" << v.z << " \t" << v.w;
	return os;
}

istream& operator>>(istream &is, Vect4f& v)
{
	is >> v.x >> v.y >> v.z >> v.w;
	return is;
}
...
//  Mat3d I/O  ////////////////////////
ostream& operator<<(ostream& os, const Mat3d& m)
{
...
istream& operator>>(istream &is, Mat3d& m)
{
  is >> m.xx >> m.xy >> m.xz;
  is >> m.yx >> m.yy >> m.yz;
  is >> m.zx >> m.zy >> m.zz;
...
istream& operator>>(istream &is, Mat3f& m)
{
  is >> m.xx >> m.xy >> m.xz;
  is >> m.yx >> m.yy >> m.yz;
  is >> m.zx >> m.zy >> m.zz;
...
//  MatXd  I/O   ///////////////////
ostream& operator<<(ostream& os, const MatXd& m)
{
  return os << m.R << m.d;
}

istream& operator>>(istream& is, MatXd& m)
{
  Se3d T;
  is >> T;
  m.set(T);
...
//  MatXf  I/O   ///////////////////
ostream& operator<<(ostream& os, const MatXf& m)
{
  return os << m.R << m.d;
}

istream& operator>>(istream& is, MatXf& m)
{
  Se3d T;
  is >> T;
  m.set(T);
...
//  QuatD  I/O   ///////////////////
istream& operator>>(istream& is, QuatD& q)
{
  is >> q.s_ >> q.x_ >> q.y_ >> q.z_;
  return is;
}

ostream& operator<<(ostream& os, const QuatD& q)
{
...
//  QuatF  I/O   ///////////////////
istream& operator>>(istream& is, QuatF& q)
{
  is >> q.s_ >> q.x_ >> q.y_ >> q.z_;
  return is;
}

ostream& operator<<(ostream& os, const QuatF& q)
{
  os << q.s_ << " \t" << q.x_ << " \t" << q.y_ << " \t" << q.z_;
  return os;
}
...
//  Se3d I/O  ///////////////////
ostream& operator<<(ostream& os, const Se3d& se3)
{
  return os << se3.q << se3.d;
}

istream& Se3d::read(istream &is)
{
  // Read Se3d from input stream.  The Se3d specification must be
  // enclosed in curly brackets { }.  Se3d's are built up from a
  ...
  *this = Se3d::ID;
  ...
  if (is.peek() == '{') is.get(c);
    ...
    cerr << "Se3d::read : didn't find '{' \a" << endl;
    return is;
  }
  ...
  while (1) {
    ...
    if (i == STR_LENGTH || is.eof()) {
      cerr << "Se3d::read : didn't find '}' or specification too long\a" << endl;
      return is;
    }
  }
  ...
  while (!((iss >> tok).fail())) {
    ...
    if (!strcmp(tok, "trans")) {
      iss >> vect;
      op.set(QuatD::ID, vect);
    }
    else if (!strcmp(tok, "rot")) {
      iss >> tok >> vect;
      x = G2R(atof(tok));
      quat.set(x, vect);
      op.set(quat, Vect3d::ZERO);
    }
    else {
      cerr << "Se3d::read : unknown token " << tok << " \a" << endl;
      break;
    }
...
//  Se3f I/O  ///////////////////
ostream& operator<<(ostream& os, const Se3f& se3)
{
  return os << se3.q << se3.d;
}
istream& Se3f::read(istream &is)
{
  // Read Se3d from input stream.  The Se3d specification must be
  // enclosed in curly brackets { }.  Se3d's are built up from a
  // sequence of translation and rotation operations.  A translation
  ...
  *this = Se3f::ID;
  ...
  while (!((iss >> tok).fail())) {
    ...
    if (!strcmp(tok, "trans")) {
      iss >> vect;
      op.set(QuatF::ID, vect);
    }
    else if (!strcmp(tok, "rot")) {
      iss >> tok >> vect;
      x = G2R(atof(tok));
      quat.set((float)x, vect);
      op.set(quat, Vect3d::ZERO);
    }
    else {
      cerr << "Se3d::read : unknown token " << tok << " \a" << endl;
