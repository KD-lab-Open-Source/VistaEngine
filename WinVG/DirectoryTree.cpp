...
cRenderWindow* render_window=NULL;
void CDirectoryTree::OnPaint()
{
	if(!render_window)
	{
		render_window=gb_RenderDevice->CreateRenderWindow(m_hWnd);		
	}

	gb_RenderDevice->SelectRenderWindow(render_window);
	gb_RenderDevice->Fill(255,0,0,0);
	gb_RenderDevice->BeginScene();
	gb_RenderDevice->DrawRectangle(10,10,30,30,sColor4c(0,255,0));

	gb_RenderDevice->EndScene();
	gb_RenderDevice->Flush();

	gb_RenderDevice->SelectRenderWindow(NULL);
}
