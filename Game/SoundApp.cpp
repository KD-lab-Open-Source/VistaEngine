...
{
	return mpeg->state() == OGG_PLAYING;;
}

bool VoiceManager::isPaused() const
{
	return mpeg->state() == OGG_PAUSED;
}
...
bool VoiceManager::FadeVolume(float time, float newVolume)
{
...
bool MusicManager::FadeVolume(float time, float newVolume)
{
	return mpeg->fadeVolume(time, newVolume);
}
...
bool MusicManager::Play(const char* soundTrack)
{
	...
	bool b=mpeg->play(soundTrack_.c_str(), true);
	xassertStr(b && "Cannot open music: ", soundTrack_.c_str());
	return b;
}
