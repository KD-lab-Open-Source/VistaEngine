...
WRAP_LIBRARY(SoundTrackTable, "SoundTrackTable", "Звуковые треки", "Scripts\\Content\\SoundTrackTable", 0, false);

bool SoundTrack::Name::serialize(Archive& ar, const char* name, const char* nameAlt) 
{
	return ar.serialize(ResourceSelector(fileName_, ResourceSelector::SOUND_TRACK_OPTIONS), name, nameAlt);
}
    ...
    void activate() 
	{
		switch(actionType_){
			...
			case SET_VOLUME_FADE:
				musicManager.FadeVolume(fadeTime_, volume_);
				break;
			case SET_VOLUME_RESTORE:
				musicManager.SetVolume(terMusicVolume);
				break;
			case SET_VOLUME_RESTORE_FADE:
				musicManager.FadeVolume(fadeTime_, terMusicVolume);
				break;
		}
