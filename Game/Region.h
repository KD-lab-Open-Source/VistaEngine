...
public:
	...
	void ellipse(const Vect2f& pos,float radiusx,float radiusy);
...
public:
	...
	void operateByEllipse(const Vect2f& pos, float radiusx,float radiusy, int add);
...
protected:
	...
	{
		start_autostop_timer(Region_scan, STATISTICS_GROUP_AI);
		assert(positive_);
	...
	float distance(const Point& p0,const Point& p1,int dy);
...
{
public:
	RegionDispatcher(int sy,int deltay=1);
	void clear();
	...
	void operateByCircleDY(const Point& pos, float radius, int add) 
	{ 
		Vect2f posf(pos.x,pos.y/deltay);
		edit_column.operateByEllipse(posf, radius,radius/deltay, add);
	}
