...
void ShowHead::init()
{
	...
	xassert(scene);
	scene->SetSunDirection(Vect3f(0,-1,-1));
	...
	camera->SetPosition(MatXf(Mat3f(-M_PI_2, X_AXIS),Vect3f(0,-100,1000)));
}
...
bool ShowHead::LoadHead(const char* model_name, const char* main_chain, Color4c skin_color)
{
	playing = false;
	if(!model_name)
	{
	...
	{
		StaticAnimationChain* chain = head->GetChain(id_curren_chain);
		xassert(chain);
