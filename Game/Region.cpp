...
#ifdef UNIVERSALREGION
#undef start_autostop_timer
#undef statistics_add
#define start_autostop_timer(title, group) 
#define statistics_add(title, group, x) 
#endif


int toolzer_radius_min = 20;
int toolzer_radius_max = 70;
int toolzer_clear_back_layers_radius_delta = 32; // Запас для стирающих толзеров фоновых слоев
int toolzer_radius_fixed = 8; // Радиус рвов и насыпей
int toolzer_segment_autoclip_radius = 34; // Радиус взаимного клиппинга отрезков 
int toolzer_lineto_saved_segments_number = 1; // Число незатираемых отрезков с конца
float toolzer_lineto_radius_factor = 0.3f;

float region_segment_length = 40; // Определяет степень усреднения

int minimal_region_size = 20;
float region_perimeter_epsilon = 0.5f;
float rasterize_dlen = 0.65f;

int energy_region_vectorize_minimal_region_size = 8;
...
int enable_check = 1;
int enable_write_state = 1;

int cells_show_by_pixel = 0;
int cells_show_by_line = 0;

int region_show_cells = 0;
int region_show_cells_ccw = 0;
int region_show_cells_space = 20;

int show_locate = 0;

int show_scan = 0;
int show_scan_space = 10;

int show_edit_column = 0;
...
void Shape::ellipse(const Vect2f& pos,float radiusx,float radiusy)
{
	y0 = int(pos.y-radiusy+1+1e-3f);
	//((y-y0)/ry)^2+((x-x0)/rx)^2=1
	//x=x0+rx*sqrt(1-((y-y0)/ry)^2)
	//x=x0+rx*sqrt(1-((y-y0)/ry)^2)
	float inv_ry2=1.0f/(radiusy*radiusy);
	int y1=int(pos.y+radiusy-1e-3f);
	clear();
	reserve(y1-y0+1);
	for(int y = y0; y <= y1; y++)
	{
		float rx=radiusx*sqrtf(1 - sqr(y-pos.y)*inv_ry2);
		int minx = round(pos.x-rx);
		int maxx = round(pos.x+rx);
		push_back(Interval(minx, maxx+1));
	}
}
...
void Column::operateByEllipse(const Vect2f& pos, float radiusx,float radiusy, int add)
{
	Shape circle;
	circle.ellipse(pos,radiusx,radiusy);
	operate(circle, add);
}
...
float Region::distance(const Point& p0,const Point& p1,int deltay)
{
	float xx=p1.x-p0.x;
	float yy=(p1.y-p0.y)*deltay;
	return sqrtf(xx*xx+yy*yy);
}

bool Region::initSpline(int deltay)
{
	xassert(handle_);
	...
	do {
		...
		perimeter += distance(p0,p1,deltay);
		p0 = p1;
	} while(cell != handle_ || by_left != by_left0);
...
//			RegionDispatcher
//////////////////////////////////////////////////////////////////////////////////
RegionDispatcher::RegionDispatcher(int sy,int deltay_)
: toolzer_pos(0, 0),
toolzer_radius(40),
edit_column(sy),
rasterize_column(sy),
	...
	for(int pass = 0; pass < 2; pass++){
		FOR_EACH(seeds, si){
			...
			if(!seed->l_region && (pass || seed->l_cw == seed)){ // Positive first
				...
				region->parent = this;
				region->set(seed);
				if(region->numCells() >= minimalRegionSize && (!initSpline || region->initSpline(deltay))){
...
void RegionDispatcher::serialize(Archive& ar)
{
	XBuffer buf(20000, 1);
	if (ar.isOutput ()) {
		save (buf);
		saveEditing (buf);
		ar.serialize(buf, "region", "Region");
	}
	else if(ar.serialize(buf, "region", "Region") && buf.tell()){
		buf.set(0);
		load (buf);
		loadEditing (buf);
	}
