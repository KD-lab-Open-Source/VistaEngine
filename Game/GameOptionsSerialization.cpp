...
const char* gameOptionsFileName()
{
	if(isUnderEditor())
		return "Scripts\\Content\\GameOptionsEditor";
	else if(isFileExists("Scripts\\ContentLocal\\GameOptions"))
		return "Scripts\\ContentLocal\\GameOptions";
	else
...
GameOptions::GameOptions()
{
	...
	locDatas_.push_back(Language(UI_COMMON_TEXT_LANG_ENGLISH,	"english",	1252));
	locDatas_.push_back(Language(UI_COMMON_TEXT_LANG_RUSSIAN,	"russian",	1251));
	locDatas_.push_back(Language(UI_COMMON_TEXT_LANG_GERMAN,	"german",	1252));
	locDatas_.push_back(Language(UI_COMMON_TEXT_LANG_FRENCH,	"french",	1252));
	locDatas_.push_back(Language(UI_COMMON_TEXT_LANG_SPANISH,	"spanish",	1252));
	locDatas_.push_back(Language(UI_COMMON_TEXT_LANG_ITALIAN,	"italian",	1252));

	resolutions_.push_back(Vect2i(640, 480)); //VGA
	resolutions_.push_back(Vect2i(720, 480));
	resolutions_.push_back(Vect2i(720, 576));
	resolutions_.push_back(Vect2i(800, 600)); //SVGA
	resolutions_.push_back(Vect2i(848, 480));
	resolutions_.push_back(Vect2i(852, 480)); //WVGA
	resolutions_.push_back(Vect2i(858, 484)); //WVGA
	resolutions_.push_back(Vect2i(960, 600));
	resolutions_.push_back(Vect2i(1024, 600));
	resolutions_.push_back(Vect2i(1024, 768)); //XGA
	...
	resolutions_.push_back(Vect2i(1280, 1024)); //SXGA
	resolutions_.push_back(Vect2i(1360, 768));
	resolutions_.push_back(Vect2i(1366, 768));  //WXGA
	resolutions_.push_back(Vect2i(1400, 1050)); //SXGA+
	resolutions_.push_back(Vect2i(1440, 900));
	resolutions_.push_back(Vect2i(1600, 900));
	resolutions_.push_back(Vect2i(1600, 1024)); //WSXGA
	resolutions_.push_back(Vect2i(1600, 1200)); //UXGA
	resolutions_.push_back(Vect2i(1680, 945));
	resolutions_.push_back(Vect2i(1680, 1050)); //WSXGA+
	resolutions_.push_back(Vect2i(1920, 1080));
	resolutions_.push_back(Vect2i(1920, 1200)); //WUXGA
	resolutions_.push_back(Vect2i(1920, 1440));
	resolutions_.push_back(Vect2i(2048, 1080));
	resolutions_.push_back(Vect2i(2048, 1536)); //QXGA
	resolutions_.push_back(Vect2i(2560, 1600));
	resolutions_.push_back(Vect2i(2560, 2048)); //QSXGA
	resolutions_.push_back(Vect2i(3200, 2048)); //WQSXGA
	resolutions_.push_back(Vect2i(3200, 2400)); //QUXGA
	resolutions_.push_back(Vect2i(3840, 2400)); //WQUXGA
	resolutions_.push_back(Vect2i(5120, 4096)); //HSXGA
	resolutions_.push_back(Vect2i(6400, 4096)); //WHSXGA
	...
	gameOptionPrms_[OPTION_LANGUAGE]				.set(GAME|INDEXED,		TYPE_LIST, 0, locDatas_.size()-1);
	...
	gameOptionPrms_[OPTION_SHADOW]					.set(GRAPHICS,	TYPE_LIST,	0, 2);
	...
	gameOptionPrms_[OPTION_UNIT_SPRITE_SCALE]		.set(GAME|INSTANT,	TYPE_PHASA, 0.5f, 1.5f);
	...
	options_.push_back(Option(OPTION_UNIT_SPRITE_SCALE, 0.5f));
	...
	LanguagesList::const_iterator ld_it;
	FOR_EACH(locDatas_, ld_it){
		locDataComboPaths_ += ld_it->language_;
		locDataComboPaths_ += "|";
	}
	locDataComboPaths_.pop_back();
...
void GameOptions::setTranslate()
{
	...
	buf.init();
	for(int i = 0; i < locDatas_.size(); ++i)
		buf < getLocString(locDatas_[i].lockKey_, a2w(getEnumName(locDatas_[i].lockKey_)).c_str()) < L"|";
	--buf; buf < L'\0';
	gameOptionPrms_[OPTION_LANGUAGE].setTranslate(buf);

	buf.init();
	for(int i = 0; i < resolutions_.size(); ++i)
		buf <= resolutions_[i].x < L"*" <= resolutions_[i].y < L"|";
	--buf; buf < L'\0';
	...
	for(int i = 1; i < antialiasModes_.size(); ++i)
		buf < L"|" <= (int)antialiasModes_[i] < L"x";
	gameOptionPrms_[OPTION_ANTIALIAS].setTranslate(buf);

	buf.init();
	buf < GET_LOC_STR(UI_COMMON_TEXT_DISABLED);
	for(int i = 1; i < anisatropicFiltering_.size(); ++i)
		buf < L"|" <= (int)anisatropicFiltering_[i] < L"x";
	gameOptionPrms_[OPTION_ANISOTROPY].setTranslate(buf);

	buf.init();
	buf < GET_LOC_STR(UI_COMMON_TEXT_DISABLED) < L"|" < GET_LOC_STR(UI_COMMON_TEXT_SHADOW_BAD) < L"|" < GET_LOC_STR(UI_COMMON_TEXT_SHADOW_GOOD);
	gameOptionPrms_[OPTION_SHADOW].setTranslate(buf);

	buf.init();
	buf < GET_LOC_STR(UI_COMMON_TEXT_TEXTURE_LOW) < L"|" < GET_LOC_STR(UI_COMMON_TEXT_TEXTURE_MEDIUM) < L"|" < GET_LOC_STR(UI_COMMON_TEXT_TEXTURE_GOOD);
	gameOptionPrms_[OPTION_TEXTURE_DETAIL_LEVEL].setTranslate(buf);

	buf.init();
	buf < GET_LOC_STR(UI_COMMON_TEXT_DISABLED) < L"|" < GET_LOC_STR(UI_COMMON_TEXT_EFFECT_SMOOTH_TERRAIN) < L"|" < GET_LOC_STR(UI_COMMON_TEXT_EFFECT_SMOOTH_FULL);
	gameOptionPrms_[OPTION_SOFT_SMOKE].setTranslate(buf);

	buf.init();
	buf < GET_LOC_STR(UI_COMMON_TEXT_HIGH_QA) < L"|" < GET_LOC_STR(UI_COMMON_TEXT_LOW_QA);
...
{ // тут проверять независимые ни от чего настройки, например список языков
	...
	for(int i = 0; i < locDatas_.size(); ++i)
		prmLang.valid_[i] = find(LDmask.begin(), LDmask.end(), locDatas_[i].language_) != LDmask.end();
...
void GameOptions::Option::serialize(Archive& ar)
{
	...
	if(type_ >= 0 && type_ < GameOptions::instance().gameOptionPrms_.size()){
		...
		case TYPE_BOOL:
			if(ar.isEdit()){
				WBuffer buf;
				ComboVectorWString lst(GameOptions::instance().getList(buf, type_), data_);
...
{
	XPrmIArchive ar;
	if(ar.open("Scripts\\Content\\GraphPresets"))
		serializePresets(ar);
}

void GameOptions::savePresets()
{
	XPrmOArchive ar("Scripts\\Content\\GraphPresets");
	serializePresets(ar);
}
...
void GameOptions::OptionPrm::set(int kindType, ValueType valueType, float min, float max, const wchar_t* lst)
{
	kindType_ = kindType;
	Vtype_ = valueType;
...
void GameOptions::OptionPrm::setTranslate(const wchar_t* lst)
{
	xassert(lst);
	valueList_ = lst;
}
...
const wchar_t* GameOptions::getUnfilteredList(GameOptionType type) const
{
	const OptionPrm& prm = gameOptionPrms_[type];
	return prm.valueList_.c_str();
}

const wchar_t* GameOptions::getPresetList(WBuffer& out) const
{
	if(presets_.empty())
		return L"";
...
const wchar_t* GameOptions::getList(class WBuffer& out, GameOptionType type) const
{
	out.init();
	...
	if(prm.isIndexed()){
		...
		splitComboListW(strings, prm.valueList_.c_str(), L'|');
		dassert(strings.size() == prm.valid_.size());
		for(int i = 0; i < strings.size(); ++i)
			if(prm.valid_[i])
				out < strings[i].c_str() < L"|";
		if(out.tell()){
			--out;
			out < L"\0";
		}
