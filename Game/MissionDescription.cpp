...
const int ReplayHeader::FilePlayReelVersion = 12;
...
MissionDescription::MissionDescription(const char* fname, GameType gameType) 
{
	...
	worldGUID_ = XGUID::ZERO;
	missionGUID_ = XGUID::ZERO;
...
void MissionDescription::setDifficulty(const Difficulty& dif)
{
	for(int slot = 0; slot < NETWORK_PLAYERS_MAX; ++slot){
		if(slotsData[slot].realPlayerType == REAL_PLAYER_TYPE_AI)
			slotsData[slot].difficulty = dif;
	}
}
...
const wchar_t* MissionDescription::getMapSizeName(float size)
{
...
const wchar_t* MissionDescription::getMapSizeName() const
{
...
{ 
	...
	in.read(&worldGUID_, sizeof(worldGUID_));
	for(int i = 0; i < NETWORK_PLAYERS_MAX; i++)
...
{ 
	...
	out.write(&worldGUID_, sizeof(worldGUID_));
	for(int i = 0; i < NETWORK_PLAYERS_MAX; i++)
...
void MissionDescription::serialize(Archive& ar)
{
	...
	if(ar.isOutput() && !ar.isEdit())
		saveTime_.setCurrentTime();
	ar.serialize(saveTime_, "saveTime_", 0);
	...
	ar.serializeArray(slotsData, "slotsData", 0);
	...
	ar.serialize(worldGUID_, "worldGUID", 0);
	ar.serialize(missionGUID_, "missionGUID", 0);
...
const wchar_t* MissionDescription::getPlayerName(WBuffer& name, int slotID, int cooperativeIdx) const 
{ 
	...
	//if(sd.playerID!=PlayerData::PLAYER_ID_NONE){
		...
		if(idx!=USER_IDX_NONE){
			return (name < usersData[idx].playerNameE).c_str();
		}
	//}
	return L"";
}
void MissionDescription::setPlayerName(int slotID, int cooperativeIdx, const wchar_t* str)
{ 
	const SlotData& sd=playerData(slotID);
	//if(sd.playerID!=PlayerData::PLAYER_ID_NONE){
		int idx=sd.usersIdxArr[cooperativeIdx];
		if(idx!=USER_IDX_NONE){
			usersData[idx].setName(w2a(str).c_str());
		}
	//}
}
...
void MissionDescriptions::readFromDir(const char* path, GameType gameType)
{
	...
	string mask = fixedPath + "*." + MissionDescription::getExtention(gameType);
...
void MissionDescriptions::readUserWorldsFromDir(const char* path)
{
	...
	string mask = fixedPath + "*." + MissionDescription::getExtention(GAME_TYPE_MULTIPLAYER);
...
const MissionDescription* MissionDescriptions::find(const wchar_t* missionName) const
{
	string name;
	w2a(name, missionName);
	const_iterator i;
	FOR_EACH(*this, i)
		if(!stricmp(name.c_str(), i->interfaceName()))
			return &*i;
	return 0;
}
