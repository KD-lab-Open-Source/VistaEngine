...
public:
	...
	void selectAll(const AttributeBase* p, bool onlyPowered = false);
	...
	void makeCommandAttack(WeaponTarget& target, bool shiftPressed = false, bool byMinimap = false, bool moveToTarget = true);
	void makeCommandSubtle(const CommandID& command_id, const Vect3f& position, bool shiftPressed = false, bool byMinimap = false);
	...
	int getTraficabilityCommonFlag();
...
private:
	...
	struct ChangeDatum
	{
		UnitInterface* oldSquad;
		UnitInterface* newSquad;
		ChangeDatum(UnitInterface* _oldSquad, UnitInterface* _newSquad) : oldSquad(_oldSquad), newSquad(_newSquad) {}
	};
	typedef vector<ChangeDatum> ChangeData;
