...
/// Графическое представление предмета в инвентаре
...
/// Инвентарь, графическое представление.
...
public:
	...
	/// лежащие в инвентаре предметы не учитываются, смотрится только по ячейкам
...
private:
	...
	/// тип ячеек
	/** 
	по этому типу проверяется, может ли объект
	быть положен в ячейку
	*/
	InventoryCellType cellType_;

	UI_InventoryType inventoryType_;
	/// тип слота (для снаряжения)
	EquipmentSlotType equipmentSlotType_;
	/// тип слота (для снаряжения быстрого доступа)
	QuickAccessSlotType quickAccessSlotType_;

	/// вид ячейки в пустом состоянии
	UI_Sprite cellEmptySprite_;
	/// вид заполненной ячейки
	UI_Sprite cellSprite_;
	/// вид активированной ячейки (для инвентаря быстрого доступа)
	UI_Sprite cellActiveSprite_;

	/// подсвеченная область (показывает куда ляжет предмет с мыши)
	Recti highlight_;
	...
	/// берётся из логического диспетчера интерфейса
