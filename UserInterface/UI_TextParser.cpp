...
UI_TextParser::UI_TextParser(const cFont* font)
{
	if(font){
		fontMap_ = &(font->GetInternal()->Font);
		scaleX_ = font->GetScale().x * font->GetTexture()->GetWidth();
		fontHeight_ = round(font->size());
	}
...
{
	fontMap_ = src.fontMap_;
	scaleX_ = src.scaleX_;
	fontHeight_ = src.fontHeight_;
...
void UI_TextParser::init()
{
	...
	size_.set(0, fontHeight_);
	lineCount_ = 1;
}

void UI_TextParser::setFont(const cFont& font)
{
	fontMap_ = &(font.GetInternal()->Font);
	scaleX_ = font.GetScale().x * font.GetTexture()->GetWidth();
	fontHeight_ = round(font.size());
...
void UI_TextParser::parseString(const char* text, const sColor4c& color, int fitIn)
{
	xassert(fontMap_);
	init();

	fitIn_ = fitIn > 2 * fontHeight_ ? fitIn : -1;
	...
	size_.y = fontHeight_ * lineCount_;
}
			...
			if(const UI_Sprite* sprite = UI_SpriteReference(img_name.c_str()))
				if(!sprite->isEmpty()){
					...
					if((node.style & 0x03) != 2)
						...
						node.width = round(size.x / size.y * fontHeight_);
					}
					++pstr_;
...
void UI_TextParser::getColor(const sColor4c& defColor)
{
	sColor4c diffuse = defColor;
