...
{
public:
	Skate(cScene* scene, cCamera* camera)
	{
		scene_ = scene;
		camera_ = camera;
		id_swim = -1;
	...
	bool Init(const char* modelName, const char* logoName,const SoundAttribute* swimAttr,const SoundAttribute* stopAttr,const SoundAttribute* riseAttr)
	{
		...
		cAnimationChain* chain = model->GetChain(model->GetChainIndex("KDV_include"));
		xassert(chain);
	...
	void Animate(float dt)
	{
		...
		if(model_kdv)
			model_kdv->SetPosition(modelPos);
		Vect2i mouse_pos = UI_Render::instance().mousePosition() * Vect2f(screenSize);
		...
		if(movePhase==1)
		{
			...
			if(id_current != id_drink&&!isDone_)
			{
				...
				id_current = id_drink;
				phase = 0;
				cAnimationChain* chain = model->GetChain(id_drink);
				xassert(chain);
			...
			if(isDone_&&id_stand != -1&&id_current != id_stand)
			{
				...
				id_current = id_stand;
				phase = 0;
				cAnimationChain* chain = model->GetChain(id_stand);
				xassert(chain);
				chain_len = 1.0f/chain->time;
				fishRise_.Stop();
				startSoundRise_ = false;
...
void ReelManager::showLogoModal(LogoAttributes& logoAttributes, const cBlobsSetting& blobsSetting, int stableTime,SoundLogoAttributes& soundAttributes)
{
	...
	xassert(scene);
	scene->SetSun(Vect3f(1,0,0),sColor4f(1,1,1,1),sColor4f(1,1,1,1),sColor4f(1,1,1,1));
	...
	if(dno)
	{
		...
		cAnimationChain* chain = dno->GetChain(id_stand);
		xassert(chain);
		chain_len = 1.0f/chain->time;
	}
	...
	Vect2i oldMousePos = UI_Render::instance().mousePosition() * Vect2f(screenWidth, screenHeight);
	while (isVisible()) {
			sndSystem.Update();
			...
			Vect2i mouse_pos = UI_Render::instance().mousePosition() * Vect2f(screenWidth, screenHeight);
