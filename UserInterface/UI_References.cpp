...
UI_ControlBase* UI_ControlReference::getControlByID(int id)
{
	...
	if(!ui_ControlMapReference.exists(id)){
		xxassert(0, XBuffer() < "Кнопка с ID=" <= id < " не найдена");
		return 0;
	}
#endif	
...
{
	if(ar.isEdit()){
		if(ar.openStruct(*this, name, nameAlt, typeid(UI_ControlReference).name())){
			ComboListString combo_str(comboList(), reference_.c_str());
			bool result = ar.serialize(combo_str, "ref", "^Ref");
			ar.closeStruct(name);
			if(ar.isInput())
