...
#define SELECT_TRANSACTION() Transaction auto_transaction(this)
#define CSELECT_TESTLOCK() xassert(getLock().locked())
...
void SelectManager::selectAll(const AttributeBase* unitType) 
{
	MTG();
	if(!player_ || gameShell->directControl() || !player_->controlEnabled()) 
		return;
...
void SelectManager::changeUnit(UnitInterface* oldUnit, UnitInterface* newUnit)
{
	...
	if(oldUnit->selected())
		if(newSquad == oldSquad){
			oldUnit->setSelected(false); // апгрейд, не слать в триггеры
			universe()->sendCommand(netCommand4G_UnitCommand(oldUnit->unitID().index(), UnitCommand(COMMAND_ID_UNIT_DESELECTED, player_->teamIndex())));
			newUnit->setSelected(true); // не слать в триггеры
			UnitCommand command(COMMAND_ID_UNIT_SELECTED, player_->teamIndex());
			command.setShiftModifier(false);
			universe()->sendCommand(netCommand4G_UnitCommand(newUnit->unitID().index(), command));
			selected_valid_ = false;
		}
		else {
			Transaction auto_transaction(this, true);
			UnitInterfaceList::iterator it = findInSelection(oldUnit);
			if(it != selection_.end()){
				erase(it);
				addToSelection(newUnit);
			}
		}
...
void SelectManager::makeCommandSubtle(const CommandID& command_id, const Vect3f& position, bool shiftPressed)
{
...
//			PositionGeneratorCircle<UnitInterfaceList> positionGenerator;
//			positionGenerator.init(0, Vect2f(position), &selectionCommandCopy);

			UnitInterfaceList::iterator ui = selectionCommandCopy.begin();
			while(ui != selectionCommandCopy.end()){ 
//				unitCommand.setPosition(To3D(positionGenerator.get(*ui, false, false)));
				(*ui)->sendCommand(unitCommand);
				++ui;
			}
		}
		else
...
void SelectManager::makeCommandAttack(WeaponTarget& target, bool shiftPressed)
{
	if(gameShell->underFullDirectControl())
		return;
	...
	UnitCommand cmd = (target.unit()) ? UnitCommand(COMMAND_ID_OBJECT, target.unit(), target.weaponID()) : 
...
void SelectManager::explodeUnit()
{
	...
	UnitInterfaceList::iterator ui;
	FOR_EACH(selection_, ui)
		(*ui)->sendCommand(UnitCommand(COMMAND_ID_EXPLODE_UNIT_DEBUG, 0));
}
