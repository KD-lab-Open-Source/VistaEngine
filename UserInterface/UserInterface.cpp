...
void UI_Screen::serialize(Archive& ar)
{
	...
	if(ar.isEdit()){
		...
		ar.serialize(backgroundModelName_, "backgroundModelName", "3D Модель");
...
void SelectionBorderColor::serialize(Archive& ar)
{
	...
	ar.serialize(ResourceSelector(selectionTextureName_,ResourceSelector::TEXTURE_OPTIONS), "selectionTextureName", "Рамка выделения: Текстура");
	ar.serialize(ResourceSelector(selectionCornerTextureName_,ResourceSelector::TEXTURE_OPTIONS), "selectionCornerTextureName", "Рамка выделения: Текстура верхнего левого угла");
	ar.serialize(needSelectionCenter_, "needSelectionCenter", "Рамка выделения: Внутреняя текстура");
	ar.serialize(ResourceSelector(selectionCenterTextureName_,ResourceSelector::TEXTURE_OPTIONS), "selectionCenterTextureName", "Рамка выделения: Текстура внутри");
}
...
{
	ar.serialize(tipsDelay_, "tipsDelay", "задержка появления подсказок");
...
void UI_Dispatcher::redraw() const
{
	...
	if(isEnabled_){
		// Рисуется до всего (потому как бэкграунд)
		if (UI_BackgroundScene::instance().ready())
...
REGISTER_ENUM(UI_ACTION_EXPAND_TEMPLATE, "управление интерфейсом\\раскрыть шаблоны")
...
REGISTER_ENUM(UI_ACTION_ANIMATION_CONTROL, "управление интерфейсом\\управление анимацией")
...
REGISTER_ENUM(UI_ACTION_UNIT_PARAMETER, "вывод значений\\вывод параметра текущего юнита")
...
REGISTER_ENUM(UI_ACTION_BIND_TO_UNIT, "привязки\\к конкретному юниту")
REGISTER_ENUM(UI_ACTION_UNIT_ON_MAP, "привязки\\cуществует на мире")
...
REGISTER_ENUM(UI_ACTION_UNIT_HAVE_PARAMS, "привязки\\отсутствие параметров у юнита")
...
REGISTER_ENUM(UI_ACTION_BUILDING_CAN_INSTALL, "привязки\\можно построить здание")
...
REGISTER_ENUM(UI_ACTION_PRODUCTION_PROGRESS, "вывод значений\\отображение прогресса производства")
...
REGISTER_ENUM(UI_ACTION_SOURCE_ON_MOUSE, "команды\\управление источником на мыши")
...
REGISTER_ENUM(GAME_INFO_TAB_LIST, "Табулированный набор")
...
REGISTER_ENUM(GAME_INFO_PLAYERS_NUMBER, "Количество игроков")
