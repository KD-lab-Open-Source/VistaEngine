...
STARFORCE_API void Profile::serialize(Archive& ar)
{
	...
	ar.serialize(lastInetDirectAddress, "lastInetDirectAddress", 0);
	...
	ar.serialize(difficulty, "difficulty", 0);
	ar.serialize(passedMissions, "passedMissions", 0);
...
currentProfileIndex_(-1)
{
		...
		szPath))) 
	{
		...
		path += GlobalAttributes::instance().savePath;
		path += "\\";
	}
	else
		path = "Resource\\Saves\\";
	const_cast<string&>(rootPath) = path;
...
int ProfileManager::updateProfile(const wstring& name)
{
...
{
	if(dir.substr(0, 1) == "."){
		xassert(false && "А-А-А-А-А-А-А-А-А-А !!!!!");
		ErrH.Exit();
		return;
	}
	WIN32_FIND_DATA findFileData;
...
int ProfileManager::getProfileByName(const wstring& name) const
{
	int idx;
...
STARFORCE_API void ProfileManager::serialize(Archive& ar)
{
	...
	if(ar.isInput() || currentProfileIndex() >= 0){
		wstring lastProfileName = profile().name;
...
bool ProfileManager::isProfileExist(const wchar_t* profile_name) const
{
	wstring name(profile_name);
